...
\end{equation}
其中$T_pN$是$N$在$p$点的切空间（等同于$\imath_* (T_pN)\subset T_{\imath(p)}{M}$），
...
\end{equation}
称为子流形$\imath : N\to {M}$在点$\imath(p)$的{\heiti 法空间}（Normal）；
...
上述分类不涉及零模矢量；在第\ref{chnull}章会初步涉及零模法矢量的内容．
...
\subsection{子流形联络}
设${\nabla}_a$是广义黎曼流形$({M},{g})$的Levi-Civita联络．
$\forall X^a, Y^a \in \mathfrak{X}(N)$，
切矢量$\imath_* X^a$、$\imath_* Y^a$是${M}$中沿$\imath$的切矢量；
为使意义清晰，我们将多次写出“$\imath_*$”等记号．
显然${\nabla}_{\imath_* X} (\imath_* Y^a) \in \mathfrak{X}({M})$，
但它未必只包含$\imath_* (\mathfrak{X}(N)) \subset \mathfrak{X}({M})$中切矢量，
还可能包含法向矢量；利用式\eqref{chsm:eqn_orthogonal-decomposition}，可以假设
...
\eqref{chsm:eqn_Gauss}式通常称为{\bfseries\heiti Gauss公式}；
$K^a_{bc}$称为等距浸入$\imath$的{\heiti 第二基本形式}，
或称为子流形$N$在流形${M}$中的第二基本形式．
...
\end{theorem}
\begin{proof}
    $\forall f\in C^\infty(N)$、$\forall X^a,Y^a,Z^a\in TN$，
    ...
    \begin{align*}
        ...
        = {\rm D}_{X} (Y^a)+X(f)Z^a +f {\rm D}_{X} (Z^a). \\
    ...
    \end{align*}
    上面验证了线性属性以及Leibnitz律．下面验证加法：
    \begin{align*}
        {\rm D}_{X+fY} Z^a=&{}^{\top}\bigl({\nabla}_{X+fY} Z^a\bigr) 
        ={}^{\top}\bigl({\nabla}_{X} Z^a+{\nabla}_{fY} Z^a\bigr) \\
        =&{}^{\top}\bigl({\nabla}_{X} Z^a\bigr)+f \left({}^{\top}\bigl({\nabla}_{Y} Z^a\bigr)\right)
        ={\rm D}_{X} Z^a + f{\rm D}_{Y} Z^a . \\
        K^a_{bc} (X^b+fY^b, Z^c)=&{}^{\bot}\bigl({\nabla}_{X+fY} Z^a\bigr) 
        ={}^{\bot}\bigl({\nabla}_{X} Z^a+{\nabla}_{fY} Z^a\bigr) \\
        =&{}^{\bot}\bigl({\nabla}_{X} Z^a\bigr)+f\left({}^{\bot}\bigl({\nabla}_{Y} Z^a\bigr)\right)
    ...
    \end{align*}
    注意到$Y^b$、${Z}^c$是切矢量部分，而$K^b_{ad} (X^a, Y^d)$是法矢量部分，故两者内积必然为零．
...
${\rm D}_a ^\bot: \mathfrak{X}^\bot(N)\times \mathfrak{X}(N)
\to \mathfrak{X}^\bot(N)$．
...
$N$上的$\binom{1}{1}$型光滑张量场；但因$\xi^a \in \mathfrak{X}^\bot (N)$，
故$S$（及$K$）不是流形$N$上的$\binom{1}{2}$型张量场．
但$S$和$K$却是流形${M}$上的$\binom{1}{2}$型张量场．
...
\begin{align}
    ...
    ={g}_{ab} X^a S^b_{\xi c} Y^c , \label{chsm:eqn_sym-Sh}\\
...
上式第二行我们给出了此对称性用\S \ref{chmla:sec_tensor}记号的表示方法，
但没有给方程式标号．
...
式\eqref{chsm:eqn_riemann-N-tan}给出的定义本就是流形$N$中的黎曼曲率，
本来无需在左下角增加记号“$\top$”，
但是为了与式\eqref{chsm:eqn_riemann-N-nor}形成对比才附加此记号；
在其他章节中，当只有${}^{n}_{\top}R^d_{cab}$出现时，
我们将其记成${}^{n}R^d_{cab}$．
...
\end{equation}
交换\eqref{chsm:eqn_DxDyZ}中$X,Y$位置并相减，
...
%     - {\rm D}^\bot_{Y} (K^{a}_{bc} X^b Z^c) + S^a_{\eta b} Y^b\\
    ...
    - K^a_{ef} Z^f {\rm D}_{X} Y^e   \notag \\
    +& K^a_{ef} Z^f {\rm D}_{Y} X^e + {\rm D}^\bot_{X} \zeta^{a} -{\rm D}^\bot_{Y} \eta^a
...
由于是在法空间作内积，故需用度规${g}$，而不能用诱导度规$h$．

以上考虑了式\eqref{chsm:eqn_gwcr}的切向部分；
下面取式\eqref{chsm:eqn_gwcr}等号两侧的法向部分，
...
参考黎曼曲率\eqref{chsm:eqn_riemann-MN-TN}；
...
\begin{align}
    {}^{m}{R}^a_{bcd} X^c Y^d \xi^b \theta_a =& {}^{n}_{\bot}{R}^a_{bcd} X^c Y^d \xi^b \theta_a
        ...
        \label{chsm:eqn_Ricci-a} \\    {\Leftrightarrow \qquad  }
...
\begin{equation}    \label{chsm:eqn_NM-sect-curv}
    ...
    -(K^f_{bc} v^b v^c) (K^a_{ed} u^e u^d)     \bigr)}
...
大写拉丁字母$n+1 \leqslant A,B,\cdots \leqslant m$；注意$m>n$．
...
我们先化简联络型式场$(\omega _{\cdot i}^A)_a$．
因${\rm d}\circ \imath^* = \imath^* \circ {\rm d}$（即拉回映射与外微分可交换），
由$M$上Cartan第一结构方程得
\begin{equation}
...
\begin{subequations}\label{chsm:eqn_tmpdn0}
\begin{align}
...
流形$M$上的联络型式为
\begin{subequations}\label{chsm:eqn_tmp0121}
\begin{align}
...
\begin{subequations}\label{chsm:eqn_Dbotij}
\begin{align}
...
如果在\eqref{chsm:eqn_omega-Kija}式中没有用“$K$”而用其它字符，反而有违和感．
...
\begin{equation}\label{chsm:eqn_Codazzi-Frame}
    ...
    \end{aligned}
\end{equation}
由\S \ref{chccr:sec_connectionForm}可知上式是第二基本形式的协变导数之差，
...
\end{align}
计算中需注意度规的切空间与法空间分量是零，即$g_{B i}$等项恒为零． \qed
...
令式\eqref{chsm:eqn_DaY}中$Y^a=\left(\frac{\partial}{\partial s}\right)^a$，则有
...
因浸入映射在局部上就是包含映射，故可认为$V$中的第$1 \leqslant i \leqslant m$个坐标
...
在超曲面几节中，只考虑\CJKunderwave{正定}或\CJKunderwave{广义Lorentz度规}．
...
标架场$(e_\mu)^a (0\leqslant \mu \leqslant m)$便构成了$TM$中的局部基矢场．
...
以$\binom{1}{1}$型张量场$T^c_{\cdot d}\in \mathfrak{T}^p_q(M)$为例来说明$h_{ab}$还可以看成一种{\heiti 投影算符}：
...
仅以$\binom{1}{1}$型张量场$T^b_c \in \mathfrak{T}^p_q(\Sigma)$为例来说明．
...
将上式和式\eqref{chsm:eqn_Gauss-Hypersurface}比对后，可得
...
由式\eqref{chccr:eqn_LieD-tensor-Nabla}易得流形$M$度规场$g_{ab}$沿法矢量$n^a$的李导数
\begin{equation}
    \Lie_{n} g_{ab} =  g_{ac} \nabla_b n^c + g_{cb} \nabla_a n^c
      = \nabla_b n_a + \nabla_a n_b .
\end{equation}
利用$K_{ab}=K_{ba}$，由上式以及式\eqref{chsm:eqn_Kab-1}易得
...
结合式\eqref{chsm:eqn_Gauss-Hypersurface}，对比如下导数
\begin{align*}
...
设二维流形有局部坐标系$\{x,y\}$，其法矢为$\xi_a=\xi_1 ({\rm d}x)_a+\xi_2 ({\rm d}y)_a$．则有
\begin{align*}
...
\begin{equation*}
    \sum_{i=1}^{m}\sum_{\alpha=0}^{m} g_{\alpha i}  n^\alpha g^{i\beta} =0 {\quad \Rightarrow\quad }
...
可以验证超曲面$\Sigma$诱导体积元与诱导度规$h_{ab}$相容（留给读者当练习），
...
为了与\S\ref{chdf:sec_manifold-boundary}中边界的诱导定向相符合，
须令$n^0\geqslant 0$，即取式\eqref{chsm:eqn_n0i=g}中的正号
（$n^0 = + \sqrt{\epsilon g^{00}}$）．
在这样约定下，法矢量$n^a$是指向带边流形外部的．
...
设有$m+1$维、已定向、广义黎曼流形$(M,g)$，${\Omega}_{a_0 a_1 \cdots a_{m}}$是$M$上相容体积元；
$N$是$M$上带边嵌入子流形，边界是超曲面$\Sigma$．对于任意非零、
至少一阶可微的切矢量场$\xi^a\in TM$，有
\begin{equation}\label{chsm:eqn_Gauss-Divergence}
    \int_{{\rm i}(N)} (\nabla_a \xi^a) {\Omega}_{a_0 a_1 \cdots a_{m}}
...
\begin{proof}
    ...
    \begin{align*}
        & {\rm d}_{a_0}  (\xi^b \Omega_{b a_1 \cdots a_m}) = f\cdot \Omega_{a_0 \cdots a_m} ,
           \qquad \exists f \in C^\infty(M); \quad \Rightarrow \\
        & (m+1) \bigl(\nabla_{[a_0}\xi^b \bigr) \Omega_{|b| a_1 \cdots a_m]}   = f\cdot \Omega_{a_0 \cdots a_m} ,
           \qquad \text{两边同时缩并}\ \Omega^{a_0 \cdots a_m}; \quad \Rightarrow \\
        & (m+1) \bigl(\nabla_{[a_0}\xi^b \bigr) \Omega_{|b| a_1 \cdots a_m]}  \Omega^{a_0 a_1 \cdots a_m}
         = f\cdot \Omega_{a_0 a_1 \cdots a_m} \Omega^{a_0 a_1 \cdots a_m},
            \ \text{利用}\eqref{chrg:eqn_VE-contract-VE};\ \Rightarrow \\
        & (m+1)(-)^s m! \bigl(\nabla_{a_0}\xi^b \bigr) \delta^{a_0}_{b}=f\cdot (-)^s (m+1)!
    ...
    \end{equation}
    上式中最后一个积分号中的“$\xi^b \Omega_{b a_1 \cdots a_m}$”需限制在$\Sigma$上取值．
    定理证明过半，下面证明后半部分．
    ...
    \begin{equation*}
        ...
        \xi^a = K n^a {\quad \Rightarrow\quad  }  K= \epsilon \xi^a n_a .
    \end{equation*}
    带回便可证明定理．
\end{proof}
...
\end{equation*}
配上体积元，取积分，得（其中${\rm i}(N)$上积分体积元是${\Omega}_{a_0 \cdots a_{m} }$，省略未标出）：
\begin{equation}\label{chsm:eqn_Gauss-Divergence-Tab}
    \int_{{\rm i}(N)} \left(\nabla_a T^{ab}\right) \xi_b  
    =\epsilon \int_{\partial N} T^{ab}\xi_b n_a \, \hat{\Omega}_{a_1 \cdots a_{m} }
    - \frac{1}{2}\int_{{\rm i} (N)} T^{ab} \Lie_{\xi}g_{ab} .
\end{equation}
...
总这样声明多有不便，所以在式\eqref{chsm:eqn_curvature-Gauss-Frame-HS}中，
我们加上了投影算符以保证只取黎曼曲率${R}_{abcd}$的切分量；
约化后的第二基本形式$K_{ab}$本身就没有法分量，无需再次投影．
...
得超曲面$\Sigma$上的Ricci曲率，  %（计算中注意${n}^{c'} {n}^{d'} {R}_{ab'c'd'} ={R}_{ab'00}=0$）．
...
\end{align}
由此不难得到：伪球面上有$\eta_{ab}n^a n^b =1$；伪双曲空间上有$\eta_{ab}n^a n^b =-1$．
...
\begin{proof}
    ...
    超曲面$S^m_\nu=\{\boldsymbol{v} \ | \  \left<\boldsymbol{v} ,\boldsymbol{v}\right>_h = r^2 \}$上的充分必要条件是：$a^2+b^2=r^2$．
...
此例还说明了定理\ref{chrg:thm_isometry-connection-vector}只在{\kaishu 局部等距}的
情形下成立，在等距浸入情形下未必成立．
\qed
...
$x^0$由这些变量表示；
$\mathbb{R}^{m+1}_\nu$空间是Lorentz度规．
从式\eqref{chsm:eqn_pssphere-space}和\eqref{chsm:eqn_gijhab}可求得诱导度规
\begin{align}
...
则由式\eqref{chsm:eqn_SigmaM-sect-curv}可得$S^m_\nu(r)$截面曲率（$\epsilon=n_a n^a=1$）：
\begin{align*}   
    {}^{m} K([u\wedge v]) 
    =&\frac{1}{r^2}
...
令$S=(-r,0,\cdots,0)\in \mathbb{R}^{m+1}_1$．
命（其中$(x^0,x^1,\cdots,x^{m})\in \mathbb{R}^{m+1}_1$）
\begin{align}
    H^m_{+}(r) = & \bigl\{(x^0,\cdots,x^{m})\mid (x^1)^2+\cdots+(x^m)^2 - (x^{0})^2 = -r^2,
...
\begin{align*}
    ...
    =\frac{r^2 }{(r+x^{0})^2}\left(-r^2+(x^{0})^2\right) \\    \Rightarrow \quad 
...
\end{align}
上面第二式为双曲空间下叶的坐标（其中$\eta^i=\frac{r x^i}{r-x^{0}}$）．
...
根据式\eqref{chsm:eqn_gijhab}经过计算可得
\begin{align}
        h'_{ab}=& \sum_{i,j=1}^{m}\left(\sum_{\alpha,\beta=1}^{m}\frac{\partial x^\alpha}{\partial x^i}
        ...
        =& \sum_{i,j=1}^{m}\left(\delta_{ij}-\frac{x^i x^j}{r^2+\sum_{k=1}^{m}(x^k)^2}\right)
        ({\rm d} x^i)_a ({\rm d} x^j)_b \notag \\
        \xlongequal{\ref{chdm:eqn_hybxi2x}}&
        \frac{4r^4 \sum\nolimits_{i=1}^{m} ({\rm d}\xi^i)_a ({\rm d}\xi^i)_b} 
        {(r^2-\sum\nolimits_{k=1}^{m} (\xi^k)^2)^2}.       \label{chsm:eqn_gphihabb}
\end{align}
...
\begin{example}
    ...
    再计算Lorentz度规下的数值．推前$v^a$有
    \begin{equation*}
    	\varphi_*(v^a)= \frac{4r^3\xi v_\xi}{(r^2-\xi^2)^2}
    	\left(\frac{\partial}{\partial x^0}\right)^c
    	+ \frac{2r^2(r^2+\xi^2) v_\xi}{(r^2-\xi^2)^2}
    	\left(\frac{\partial}{\partial x^1}\right)^c.
    \end{equation*}    
    再来计算矢量长度，$g_{ab}v^a v^b= \frac{4r^4 v^2_\xi}{(r^2-\xi^2)^2}$；
    两者相等．   \qed
\end{example}
...
从纯粹数学角度来看任意四维闵氏流形$(M,g)$，未必存在这样的分解，
或者说需要满足一定条件才存在这样的类时矢量场，
然后依据这个类时场将空间$M$划分成一族类空超曲面$\Sigma_t$；
寻找数学上的这些存在性条件（对物理学工作者来说）太过艰深．
从物理学角度看，这样的分解一定存在，
因为我们就生活在这样的一个四维时空中，
这个时空然后被$1+3$分解成时间和空间．
...
\end{equation}
由式\eqref{chrg:eqn_Riemannian04-component}、\eqref{chsm:eqn_curvature-Gauss-Frame-HS}
和\eqref{chsm:eqn_Codazzi-Frame-HS}可得同步参考系中的黎曼曲率：
\begin{subequations}
\begin{align}
	{}^{4} {R}_{0 i 0 j}  %=& \frac{1}{2}\left(
%	\frac{\partial^2 g_{0 j}} {\partial x^i \partial x^0}
%	- \frac{\partial^2 g_{i j}} {\partial x^0 \partial x^0}
%	- \frac{\partial^2 g_{0 0}} {\partial x^i \partial x^j}
%	+ \frac{\partial^2 g_{i 0}} {\partial x^0 \partial x^j} \right) 
%	+ \Gamma_{i 0}^\rho \Gamma_{\rho 0 j} 
%	- \Gamma_{i j}^\rho  \Gamma_{\rho 0 0} \\
%	=& -\frac{1}{2} \ddot{h}_{ij}
%	+ \Gamma_{i 0}^0 \Gamma_{0 0 j} + \Gamma_{i 0}^l \Gamma_{l 0 j} 
%	- \Gamma_{i j}^0  \Gamma_{0 0 0} - \Gamma_{i j}^l  \Gamma_{l 0 0} \\
%	=& -\frac{1}{2} \ddot{h}_{ij} + h_{lk} \Gamma_{i 0}^l \Gamma^k_{0 j} 
	=& -\frac{1}{2} \ddot{h}_{ij} + \frac{1}{4} \dot{h}^l_i \dot{h}_{lj} . \\
	{}^{4}{R}_{0jkl} =& +\frac{1}{2} \dot{h}_{jl;k} - \frac{1}{2} \dot{h}_{jk;l} . \\
	{}^{4}{R}_{ijkl }  =&\ {}^{3}{R}_{ijkl} + \frac{1}{4} \dot{h}_{ik} \dot{h}_{jl} 
	- \frac{1}{4} \dot{h}_{il} \dot{h}_{jk} . 
\end{align}
\end{subequations}
由式\eqref{chccr:eqn_Ricci-component}可得同步参考系中的Ricci曲率：
\begin{subequations}
\begin{align}
...
%	+ \Gamma_{0i}^{k} \Gamma_{kj}^{j} - \Gamma_{0j}^{k} \Gamma_{ki}^{j} \notag\\
	...
	+ \frac{1}{4} \dot{h}_{l}^l  \dot{h}_{ij}  . \label{chsm:eqn_SF-Rij}
\end{align}
\end{subequations}
其中式\eqref{chsm:eqn_SF-Rij}采用$(\partial_i)^a$、$(\partial_j)^b$缩并式\eqref{chsm:eqn_SF-dtKab}得到．



%\begin{align*}
%R_{\nu \mu \alpha \beta} = \frac{1}{2}\left(
%\frac{\partial^2 g_{\nu \beta}} {\partial x^\mu \partial x^\alpha}
%- \frac{\partial^2 g_{\mu \beta}} {\partial x^\nu \partial x^\alpha}
%- \frac{\partial^2 g_{\nu \alpha}} {\partial x^\mu \partial x^\beta}
%+ \frac{\partial^2 g_{\mu \alpha}} {\partial x^\nu \partial x^\beta} \right) 
%+ \Gamma_{\mu \alpha}^\rho \Gamma_{\rho \nu \beta} 
%- \Gamma_{\mu \beta}^\rho  \Gamma_{\rho \nu \alpha}
%\end{align*}
