...
在\S \ref{chmla:sec_exterior-algebra}中，我们已经引入了线性空间$V$中外代数的概念；
那里只是针对一个特定空间$V$（或其对偶空间$V^*$），而本节我们将这些概念推广到\CJKunderwave{场}中．
...
\end{subequations}
上面公式中，
$\omega_{a_{1}\cdots a_{r}}$和$\omega_{\mu_{1}\cdots \mu_{r}}$的下标是全反对称的，
即交换其中任意两个指标都会产生一个负号．
...
可将\S \ref{chdm:sec_tensor-fields}中定义的张量间的加法和数量乘法，
以及$C^\infty(M)$-乘法直接运用到微分型式场（微分型式场只是张量场的特例，自然可直接运用这些运算）．
...
\begin{theorem}\label{chdf:thm_dfs-fsd}
    ...
    拉回映射$\phi^*: A^r(N) \to A^r(M)$（ $0\leqslant r \leqslant \min(m,n)$）是线性的，
    且与外积可交换，即
    \begin{align*}
        \phi^{*} (\alpha \wedge \beta ) = (\phi^{*}\alpha) \wedge \phi^{*}\beta,
        \quad \forall \alpha \in A^p(N), \ \beta \in A^q(N), \quad 0\leqslant p+q \leqslant \min(m,n) .
    \end{align*}
\end{theorem}
\begin{proof}
    ...
    \end{align*}
    证明过程需参阅式\eqref{chmla:eqn_ep-base}、式\eqref{chmla:eqn_alpha-beta}、
    式\eqref{chdm:eqn_pull-bases}和式\eqref{chdf:eqn_omega-expand-on-bases}．
\end{proof}
证明过程中，我们还得到了一个常用公式：
...
\begin{align*}
	&(e^{i_1})_{a_1} \wedge \cdots \wedge (e^{i_p})_{a_p} (e_\kappa)^{a_p} = 
...
\end{align*}
上式对$s$求和过程中至多有一项非零．给出两个具体示例：
\begin{align}
	&({\rm d}x^{0})_{a_{0}} \wedge \cdots \wedge ({\rm d}x^{3})_{a_{3}} (\partial_\kappa)^{a_3}
	=(-)^{3+\kappa} ({\rm d}x^{0})_{a_{0}} \wedge\cdots \wedge \widehat{{\rm d}{x}^{\kappa}}
	\wedge \cdots \wedge ({\rm d}x^{3})_{a_{2}} . \\
	& (\partial_\kappa)^{a_0} ({\rm d}x^{0})_{a_{0}} \wedge \dots \wedge ({\rm d}x^{3})_{a_{3}} 
	=(-)^{\kappa} ({\rm d}x^{0})_{a_{1}} \wedge\cdots \wedge \widehat{{\rm d}{x}^{\kappa}}
	\wedge \cdots \wedge ({\rm d}x^{3})_{a_{3}} . 
\end{align}
其中$0\leqslant \kappa \leqslant 3$．
在广义相对论中会经常用到上两式中的缩并．
\qed
...
\begin{example}\label{chdf:exam_eiapkl}
	与例\ref{chdf:exam_eiapk}相仿，计算缩并式
	$(e^{0})_{a} \wedge \cdots \wedge (e^{3})_{d} \left[(e_\tau)^{c}\wedge(e_\kappa)^{d}\right]$．
\end{example}
直接展开，有
\begin{align*}
	&(e^{0})_{a} \wedge (e^{1})_{b} \wedge (e^{2})_{c} \wedge (e^{3})_{d} 
	\left[(e_\tau)^{c}\wedge(e_\kappa)^{d}\right] \\
	=& \delta^{0123}_{\alpha\beta\mu\nu} 
	(e^{\alpha})_{a} \otimes (e^{\beta})_{b} \otimes (e^{\mu})_{c} \otimes (e^{\nu})_{d} 
	\left[(e_\tau)^{c}\otimes (e_\kappa)^{d} -(e_\kappa)^{c}\otimes (e_\tau)^{d}\right] \\
	=& \delta^{0123}_{\alpha\beta\mu\nu} (e^{\alpha})_{a} \otimes (e^{\beta})_{b}  
	\left[\delta_{\tau}^{\mu} \delta_{\kappa}^{\nu}	- \delta_{\kappa}^{\mu}\delta_{\tau}^{\nu} \right]
	=\delta^{0123}_{\alpha\beta\tau\kappa}(e^{\alpha})_{a} \otimes (e^{\beta})_{b} 
	-\delta^{0123}_{\alpha\beta\kappa\tau}(e^{\alpha})_{a} \otimes (e^{\beta})_{b} \\
	=&\delta^{0123}_{\alpha\beta\tau\kappa}(e^{\alpha})_{a} \otimes (e^{\beta})_{b} 
	-\delta^{0123}_{\alpha\beta\tau\kappa}(e^{\beta})_{a} \otimes (e^{\alpha})_{b} .
\end{align*}
故，有
\begin{align}
	(e^{0})_{a} \wedge (e^{1})_{b} \wedge (e^{2})_{c} \wedge (e^{3})_{d} 
	\left[(e_\tau)^{c}\wedge(e_\kappa)^{d}\right] 
	=& \delta^{0123}_{\alpha\beta\tau\kappa} (e^{\alpha})_{a} \wedge (e^{\beta})_{b}  . \\
	\left[(e_\tau)^{a}\wedge(e_\kappa)^{b}\right] 
	(e^{0})_{a} \wedge (e^{1})_{b} \wedge (e^{2})_{c} \wedge (e^{3})_{d} 
	=& \delta^{0123}_{\alpha\beta\tau\kappa} (e^{\alpha})_{c} \wedge (e^{\beta})_{d}  .
\end{align}
其中$0\leqslant \tau,\kappa \leqslant 3$．
在广义相对论中会经常用到上两式中的缩并．
\qed

%$\delta_{j_1 \cdots j_q j_{q+1}\cdots j_m}^{i_1 \cdots i_q i_{q+1}\cdots i_m}
%\cdot \delta_{k_{q+1} \cdots k_m}^{j_{q+1} \cdots j_m}
%=(m-q)! \ \delta_{j_1 \cdots j_q k_{q+1}\cdots k_m}^{i_1 \cdots i_q i_{q+1}\cdots i_m} $
	


%\begin{align*}
%	&(e^{0})_{a_0} \wedge (e^{1})_{a_1} \wedge (e^{2})_{a_2} \wedge (e^{3})_{a_3} 
%	\left[(e_\tau)^{a_{2}}\wedge(e_\kappa)^{a_3}\right] \\
%	=& \left[ (e^{0})_{a_0} \otimes (e^{1})_{a_1} \otimes (e^{2})_{a_2} \otimes (e^{3})_{a_3} 
%	+\cdots\cdots	\right]
%	\left[(e_\tau)^{a_2}\otimes (e_\kappa)^{a_3}
%	-(e_\kappa)^{a_2}\otimes (e_\tau)^{a_3}\right] \\
%	%	=&+(e^{0})_{a_0} \otimes (e^{1})_{a_1} \delta_\tau^2 \delta_\kappa^3 
%	%	-(e^{0})_{a_0} \otimes (e^{1})_{a_1} \delta_\kappa^2 \delta_\tau^3 
%	%	-(e^{0})_{a_0} \otimes (e^{1})_{a_1} \delta_\tau^3   \delta_\kappa^2
%	%	+(e^{0})_{a_0} \otimes (e^{1})_{a_1} \delta_\kappa^3 \delta_\tau^2 	\\
%	%	&+(e^{0})_{a_0}\otimes (e^{2})_{a_1} \delta_\tau^3   \delta_\kappa^1 
%	%	-(e^{0})_{a_0} \otimes (e^{2})_{a_1} \delta_\kappa^3 \delta_\tau^1
%	%	-(e^{0})_{a_0} \otimes (e^{2})_{a_1} \delta_\tau^1   \delta_\kappa^3
%	%	+(e^{0})_{a_0} \otimes (e^{2})_{a_1} \delta_\kappa^1 \delta_\tau^3 	\\
%	%	&+(e^{0})_{a_0}\otimes (e^{3})_{a_1} \delta_\tau^1   \delta_\kappa^2
%	%	-(e^{0})_{a_0} \otimes (e^{3})_{a_1} \delta_\kappa^1 \delta_\tau^2
%	%	-(e^{0})_{a_0} \otimes (e^{3})_{a_1} \delta_\tau^2   \delta_\kappa^1
%	%	+(e^{0})_{a_0} \otimes (e^{3})_{a_1} \delta_\kappa^2 \delta_\tau^1 	\\
%	%	&+(e^{1})_{a_0}\otimes (e^{2})_{a_1} \delta_\tau^0   \delta_\kappa^3
%	%	-(e^{1})_{a_0} \otimes (e^{2})_{a_1} \delta_\kappa^0 \delta_\tau^3
%	%	-(e^{1})_{a_0} \otimes (e^{2})_{a_1} \delta_\tau^3   \delta_\kappa^0
%	%	+(e^{1})_{a_0} \otimes (e^{2})_{a_1} \delta_\kappa^3 \delta_\tau^0 	\\
%	%	&+(e^{1})_{a_0}\otimes (e^{3})_{a_1} \delta_\tau^2   \delta_\kappa^0
%	%	-(e^{1})_{a_0} \otimes (e^{3})_{a_1} \delta_\kappa^2 \delta_\tau^0
%	%	-(e^{1})_{a_0} \otimes (e^{3})_{a_1} \delta_\tau^0   \delta_\kappa^2
%	%	+(e^{1})_{a_0} \otimes (e^{3})_{a_1} \delta_\kappa^0 \delta_\tau^2 	\\
%	%	&+(e^{1})_{a_0}\otimes (e^{0})_{a_1} \delta_\tau^3   \delta_\kappa^2
%	%	-(e^{1})_{a_0} \otimes (e^{0})_{a_1} \delta_\kappa^3 \delta_\tau^2
%	%	-(e^{1})_{a_0} \otimes (e^{0})_{a_1} \delta_\tau^2   \delta_\kappa^3
%	%	+(e^{1})_{a_0} \otimes (e^{0})_{a_1} \delta_\kappa^2 \delta_\tau^3 	\\
%	%	&+(e^{2})_{a_0}\otimes (e^{0})_{a_1} \delta_\tau^1   \delta_\kappa^3
%	%	-(e^{2})_{a_0} \otimes (e^{0})_{a_1} \delta_\kappa^1 \delta_\tau^3
%	%	-(e^{2})_{a_0} \otimes (e^{0})_{a_1} \delta_\tau^3   \delta_\kappa^1
%	%	+(e^{2})_{a_0} \otimes (e^{0})_{a_1} \delta_\kappa^3 \delta_\tau^0 	\\
%	%	&+(e^{2})_{a_0}\otimes (e^{1})_{a_1} \delta_\tau^3   \delta_\kappa^0
%	%	-(e^{2})_{a_0} \otimes (e^{1})_{a_1} \delta_\kappa^3 \delta_\tau^0
%	%	-(e^{2})_{a_0} \otimes (e^{1})_{a_1} \delta_\tau^0   \delta_\kappa^3
%	%	+(e^{2})_{a_0} \otimes (e^{1})_{a_1} \delta_\kappa^0 \delta_\tau^3 	\\
%	%	&+(e^{2})_{a_0}\otimes (e^{3})_{a_1} \delta_\tau^0   \delta_\kappa^1
%	%	-(e^{2})_{a_0} \otimes (e^{3})_{a_1} \delta_\kappa^0 \delta_\tau^1
%	%	-(e^{2})_{a_0} \otimes (e^{3})_{a_1} \delta_\tau^1   \delta_\kappa^0
%	%	+(e^{2})_{a_0} \otimes (e^{3})_{a_1} \delta_\kappa^1 \delta_\tau^0 	\\
%	%	&+(e^{3})_{a_0}\otimes (e^{0})_{a_1} \delta_\tau^2   \delta_\kappa^1
%	%	-(e^{3})_{a_0} \otimes (e^{0})_{a_1} \delta_\kappa^2 \delta_\tau^1
%	%	-(e^{3})_{a_0} \otimes (e^{0})_{a_1} \delta_\tau^1   \delta_\kappa^2
%	%	+(e^{3})_{a_0} \otimes (e^{0})_{a_1} \delta_\kappa^1 \delta_\tau^2 	\\
%	%	&+(e^{3})_{a_0}\otimes (e^{1})_{a_1} \delta_\tau^0   \delta_\kappa^2
%	%	-(e^{3})_{a_0} \otimes (e^{1})_{a_1} \delta_\kappa^0 \delta_\tau^2
%	%	-(e^{3})_{a_0} \otimes (e^{1})_{a_1} \delta_\tau^2   \delta_\kappa^0
%	%	+(e^{3})_{a_0} \otimes (e^{1})_{a_1} \delta_\kappa^2 \delta_\tau^0 	\\
%	%	&+(e^{3})_{a_0}\otimes (e^{2})_{a_1} \delta_\tau^1   \delta_\kappa^0
%	%	-(e^{3})_{a_0} \otimes (e^{2})_{a_1} \delta_\kappa^1 \delta_\tau^0
%	%	-(e^{3})_{a_0} \otimes (e^{2})_{a_1} \delta_\tau^0   \delta_\kappa^1
%	%	+(e^{3})_{a_0} \otimes (e^{2})_{a_1} \delta_\kappa^0 \delta_\tau^1 	\\
%	=&+(e^{0})_{a_0} \wedge (e^{1})_{a_1} \delta_{\tau\kappa}^{23}
%	+(e^{0})_{a_0}   \wedge (e^{2})_{a_1} \delta_{\tau\kappa}^{31}
%	+(e^{0})_{a_0}   \wedge (e^{3})_{a_1} \delta_{\tau\kappa}^{12} \\
%	&+(e^{1})_{a_0}  \wedge (e^{2})_{a_1} \delta_{\tau\kappa}^{03}
%	+(e^{1})_{a_0}   \wedge (e^{3})_{a_1} \delta_{\tau\kappa}^{20} 
%	+(e^{1})_{a_0}   \wedge (e^{0})_{a_1} \delta_{\tau\kappa}^{32} \\
%	&+(e^{2})_{a_0}   \wedge (e^{0})_{a_1} \delta_{\tau\kappa}^{13} 
%	+(e^{2})_{a_0}  \wedge (e^{1})_{a_1} \delta_{\tau\kappa}^{30}
%	+(e^{2})_{a_0}   \wedge (e^{3})_{a_1} \delta_{\tau\kappa}^{01} \\
%	&+(e^{3})_{a_0}   \wedge (e^{0})_{a_1} \delta_{\tau\kappa}^{21} 
%	+(e^{3})_{a_0}  \wedge (e^{1})_{a_1} \delta_{\tau\kappa}^{02}
%	+(e^{3})_{a_0}   \wedge (e^{2})_{a_1} \delta_{\tau\kappa}^{10}.
%\end{align*}
%故，有（下面两个缩并是相等的，证明留给读者当练习）：
%\begin{align}
%	&\frac{1}{2} (e^{0})_{a_0} \wedge (e^{1})_{a_1} \wedge (e^{2})_{a_2} \wedge (e^{3})_{a_3} 
%	\left[(e_\tau)^{a_{2}}\wedge(e_\kappa)^{a_3}\right] \notag \\
%	=&\frac{1}{2} \left[(e_\tau)^{a_2}\wedge (e_\kappa)^{a_3}\right]
%	(e^{0})_{a_2} \wedge (e^{1})_{a_3} \wedge (e^{2})_{a_0} \wedge (e^{3})_{a_1}  \notag \\
%	=&+(e^{0})_{a_0} \wedge (e^{1})_{a_1} \delta_{\tau\kappa}^{23}
%	+(e^{0})_{a_0}   \wedge (e^{2})_{a_1} \delta_{\tau\kappa}^{31}
%	+(e^{0})_{a_0}   \wedge (e^{3})_{a_1} \delta_{\tau\kappa}^{12} \notag \\
%	&+(e^{1})_{a_0}  \wedge (e^{2})_{a_1} \delta_{\tau\kappa}^{03}
...
\begin{example}\label{chdf:exam_eiapklp}
	计算缩并式
	$(e^{0})_{a} \wedge \cdots \wedge (e^{3})_{d} \left[(e_\tau)^{b}
	\wedge(e_\kappa)^{c}\wedge(e_\sigma)^{d}\right]$．
\end{example}
直接展开，有
\begin{align*}
	&(e^{0})_{a} \wedge (e^{1})_{b} \wedge (e^{2})_{c} \wedge (e^{3})_{d} 
	\left[(e_\tau)^{b}\wedge(e_\kappa)^{c}\wedge(e_\sigma)^{d}\right] \\
	=& \delta^{0123}_{\alpha\beta\mu\nu} 
	(e^{\alpha})_{a} \otimes (e^{\beta})_{b} \otimes (e^{\mu})_{c} \otimes (e^{\nu})_{d} 
	\left[\delta^{\zeta\xi\eta}_{\tau\kappa\sigma}  
	(e_\zeta)^{b}\otimes(e_\xi)^{c}\otimes(e_\eta)^{d} \right] \\
	=&\delta^{0123}_{\alpha\beta\mu\nu} \delta^{\zeta\xi\eta}_{\tau\kappa\sigma}
	(e^{\alpha})_{a}  \delta^\beta_\zeta \delta^\mu_\xi \delta^\nu_\eta 
	=\delta^{0123}_{\alpha\beta\mu\nu} \delta^{\beta\mu\nu}_{\tau\kappa\sigma}(e^{\alpha})_{a} .
\end{align*}
利用式\eqref{chmla:eqn_gkd-210}（$\delta_{j_1 \cdots j_q j_{q+1}\cdots j_m}^{i_1 \cdots i_q i_{q+1}\cdots i_m}
\cdot \delta_{k_{q+1} \cdots k_m}^{j_{q+1} \cdots j_m}
=(m-q)! \ \delta_{j_1 \cdots j_q k_{q+1}\cdots k_m}^{i_1 \cdots i_q i_{q+1}\cdots i_m} $），有
\begin{align}
	(e^{0})_{a} \wedge (e^{1})_{b} \wedge (e^{2})_{c} \wedge (e^{3})_{d} 
	\left[(e_\tau)^{b}\wedge(e_\kappa)^{c}\wedge(e_\sigma)^{d}\right] 
	=& 6 \, \delta^{0123}_{\alpha\tau\kappa\sigma} (e^{\alpha})_{a}  . 
\end{align}
其中$0\leqslant \tau,\kappa,\sigma \leqslant 3$．
在广义相对论中会经常用到上式中的缩并．
\qed
...
\begin{theorem}\label{chdf:thm_exterior-differential}
    ...
    {\bfseries (1)} $\mathbb{R}$-线性：$\forall \alpha, \beta \in A^r(M), \ \forall \lambda \in \mathbb{R}$，有
      ${\rm d}(\alpha+ \lambda \cdot \beta) = {\rm d} \alpha+ \lambda \cdot {\rm d} \beta$；

    {\bfseries (2)} 准Leibniz律：$\forall \alpha \in A^{r}(M)$，有
...
\begin{lemma}\label{chdm:thm_d-exiterior-local}
    假设满足定理\ref{chdf:thm_exterior-differential}中四个条件的外微分${\rm d}$存在；
    如果$\alpha, \beta \in A(M)$，
    并且在开子集$U\subset M$上有$\alpha|_U = \beta|_U$，
    则必然有${\rm d}\alpha|_U = {\rm d}\beta|_U$．
\end{lemma}
\begin{proof}
...
%    利用条件(1)（即线性），令$\lambda =1,\ \alpha=0,\ \beta=0$，则有
...
%    这说明映射${\rm d}$将零微分型式场映射为零标量场．
    ...
    将外微分作用在此式上，并利用准Leibniz律（条件(2)）和条件(3)，
    ...
    又因$p$点取值的任意性，故必然可得$({\rm d}\omega)|_U=0$．
\end{proof}
...
因此，如果${\rm d}|_M$是流形$M$上的外微分算子，
那么通过式\eqref{chdf:eqn_daflg}可以定义出局部开集$U$中的外微分算子${\rm d}|_U$，
而且${\rm d}|_U$同样具有定理\ref{chdf:thm_exterior-differential}中所陈述的四条性质．
...
有${\rm d}^2f=f''(x) {\rm d}x^2$；
可见普通微分算符不满足条件(4)（但满足条件(3)）不能当成外微分算符定义．
其实略加修改，便可满足条件(4)；考虑多元标量函数场$f$，
把外积“$\wedge$”引入到${\rm d}x\ {\rm d}x$之间：
\begin{align}
...
对于标量函数场，这样定义的算符满足条件(4)．
我们将上述定义推广到一般情形；设流形$M$有局部坐标系$(U;x^i)$，
...
为验证此条件，我们计算一般外型式场的两次外微分（使公式简洁，下面诸式皆省略下标“$|_U$”）．
\begin{align*}
...
这便验证了定义\eqref{chdf:eqn_exterior-differential}符合条件(4)．
...
\begin{align*}
   ...
   =& \frac{1}{r! s!}  {\rm d}_{c}(\alpha_{\mu_{1}\cdots \mu_{r}}) \beta_{\sigma_1\cdots \sigma_q}
    ...
    =& \frac{1}{r!}  {\rm d}_{c}(\alpha_{\mu_{1}\cdots \mu_{r}})
      ...
      ({\rm d}x^{\sigma_1})_{b_{1}} \wedge\cdots\wedge ({\rm d}x^{\sigma_s})_{b_{s}}.
\end{align*}
上式最后一个等号用到了外积的反对称性，产生了$r$个负号；最终，有
...
虽然解决了外微分算子的局部、整体存在性问题（并且局部、整体定义是相容的），
但仍需证明式\eqref{chdf:eqn_exterior-differential}中的定义是唯一的，
以及与局部坐标的选取无关．

先证明唯一性．
假设另有一个算符$\bar{\rm d}$同样满足定理\ref{chdf:thm_exterior-differential}中的四个条件；
需要注意，标量函数场$f$的普通微分是唯一确定的（这是数学分析中的结论），
由条件(3)可知$\bar{\rm d} f = {\rm d}f$，也就是两个算符作用在标量场上是相等的．
...
设有开子集$(W;y^i)$，且$U\cap W \neq \varnothing$，
那么微分型式场$\omega$可有两组相等的表达式：
\begin{align}
...
至此，定理\ref{chdf:thm_exterior-differential}证毕． 

\begin{remark}
    此定理表明，在光滑流形上，通过外微分运算能够把一个光滑{\kaishu 全反对称协变}张量场变成
    {\kaishu 高一阶}的光滑全反对称协变张量场．
...
在\S \ref{chdm:sec_One-Parameter-Transformations-Groups}中，
已经指出流形上切矢量场与其积分曲线（单参数可微变换群）有一一对应关系，
这其实是在说单个切矢量场是否可积；
本节将讲述多个切矢量场是否可积的问题，由常微分方程组过渡到偏微分方程组．
省略的定理证明过程可参考文献\parencite[\S 1.4]{cc2001-zh}或\parencite[\S 3.4]{chenwh2001}或其它类似书籍． 
需要事先声明，本节所叙述的定理只在局部成立；
关于Frobenius定理的大范围描述可参阅\parencite[\S 6.6-6.7]{spivak-dif-1}．
...
其中$U$是$\mathbb{R}^m$中的开凸子集，$V$是$\mathbb{R}^n$中的开凸子集；
并且这两个子集的边界（$\partial U$和$\partial  V$）是$C^{r-1}$阶连续可微的
（除了有限个测度为零的不可微子区域）．
...
\begin{theorem}\label{chdf:thm_1d-exist}
    ...
    反之，若假设方程组\eqref{chdf:eqn_1d-eqs}解存在，
    则条件\eqref{chdf:eqn_1d-exist}也是其必要条件（即推论）之一．
\end{theorem}
...
\begin{example}\label{chdf:exm_Fij2d}
    ...
    一个$\partial_x g -\partial_y f = g \partial_u f-f \partial_u g $，
    其它全部为$0=0$的恒等式；如果假设$f$、$g$与$u$无关，
...
定理\ref{chdf:thm_1d-exist}是Frobenius定理的古典形式；
下面我们用切矢量场以及微分型式场方式再次叙述此定理，为此需要做些准备．
...
流形上每点切空间都与流形同维数，
分布的含义便是先在$T_pU$找到$h$个$\mathbb{R}$-线性无关的切矢量$X^a_1(p),\cdots,X^a_h(p)$；
然后让点$p$遍历$U$，
这样就有$h$个线性无关的切矢量\CJKunderwave{场}$X^a_1,\cdots,X^a_h\ \in \mathfrak{X}(U)$；
...
若光滑流形$M$上存在$h$个处处$\mathbb{R}$-线性无关的光滑切矢量场，则在$U$上就存在一个$h$维光滑分布．
反过来，$U$上存在一个$h$维光滑分布，那么在$M$上未必存在$h$个处处线性无关的光滑切矢量场．
典型的例子便是二维球面$S^2$上的光滑切矢量场必有奇点，
因此$S^2$整体上不可能存在两个处处线性无关的非零切矢量场；
然而$S^2$的局部上的切矢量场显然是个2维分布．
虽然在整个流形（或者称之为大范围情形）两者并不等价，但在局部开子集$U$上，两者是等价的，
即一个$h$维光滑分布等价于存在$h$个处处线性无关的非零光滑切矢量场．

\index[physwords]{积分流形}

\begin{definition}\label{chdf:def_solution}
    设$L^h$是定义在开子集$U\subset M$上的光滑分布，如果有包含映射$\imath:N\to U$对每一
    点$p \in N$有$\imath_{*}(T_p N)\subset L^h\bigl(\imath(p)\bigr)$，
    则称$(\imath,N)$是光滑分布$L^h$的一个{\heiti 积分流形}．
    若$\forall q\in U$，都有分布$L^h$的一个$h$维积分流形经过它，
    则称分布$L^h$是{\heiti 完全可积}的．
\end{definition}
...
切矢量场的积分曲线正是它作为分布时的一维积分流形．
定理\ref{chdm:thm_1PDG-ppy}阐述了任意切矢量场（在坐标变换后）都可以表示
成$X^a |_U = (\frac{\partial }{\partial x^1} )^a$，
即其积分曲线是$x^1$-参数曲线，在此条曲线上只有自变量$x^1$是变化的，
其它自变量$x^i(2\leqslant i)$都是常数．
一个自然的问题是定理\ref{chdm:thm_1PDG-ppy}能否推广到高维，
即是否存在局部坐标系使得$L^h={\rm Span}_{\infty}\{(\frac{\partial }{\partial x^1} )^a,
...
\begin{definition}\label{chdf:def_frobenius-conditions}
    ...
    其中$c_{\alpha\beta}^{\gamma} \in C^\infty(V)$、$1\leqslant \alpha,\beta,\gamma \leqslant h$，
...
定理\ref{chdf:thm_frobenius}告诉我们，开子集$V$中的坐标面
\begin{align*}
	\{(x^1,\cdots,x^h,\  x^{h+1},\cdots, x^m) \mid x^{h+1}=const, \cdots, x^{m}=const\}
\end{align*}
是分布$L^h$的$h$维积分流形．此定理的逆也成立，即
...
\S\ref{chdf:sec_frobenius}中Frobenius定理\ref{chdf:thm_1d-exist}是它的古典描述方式，
本节用外型式场方式再次表述此定理．同\S\ref{chdf:sec_frobenius}表述类似，
...
这里的$(\omega^\alpha)_a = 0$不是指一次外微分式为恒零元；
恒零元是指$\forall v^a\in \mathfrak{X}(U)$有$(\omega^\alpha)_a v^a =\left<\omega^\alpha, v\right>=0$．
...
在光滑流形$M$上开子集$U$的一次微分式\eqref{chdf:eqn_Pfaff}在各点的$\mathbb{R}$-线性无关个数未必相同，
这给研究带来诸多不便．下面，
我们只研究$U$上各点线性无关的一次微分式（$(\omega^\alpha)_a$）个数均为相同常数的情形，
此常数称为Pfaff方程组的{\heiti 秩}．
...
假设点$p\in V\subset U$邻域有局部坐标系$(V;x^i)$（进一步缩小开集$U$的主要目的是：
缩小后的$p$点开邻域$V$可用一个坐标域覆盖，同时紧致集$\overline{V}\subset U$），
$h(=m-r)$维光滑分布$L^h$可用$V$上$\mathbb{R}$-线性无关的光滑切矢量场来张成，即
\begin{equation}
...
\end{equation}
因分布$L^h$要有$h$个$\mathbb{R}$-线性无关的切矢量场，故不妨假设展开系数$\det(v_\lambda^\mu)\neq 0$，
...
$1\leqslant \alpha \leqslant r$；
这样切标架场$\{ (u_{1})^a,\cdots, (u_m)^a \}$便构成了$V$上的切基矢场．
容易验证与$\{u^a\}$对偶的余切标架场是
\begin{equation}
    (\theta^\alpha)_a =  ({\rm d}x^\alpha)_a + \sum_{\mu=r+1}^{m} l^\alpha_\mu ({\rm d}x^\mu)_a  ,
    \quad   (\theta^\lambda)_a = ({\rm d}x^\lambda)_a.  
\end{equation}
...
\begin{align*}
    ...
    \qquad \text{其中} \  c^\gamma_\nu \equiv \sum_{\alpha=1}^{r} b^\gamma_\alpha\omega^\alpha_\nu .
\end{align*}
由于乘的是可逆矩阵，所以$(\varphi^\gamma)_a$和$(\omega^\gamma)_a$是等价的．
$(\varphi^\gamma)_a (1 \leqslant \gamma \leqslant r)$在$V$上是处处线性无关的，
我们可以补上$h=(m-r)$个余切矢量场$(\varphi^\lambda)_a=({\rm d}x^\lambda)_a (r< \lambda \leqslant m)$，
...
按照\S \ref{chdf:sec_frobenius}中论述，
假设光滑分布$L^h|_V$的积分流形是$(\imath,N)$（见定义\ref{chdf:def_solution}）；
其中包含映射$\imath$的定义是$\imath:N\to V\subset U \subset M$．
此时虽然$TN$中的切矢量场与$L^h|_V$是同构的，
...
从上式可以看到：命题\ref{chdf:thm_dwwd}中描述的、
与Pfaff方程组\eqref{chdf:eqn_Pfaff}伴随的光滑分布$L^h$的积分流形$(\imath,N)$就满足此要求．
...
此积分流形中的切矢量场满足式\eqref{chdf:eqn_solution-ed2}，
同时我们也把这个积分流形称为Pfaff方程组\eqref{chdf:eqn_Pfaff}的积分流形；
...
\end{equation}
其中$f_i ^\alpha$是宗量$\{x^1,\cdots, x^m;y^1,\cdots,y^n\}$的光滑函数．
上式可改写为开子集$U\times V\subset \mathbb{R}^m\times \mathbb{R}^n$的Pfaff方程组
...
\begin{proof}
    ...
    条件\eqref{chdf:eqn_frobenius-Diff-Form-a}；
    这是说如果条件\eqref{chdf:eqn_frobenius-Diff-Form-c}成立，那么Pfaff方程组完全可积．
...
若读者需要更深入了解Frobenius定理在物理中的应用，可参考文献\parencite[Ch.8]{westenholz-1978}；
那里给出了更多的例题（约30道例题），以及更加详尽、全面地描述．
...
这个矩阵的行列式$\det (a_i^j)=\pm 1$，当其值为$+1$时，
我们称两个坐标系是定向相同的，否则是定向相反的．此方式可以推广到高维．


设$V$是$m$维矢量空间，有两组基矢$(e_i)^a$和$(\epsilon_i)^a$
（一般不要求它们正交归一），联系它们的是一个非奇异$m\times m$矩阵$A$，
即$(\epsilon_i)^a = A_i^j (e_j)^a$；当$\det(A)>0$时，
我们称两组基矢量定向相同，当$\det(A)<0$时，我们称两组基矢量定向相反；
也就是行列式的正负将所有基矢组分为两个等价类，此时可称其一为右手的，另一个为左手的．
需要注意，矢量空间的变换矩阵$A$是常数矩阵，它的行列式只能大于或者小于零．
这一方式可以推广至微分流形的切空间（一类矢量空间），但流形切空间是逐点不同的．
用不那么严格的语言来说：
...
\begin{definition}\label{chdf:def_orientation}
    设$M$是$m$维光滑流形，其微分结构$\{(U,\varphi;x^i), (V,\psi;y^i), \cdots \}$是$M$的开覆盖；
    当$U \cap V\neq \varnothing$时，\CJKunderwave{如果}坐标
    变换$\psi \circ \varphi^{-1}: \varphi(U \cap V )\to \psi(U \cap V)$的Jacobi行列
...
肯定不是零；当$p$在流形$M$上移动时，此行列式的正负可能逐点不同，
也就是有的点是正的，有的点是负的．
如果能够通过调节坐标取向（比如令$y^1=-y^1$）{ \footnote{比如通过调节坐标取向已将
...
$J'$是定义\ref{chdf:def_orientation}中Jacobi行列式$J$的转置逆，即$J'=J^{-T}$；可见$J>0$等价于$J'>0$．
这便说明了两个定义是等价的．


我们要求$\omega$是$m$阶的，而流形$M$也是$m$阶的；
由此可知流形$M$上任意两个连续且处处非零的$m$阶外微分型式场$\omega$和$\omega'$只相差一个非零倍数，
即$\omega'=c\cdot \omega$，其中$c\in C^0(M)$．
因$c$连续，故要么$c>0$要么$c<0$，在任何点都有$c\neq 0$．
这更加清晰地说明了：定向只有两种（可称为左手定向或右手定向）．

外微分型式场$\omega$与局部坐标系$(U;x)$定向相符合或者说同向是
指\eqref{chdf:eqn_hfJ}式中$h(x)>0$；如果$h(x)<0$则称定向相反．
...
定义\ref{chdm:def_Dmanifold}中的开集不包含边界，
比如包含边界的实心立方体在此定义下便不是流形；
为了更好地描述现实中的物体，需要给出带边流形的定义．引入记号：
\begin{align}
    \mathbb{R}^m_{-} \equiv & \{(x^1,\cdots,x^m)\in \mathbb{R}^m \mid x^1 \leqslant 0 \}, \label{chdf:eqn_Rn-} \\
    \partial \mathbb{R}^m_{-} \equiv &\{(x^1,\cdots,x^m)\in \mathbb{R}^m \mid x^1 = 0 \}
    = \{0\} \times \mathbb{R}^{m-1}  . \label{chdf:eqn_pRn+}
\end{align}
若局部坐标是从$x^0$开始的，则把上两式中的$x^1 \leqslant 0$换成$x^0 \leqslant 0$，及$x^1 = 0$换成$x^0 = 0$．
若用$\mathbb{R}^m$中标准拓扑来看，$\mathbb{R}^m_{-}$不是$\mathbb{R}^m$的开子集．我们采用如下策略：
使用诱导拓扑（见定义\ref{chtop:def_induced-top}）定义$\mathbb{R}^m_{-}$中的开集．
此时$\mathbb{R}^m_{-}$是诱导拓扑下的开集；$\partial \mathbb{R}^m_{-}$称为$\mathbb{R}^m_{-}$的{\heiti 边界}，
...
若带边流形$M$有局部坐标$(U,\phi)$将点$p\in U$映射到$\partial \mathbb{R}^m_{-}$，
...
\begin{theorem}\label{chdf:thm_induced-orientation}
    ...
    那么可由$M$的微分结构$\mathscr{A}$诱导出$\partial M$上的微分结构$\widetilde{\mathscr{A}}$，
    使得$\partial M$成为$m-1$维光滑无边流形．
    此时包含映射$(\imath,\partial M)$是$M$的正则嵌入闭子流形．
    如果$M$是可定向的，那么$\partial M$也是可（诱导）定向的．
    （证明可参阅\parencite[\S3.4]{cc2001-zh}定理4.1）
\end{theorem}

设$(U;x^i)$是流形$M$的局部开邻域，并且$U\cap \partial M \neq \varnothing$．
设开集$U$的定向为$ ({\rm d}x^1)_{a_1} \wedge \cdots\wedge({\rm d}x^{m})_{a_{m}}$，
{\heiti 约定}：\CJKunderwave{$U\cap \partial M$的诱导定向为
...
文献\parencite[\S 16]{munkres-1997-Analysis}提供了一份较易理解的证明，
它只讨论了$\mathbb{R}^n$空间问题，没有讨论一般流形．

\index[physwords]{单位分解定理}

\begin{definition}
    设光滑流形$M$上存在实函数$f:M\to \mathbb{R}$，
    函数$f$的{\heiti 支撑集}（support set）是指使$f$取非零值的点集的闭包，
    记作${\rm supp} f \equiv \overline{\{p\mid f(p)\neq 0, \forall p\in M \}}$．
\end{definition}

%如果$x\notin {\rm supp} f$，那么$x$必然有一开邻域使得函数$f$在该邻域内恒为零．

\begin{definition}
    设$\Sigma$是光滑流形$M$的一个开覆盖；
    如果$M$的任意一个紧致子集只与$\Sigma$中有限个成员相交，
    则$\Sigma$是$M$的{\heiti 局部有限开覆盖}．
\end{definition}
...
\begin{theorem}\label{chdf:thm_partition-unity}
    （单位分解定理）设$\Sigma=\{W_i\}$是光滑流形$M$的一个开覆盖，
    则在$M$上{\heiti 存在}一族光滑函数$\{g_\alpha\}$，满足如下条件：
    {\bfseries (1)} 对每一个$\alpha$，$0\leqslant g_\alpha \leqslant 1$，
    支撑集${\rm supp}g_\alpha$是紧致的，
    并且有开集$W_i \in \Sigma$使得${\rm supp}g_\alpha \subset W_i$；
...
在数学分析中讲述过重积分定义与计算，以及重积分的变量替换定理．
设$\Omega$和$\Omega'$是$\mathbb{R}^m$中的开集，
且有微分同胚映射$\phi:\Omega'\to \Omega$；
若标量函数$f$在其定义域是连续的，则有
...
设$M$是$m$维的、已定向的、光滑流形；$M$可能带边、也可能无边．
设$\omega \in A^m(M)$，则$\omega$的{\heiti 支撑集}（support set）定义为
\begin{equation}
    {\rm supp}\omega = \overline{\{p\in M \ |\ \omega(p) \neq 0 \}}.
    \qquad \text{注意是闭包}
\end{equation}
我们将积分学的讨论限制在有{\kaishu 紧致支撑集的外微分型式场}；
将要定义的积分是一个{\kaishu 线性映射}，
即$\int : A^m(M)\to \mathbb{R}$是$\mathbb{R}$-线性的．
...
\end{equation}
下面需要验证，这个定义与局域坐标卡选取无关．
设$(M,\psi;y^i)$是此流形的另一套坐标系，$\omega$表达式变为
...
如果不是，比如是一个二阶对称张量场，那么积分定义都成问题；当作变量替换时，
二阶张量场无法由自身变换产上一个Jacobi行列式，
也就无法与基本定理\eqref{chdf:eqn_multiple-integral}相符合．
...
下面考虑一般情形．
设微分流形$M$有定向相符合的坐标卡开覆盖集合$(U_\alpha,\varphi_\alpha;x^i_\alpha)$；
...
因${\rm supp}\omega$是紧致的，所以上式右端只有有限项非零，
即上式右端平凡收敛．命
...
设$(V_\lambda,\psi_\lambda;y^i_\lambda)$是$M$的另一个定向相符的坐标卡，
同时它也是$M$的局部有限开覆盖，再设$\{g_\lambda\}$是从属于$\{V_\lambda\}$的单位分解；
...
\end{align*}
证明过程需要参考定义式\eqref{chdm:eqn_f-cdot-g-circo-gamma}以及式\eqref{chdf:eqn_tmpxayl104}．
上面这个长式推导说明积分定义\eqref{chdf:eqn_def-integral-omega-partition}与局部坐标选取
...
\begin{example}\label{chdf:exam_IonSM}
	子流形上积分定义（本例中假设支撑集都是紧致的）．
\end{example}
在$m$维、可定向光滑流形$M$上有$r(<m)$维全反对称、连续协变张量场$\omega \in A^r(M)$；
一般说来，无法在$M$上直接定义关于$\omega$的积分．
若有$r$维已定向流形$N$，且存在$\phi: N \rightarrow M$是$N$在$M$中的光滑浸入子流形，
则$\phi^* \omega \in A^r(N)$；
此时，$\omega$可以在$M$上定义积分为：
\begin{equation}\label{chdf:eqn_IonSM}
	\int_{\phi(N)} \omega \overset{def}{=} \int_N \phi^* \omega .
\end{equation}
%%容易判断上式为良定义．\qed
...
\section{Stokes--Cartan定理}\label{chdf:sec_stokes-cartan}

下面叙述Stokes--Cartan定理，这是流形论积分学中一个极为重要的定理，
现在所用表述形式是由Cartan给出并证明的．

\begin{theorem}\label{chdf:thm_stokes-cartan}
    设$M$是$m+1$维、已定向、带边光滑流形，$\omega \in A^{m}(M)$，则
    \begin{equation}\label{chdf:eqn_stokes-cartan}
        \int_{M} {\rm d} \omega =  \int_{\partial M} \imath^{*} \omega .
    \end{equation}
    其中$\partial M$是带边流形$M$的边界，具有从$M$诱导的定向（见\S\ref{chdf:sec_manifold-boundary}末尾）．
    映射$\imath:\partial M \to M$是包含映射，
    使$(\imath,\partial M)$成为$M$的正则嵌入闭子流形．
\end{theorem}

\begin{proof}
    需注意$\omega \in A^{m}(M)$是$m+1$维光滑流形$M$上的$m$阶外微分型式场，
    无论是在流形$M$上，还是在$\partial M$上，积分定义都略显含混；
    需要用包含映射将其拉回到$\partial M$上，
    使其成为$\partial M$上的外微分型式场，然后才有良性积分定义．
    
    为了与相对论内容匹配，取局部坐标为$\{x^0,x^1,\cdots,x^m\}$，即从“$0$”开始．
    
    \fbox{甲}
    设$\left\{\left(U_\alpha, \varphi_\alpha\right)\right\}$是$M$的定向相符的坐标卡集，
    使得局部坐标卡集$\left\{U_\alpha\right\}$成为$M$的开覆盖；
    $\left\{h_\alpha\right\}$是从属于$\left\{U_\alpha\right\}$的单位分解，则
    $$
    \omega=\left(\sum\nolimits_\alpha h_\alpha\right) \cdot \omega
    =\sum\nolimits_\alpha \left(h_\alpha \cdot \omega\right) .
    $$ 
    因为支撑集${\rm supp} \omega$ 是紧致的，
    故${\rm supp}\omega$只与$\left\{U_\alpha\right\}$中有限多个成员相交，
    因此上式最右端为平凡收敛．于是
    $$
    \int_M \mathrm{d} \omega=\sum_\alpha \int_M \mathrm{d}\left(h_\alpha \cdot \omega\right),
    $$  
    并且
    $$
    \int_{\partial M} \imath^* \omega=\sum_\alpha \int_{\partial M} \imath^*\left(h_\alpha \cdot \omega\right) .
    $$
    所以，只要对于每一个指标$\alpha$证明下式成立就可以了：
    \begin{equation}
    	\int_M \mathrm{d}\left(h_\alpha \cdot \omega\right)=\int_{\partial M} \imath^*\left(h_\alpha \cdot \omega\right) .
    \end{equation}
    注意到此时外微分式$h_\alpha \cdot \omega$的特性是
    $$
    {\rm supp} \left(h_\alpha \cdot \omega\right) \subset {\rm supp} h_\alpha \cap {\rm supp} \omega \subset U_\alpha,
    $$
    即支撑集 ${\rm supp} \left(h_\alpha \cdot \omega\right)$是紧致的，并且包含在坐标域$U_\alpha$内．
    这样，我们已经把问题归结为对于具有如下性质的$m$次外微分式$\omega$证明式\eqref{chdf:eqn_stokes-cartan}成立：
    ${\rm supp}\omega$是紧致的，并且存在$M$的、与定向相符的坐标卡$(U, \varphi)$，
    使得${\rm supp} \omega \subset U$．也就是说，不显示写出单位分解$\{h_\alpha\}$，以令证明过程简洁一些．
    
    $\forall p \in U$，令$x^i(p)=(\varphi(p))^i$．假定$\omega$的局部坐标表达式为：
    \begin{equation}
    	\omega_{a_1 \cdots a_m}=\sum_{j=0}^m (-)^{j} a_j (\mathrm{d} x^0)_{a_1} \wedge \cdots 
    	\wedge \widehat{\mathrm{d}x^j}  \wedge \cdots \wedge (\mathrm{d} x^m)_{a_m},
    \end{equation}
    其中$a_j \in C^{\infty}(U)$，且${\rm supp} a_j \subset U$．它的外微分$\mathrm{d} \omega$的表达式为
    \begin{align}
    	\mathrm{d}_{a_0} \omega_{a_1 \cdots a_m}= 
    	\left(\sum\nolimits_{j=0}^m \frac{\partial a_j}{\partial x^j}\right) 
    	(\mathrm{d} x^0)_{a_0} \wedge \cdots \wedge (\mathrm{d} x^m)_{a_m} .
    \end{align}    
    下面分两种情形进行讨论．
    
    \fbox{乙} $U \cap \partial M=\varnothing$．
    此时$U \subset {\rm i}(M)$且$\imath^* \omega=0$，
    故式\eqref{chdf:eqn_stokes-cartan}的右端为零．
    我们的目标是证明式\eqref{chdf:eqn_stokes-cartan}左端也是零．
    因${\rm supp} \omega$是紧的，
    故不妨设 $\varphi(U) \subset \mathbb{R}_{-}^{m+1}$是有界的开区域，
    因此有充分大的正实数$K$，使得$ \varphi(U) \subset \mathbb{R}_{-}^{m+1} \cap C$，其中
    \begin{equation}\label{chdf:eqn_sc-C}
    	C=\left\{\left(x^0, \cdots, x^m\right) \in \mathbb{R}^{m+1} \mid 
    	-K \leqslant x^i \leqslant K, \ 0 \leqslant i \leqslant m \right\} .
    \end{equation}
    现在${\rm supp}\left(a_j \circ \varphi^{-1}\right) \subset \varphi(U)$，
    而$\varphi(U) \backslash {\rm supp}\left(a_j \circ \varphi^{-1}\right)$是开集，
    所以可以让函数$a_j \circ \varphi^{-1}$光滑地延拓到整个$\mathbb{R}_{-}^{m+1} \cap C$，
    使得它在$\varphi(U)$外的值为零．由于$U \subset {\rm i}(M)$，故有
    ${\rm supp}\left(a_j \circ \varphi^{-1}\right) \subset \varphi(U) \subset {\rm i} (\mathbb{R}_{-}^{m+1})$．
    所以扩充后的函数$a_j \circ \varphi^{-1}$在$\mathbb{R}_{-}^{m+1} \cap C$的边界上皆取零值．
    这样，对于$1 \leqslant j \leqslant m$ 有
    \begin{align*}
    	\int_{-K}^K \frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j} \mathrm{d} x^j 
    	= & +a_j \circ \varphi^{-1}\left(x^0, \cdots, x^{j-1}, K, x^{j+1}, \cdots, x^m\right) \\
    	& -a_j \circ \varphi^{-1}\left(x^0, \cdots, x^{j-1},-K, x^{j+1}, \cdots, x^m\right) 
    	=  0.
    \end{align*}
    同时
    \begin{align*}
    \int_{-K}^0 \frac{\partial\left(a_0 \circ \varphi^{-1}\right)}{\partial x^0} \mathrm{d} x^0
    =a_0 \circ \varphi^{-1}\left(0, x^1, \cdots, x^{m} \right)
    -a_0 \circ \varphi^{-1}\left(-K, x^1, \cdots, x^{m} \right)    =0.
    \end{align*}
    所以，有
    \begin{align*}
    		\int_M \mathrm{d} \omega= & \int_{\varphi(U)}\left(\sum_{j=0}^m 
    		\frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j}\right) \mathrm{d} x^0 \cdots \mathrm{d} x^m \\
    		= & \sum_{j=0}^m \int_{\mathbb{R}_{-}^{m+1} \cap C} \frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j} 
    		\mathrm{d} x^0 \cdots \mathrm{d} x^m \\
    		= & \sum_{j=1}^{m} \int_{\substack{|x^i| \leqslant K \\ i \neq 0, j \\ x^0 \leqslant 0}}
    			\left(\int_{-K}^K \frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j} 
    			\mathrm{d} x^j\right) \mathrm{d} x^1 \cdots \widehat{\mathrm{d}x^j}  \cdots \mathrm{d} x^m \\
    		& +\int_{\substack{|x^i| \leqslant K \\  i > 0}}
    			\left(\int_{-K}^0 \frac{\partial\left(a_0 \circ \varphi^{-1}\right)}{\partial x^0} \mathrm{d} x^0\right) 
    			\mathrm{d} x^1 \cdots \mathrm{d} x^{m}=0 .
    \end{align*}
    
    \fbox{丙}
    $U \cap \partial M \neq \varnothing$．此时$(U, \varphi)$是$M$的、与定向相符的坐标卡，并且
    \begin{equation}\label{chdm:eqn_sc-UM}
    	U \cap \partial M=\left\{p \in U \mid \left(\varphi(p)\right)^0=0\right\} .
    \end{equation}    
    如在情形\fbox{乙}所述，可取充分大的正实数$K$使得$\varphi(U) \subset \mathbb{R}_{-}^{m+1} \cap C$，
    其中$C$是如式\eqref{chdf:eqn_sc-C}给出的方体．
    由于${\rm supp}\left(a_j \circ \varphi^{-1}\right) \subset \varphi(U)$，
    而$\varphi(U) \backslash {\rm supp}\left(a_j \circ \varphi^{-1}\right)$是开集，
    故每一个函数$a_j \circ \varphi^{-1}$可以光滑地延拓到整个$\mathbb{R}_{-}^{m+1} \cap C$，
    使得它在$\varphi(U)$外的值为零．特别是，扩充后的函数$a_j \circ \varphi^{-1}$
    在$\mathbb{R}_{-}^{m+1} \cap C$的边界上除了$\partial \mathbb{R}_{-}^{m+1} \cap C$以外的值皆为零．
    这样，我们有
    \begin{align*}
    	\int_M \mathrm{d} \omega=&
    	\int_{\varphi(U)}\left(\sum_{j=0}^m \frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j}\right) 
    	\mathrm{d} x^0 \cdots \mathrm{d} x^m\\
    	= & \sum_{j=0}^m \int_{\mathbb{R}_{-}^{m+1} \cap C} \frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j} 
    	\mathrm{d} x^0 \cdots \mathrm{d} x^m \\
    	= & \sum_{j=1}^{m} \int_{\substack{|x^i| \leqslant K \\ i \neq 0, j \\ x^0 \leqslant 0}}
    		\left(\int_{-K}^K \frac{\partial\left(a_j \circ \varphi^{-1}\right)}{\partial x^j} \mathrm{d} x^j\right) 
    		\mathrm{d} x^1 \cdots \widehat{\mathrm{d}x^j}  \cdots \mathrm{d} x^m \\
    	& +\int_{\substack{|x^i| \leqslant K \\ 1 \leqslant i \leqslant m}}
    	\left(\int_{-K}^{0} \frac{\partial\left(a_0 \circ \varphi^{-1}\right)}{\partial x^0} \mathrm{d} x^0\right) 
    	\mathrm{d} x^1 \cdots \mathrm{d} x^{m} \\
    	= & \int_{\substack{|x^{i}| \leqslant K \\
    			1 \leqslant i \leqslant m}} a_0 \circ \varphi^{-1}\left(0,x^1, \cdots, x^{m}\right) 
    		\mathrm{d} x^1 \cdots \mathrm{d} x^{m} .
    \end{align*}
    对于边界流形$\partial M$来说，式\eqref{chdm:eqn_sc-UM}表明$U \cap \partial M$是$\partial M$的坐标域，
    它的诱导定向见\S\ref{chdf:sec_manifold-boundary}末尾．
    因为${\rm supp}\left(\left.\imath^* \omega\right|_{\partial M}\right) \subset U \cap \partial M$，
    所以
    \begin{align*}
    	\int_{\partial M} \imath^* \omega=&\int_{U \cap \partial M} \imath^* \omega 
    	=  \int_{U \cap \partial M} \sum_{j=0}^m(-)^{j} a_j 
    	\mathrm{d} x^0 \wedge \cdots \wedge \widehat{\mathrm{d}x^j}  \wedge \cdots \wedge \mathrm{d} x^m \\
    	\xlongequal[\text{的诱导定向}]{\text{使用}U\cap\partial M} & 
    	\int_{U \cap \partial M}(-)^{0} a_0 \mathrm{d} x^1 \wedge \cdots \wedge \mathrm{d} x^{m} \\
    	= & \int_{\varphi(U) \cap \partial \mathbb{R}_{-}^{m+1}}
    	\left(a_0 \circ \varphi^{-1}\right) \mathrm{d} x^1 \cdots \mathrm{d} x^{m} \\
    	= & \int_{\partial \mathbb{R}_{-}^{m+1} \cap C}\left(a_0 \circ \varphi^{-1}\right) 
    	\mathrm{d} x^1 \cdots \mathrm{d} x^{m} \\
    	= & \int_{\substack{|x^i| \leqslant K \\ 1 \leqslant i \leqslant m}} 
    		a_0 \circ \varphi^{-1}\left(0, x^1, \cdots, x^{m}\right) 
    		\mathrm{d} x^1 \cdots \mathrm{d} x^{m} .
    \end{align*}
    故有$\int_M \mathrm{d} \omega=\int_{\partial M} \imath^* \omega$．
\end{proof}


\begin{example}
    设$M$是$\mathbb{R}^2$上有界开集的闭包，其边界是$C^r(r>0)$简单闭曲线．
...
\begin{example}
    ...
    具有从$M$诱导的定向（见\S\ref{chdf:sec_manifold-boundary}末尾）．
...
\begin{exercise}
	Stokes--Cartan公式\eqref{chdf:eqn_stokes-cartan}等号右端的符号与$\partial M$边界的选择以及诱导定向密切相关．
	{\bfseries (1)}如果在式\eqref{chdf:eqn_Rn-}中选择$\{x^0 \geqslant 0\}$，
	请查验式\eqref{chdf:eqn_stokes-cartan}右端应该是正号、还是负号？
	{\bfseries (2)}如果在式\eqref{chdf:eqn_Rn-}中选择$\{x^m \geqslant 0\}$（或$\{x^m \leqslant 0\}$）为边界，	符号该如何确定？
	{\bfseries (3)}更改诱导定向，等号右端符号如何变化？
	（可参考\parencite[\S 3.4]{cc2001-zh}、\parencite[\S 5.2]{Luqk-2026}、
	\parencite[\S 22.6]{tu-IM-2011}等）
\end{exercise}
