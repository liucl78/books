...
\begin{proposition}\label{chhs:thm_fBC}
	若$f$是$\mathscr{D}(f)\subset X$中的线性泛函，
	则$f$是连续泛函等价于$f$是有界泛函．
...
由命题\ref{chhs:thm_FSB}可知：有限维内积空间必是Hilbert空间．
...
\begin{proof}
	...
	\end{align*}
	可以验证它是一个满足定义\ref{chhs:def_IPN}的内积．
\end{proof}
...
上面两个例子表明：可以将不完备的$C[a,b]$延拓为完备的$L^2[a,b]$．
与度量空间、赋范空间类似，Hilbert空间也有如下重要定理：
...
\end{equation}
上式中$t$为实参数．完备性证明参见\parencite[例 1.4.11]{zhang-lin-2021}．
{\kaishu 大多数情形下，量子物理中的Hilbert空间就是指上述的$L^2(\mathbb{R}^3)$，
参数$t$一般理解成时间．}
\qed
...
\begin{definition}\label{chhs:def_bld}
	Hilbert空间$\mathcal{H}$的连续对偶空间$\mathcal{H}'$为$\mathcal{H}$上的连续线性泛函全体．
\end{definition}
...
完备的线性空间$\mathcal{H}'$有了内积之后便成为了Hilbert空间．
...
\begin{definition}
	若$T$是单一映射且$T^{\dagger}=T^{-1}$，则称$T$是{\heiti 酉算符}．
\end{definition}
...
\begin{definition}
	$T$在Hilbert空间$\mathcal{H}$中是{\heiti 稠定的}是指$\mathscr{D}(T)$在$\mathcal{H}$中是稠密的．
\end{definition}


\begin{definition}
	设$T: \mathscr{D}(T) \rightarrow \mathcal{H}$是复Hilbert空间$\mathcal{H}$中的（可能无界的）稠定线性算符，
