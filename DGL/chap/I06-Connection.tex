...
现在联络概念已推广到纤维丛上，而本章只讨论切丛上的联络，
其实质是定义在切矢量场（切丛的截面）上的一种方向导数；并由此产生一种新的微分法则，
即所谓的“协变微分”（或共变微分）．
...
$C^\times_\times(\nabla_X K_{\cdots}^{\cdots}) \overset{def}{=} \nabla_X(C^\times_\times K_{\cdots}^{\cdots})$．
    ...
    则称$\nabla_a$是作用在光滑流形$M$上张量场的联络，仍称之为仿射联络．
\end{definition}


\begin{remark}\label{chccr:rek_ccl}
    仿射联络必然存在且不只一个，请参考定理\ref{chfb:thm_existence}．
\end{remark}
\begin{remark}
...
\begin{equation}\label{chccr:eqn_deltaac}
	\sum_i ({\rm d} x^i)_c \left(\frac{\partial }{\partial x^i}\right) ^b = \delta_c^b .
\end{equation}
关于此式的进一步解释可参见例题\ref{chccr:exam_soldering}．
利用上式可以证明式\eqref{chccr:eqn_Christoffel2}的等价性；
...
\begin{equation}
    ...
    \Gamma^{k}_{ji} \left(\frac{\partial }{\partial x^k}\right) ^a ({\rm d} x^i)_c .
\end{equation}
再利用式\eqref{chccr:eqn_deltaac}即可得\eqref{chccr:eqn_Christoffel2}的第二式．
反向证明留给读者当练习．
...
\begin{equation}
    ...
    \frac{\partial }{\partial y^\alpha} {\quad \Leftrightarrow \quad }
...
\begin{align*}
    ...
    \frac{{\partial {x^j}\left( z \right)}}{{\partial {z^B}}}
     + \frac{{\partial {y^\rho }}}{{\partial {x^l}}}\frac{{{\partial ^2}{x^l}
             \left( z \right)}}{{\partial {z^A}\partial {z^B}}} .
\end{align*}
令$z\equiv y$，则上式最左端恒为零；有
\begin{equation} \label{chccr:eqn_d2ydx2}
	\frac{{{\partial ^2}{y^\rho }}}{{\partial {x^i}\partial {x^j}}}
		\frac{{\partial {x^i}\left( y \right)}}{{\partial {y^\alpha}}}
		\frac{{\partial {x^j}\left( y \right)}}{{\partial {y^\beta}}}
		+ \frac{{\partial {y^\rho }}}{{\partial {x^l}}}\frac{{{\partial ^2}{x^l}
				\left( y \right)}}{{\partial {y^\alpha}\partial {y^\beta}}} =0.
\end{equation}
利用上式便可证明\eqref{chccr:eqn_Exchange-Christoffel}．
...
上面从联络得到了联络系数．反之，
在$m$维光滑流形$M$的每个局部开集$(U;x^i)$中给定$m^3$个光滑函数$\Gamma^{k}_{ji}\in C^\infty(U)$，
且在坐标变换时这些系数满足式\eqref{chccr:eqn_Exchange-Christoffel}；
那么在$M$上存在唯一的仿射联络与之相对应．证明如下；
设流形$M$有开子集$(U;x^i)$和$(V;y^\alpha)$，并且$U\cap V \neq \varnothing$；
则$\forall v^b\in \mathfrak{X}(U\cap V)$，可设$v^b|_{U\cap V}
...
%     +v^\mu \Gamma^{\rho}_{\mu\nu}(y) \right)
%     \Bigl(\frac{\partial}{\partial y^\rho}\Bigr)^b({\rm d}y^\nu)_a    \\
     \xlongequal[\ref{chccr:eqn_d2ydx2}]{\ref{chccr:eqn_Exchange-Christoffel-a}} &
      \left( \frac{\partial v^\rho}{\partial y^\nu}
     +v^\mu \Gamma^{\rho}_{\mu\nu}(y) \right)
     \Bigl(\frac{\partial}{\partial y^\rho}\Bigr)^b({\rm d}y^\nu)_a
...
\subsubsection{普通偏导数算符}

给定$m$维仿射联络空间$(M,\nabla_a)$的局部坐标系$(U;x^i)$，下面引入一个常用记号．
...
这条原则在此处变成（下式中第一条）：在坐标域$(U;x^i)$内偏导数$\partial_a$满足如下规则，
...
联络不能只在一个坐标域内成立，在进行坐标变换时，
联络形式必须是协变的（不同坐标系下，数学表达式“长得一样”才行）．
再给$M$的另一个局部坐标系$(V;y^\alpha)$，且$U\cap V \neq \varnothing$，
那么对$V$中自然坐标基矢的偏导数是：
\begin{align*}
...
有了联络系数，逆变、协变矢量场的协变导数分量表示为（计算见后）：
\begin{align}
...
\begin{example}\label{chccr:exm_c-c=t}
    ...
    \end{equation}
    虽然联络系数不是张量，但由上式，
    利用定理\ref{chdm:thm_Tensor-Characterization-Lemma}易证$\Xi_{ca}^b$是$\Tpq{1}{2}$型张量场．
...
\CJKunderwave{很明显挠率下标是反对称的}，在局部坐标系的分量表达式为
...
\subsection{无挠联络的存在性}\label{chccr:sec_no-torsion}

我们可以对仿射联络系数的下标进行对称和反对称分解，有
\begin{equation}
	\Gamma^k_{ij} = \frac{1}{2}\left(\Gamma^k_{ij} + \Gamma^k_{ji}\right)
	+\frac{1}{2}\left(\Gamma^k_{ij} - \Gamma^k_{ji}\right)
	\overset{def}{=} \oversetmy{s}{\Gamma}^k_{ij} + \oversetmy{a}{\Gamma}^k_{ij}.
\end{equation}
其中$\oversetmy{s}{\Gamma}^k_{ij}$是对称部分，$\oversetmy{a}{\Gamma}^k_{ij}$是反对称部分．

当$\Gamma^k_{ij}$的反对称部分$\oversetmy{a}{\Gamma}^k_{ij}$为零时，联络无挠，这是平庸情形．

若$\Gamma^k_{ij}$是有挠的，则反对称部分$\oversetmy{a}{\Gamma}^k_{ij}$肯定非零；
无论$\Gamma^k_{ij}$有挠与否，对称部分$\oversetmy{s}{\Gamma}^k_{ij}$肯定无挠的．
只剩下最关键的一步，证明$\oversetmy{s}{\Gamma}^k_{ij}$是仿射联络，见下．
根据注\ref{chccr:remark_con-Nabla}，我们只需验证$\oversetmy{s}{\Gamma}^k_{ij}$满
足式\eqref{chccr:eqn_Exchange-Christoffel}即可．
\begin{align*}
		0 = & \Gamma^{k}_{ji}(x) \frac{\partial y^\rho}{\partial x^k} -
		\frac{\partial y^\beta}{\partial x^j} \frac{\partial y^\alpha}{\partial x^i}
		\Gamma^{\rho}_{\beta\alpha}(y) -2 \frac{\partial^2 y^\rho}{\partial x^j \partial x^i}
		+\Gamma^{k}_{ij}(x) \frac{\partial y^\rho}{\partial x^k} -
		\frac{\partial y^\alpha}{\partial x^i} \frac{\partial y^\beta}{\partial x^j}
		\Gamma^{\rho}_{\alpha\beta}(y)  \\
		=& 2\oversetmy{s}{\Gamma}^k_{ji}  \frac{\partial y^\rho}{\partial x^k}
		-2\frac{\partial y^\beta}{\partial x^j} \frac{\partial y^\alpha}{\partial x^i}
		\oversetmy{s}{\Gamma}^{\rho}_{\beta\alpha}(y) 
		- 2\frac{\partial^2 y^\rho}{\partial x^j \partial x^i} .
\end{align*}
由上式最后一步可见：$\oversetmy{s}{\Gamma}^k_{ij}$满足式\eqref{chccr:eqn_Exchange-Christoffel}，
故它是无挠的仿射联络．
...
\begin{equation}\label{chccr:eqn_exterior-differential-covD}
    ({\rm d}\omega)_{b a_{1}\cdots a_{r}} = (r+1) \nabla _{[b}\omega_{a_{1}\cdots a_{r}]} .
\end{equation}
...
\begin{definition}\label{chccr:def_px}
    给定$m$维仿射空间$(M,\nabla_a)$，$\gamma:[r,s]\to M$是$M$中
    的一条光滑曲线，$T^b=(\frac{\partial}{\partial t})^b|_{\gamma(t)}$是$\gamma(t)$的切线切矢量．
    $\forall Y^a \in \mathfrak{X}(\gamma(t))$，若沿曲线$\gamma(t)$上有
...
\begin{equation}
    Y^a(t)=P_0^t \left(Y^a_0\right), \qquad 0 \leqslant t \leqslant c .
\end{equation}
...
\begin{theorem}\label{chccr:thm_parallel-transport}
    设$(M, \nabla_a)$是一个$m$维仿射联络空间，$\gamma:[0, c] \to M$是$M$中任意一条光滑曲线．
...
\end{theorem}
\begin{proof}
记 $p=\gamma(0)$．在$T_p M$中取定一个基底 $\{(e_i)^a\}$，
且设$\bigl(e_i(t)\bigr)^a=P_0^t\bigl((e_i)^a\bigr)$，
...
式\eqref{chccr:eqn_parallel-transport}指出：
$\nabla_T Y^a$是切矢场$Y^a$在邻近两点$\gamma(t)$、
$\gamma(t+\Delta t)$的值之差与$\Delta t$的商的极限；
只不过 $Y^a\bigl(\gamma(t+\Delta t)\bigr)$是$M$在$\gamma(t+\Delta t)$处的切矢，
必须借助于线性同构$P_{t+\Delta t}^t$把它变为在$\gamma(t)$处的切矢之后，
才能与$Y^a\bigl(\gamma(t)\bigr)$相减． 
这也是联络的一个表观解释，两点$\gamma(t)$、$\gamma(t+\Delta t)$的值原来是没有关联，无法作差；
现在利用曲线$\gamma$上的平行移动将这两点联系起来，它们可以作差了；
...
现代几何学中的黎曼曲率张量似乎是一个神秘的东西．
部分地是因为其名称显示它能够对难以捉摸的“空间弯曲”性质给出一个精确度量；
部分地是因为尽管看上去它很简单，但它似乎控制了黎曼几何学的每一方面；
又部分地因为虽然从过去到现在，学者们已花费极大努力，但距离完全掌握它还相距甚远．
笔者学识所限，无法深入展示黎曼曲率的种种精妙，
我们只给出它的定义并叙述它的一些简单性质．
若读者有兴趣，可参阅更专业书籍．
...
由式\eqref{chccr:eqn_Riemannian13-Vec-commutator}易得（注意利用挠率下标反对称性）：
\begin{align*}
	R_{cab}^d Z^c =- \left(\nabla_b \nabla_a Z^d -\nabla_a \nabla_b Z^d 
	+T_{ba}^e \nabla_e Z^d \right)	=-R_{cba}^d Z^c 
\end{align*}
因$Z^c$的任意性，从而可知黎曼曲率$R_{cab}^d$关于指标$a$、$b$反对称，即
\begin{equation}\label{chccr:eqn_Rab=-Rba}
    R_{cab}^d= -R_{cba}^d . \qquad \text{注意这个反对称性与挠率无关}
\end{equation}
...
我们定义{\bfseries \heiti Ricci曲率}以及另外一个曲率；这两个定义无需度量．
\begin{align}
	R_{cb} \overset{def}{=}& R_{\hphantom{a} cab}^a . \label{chccr:eqn_RicciCurvatured2}\\
	R'_{cd} \overset{def}{=}& R_{\hphantom{a} acd}^a . \label{chccr:eqn_Ricci222}
\end{align}
显然有$R'_{cd} = - R'_{dc}$，曲率\eqref{chccr:eqn_Ricci222}几乎无用．
...
\begin{equation}\label{chccr:eqn_Ricci-component}
    R_{jn} \equiv R_{\hphantom{a} jkn}^k = {\partial_k} \Gamma_{jn}^{k} -\partial_n \Gamma_{jk}^{k}
...
\begin{align*}
    ...
    =& f \cdot  R_{ cab}^d {X^a}{ Y^b}{Z^c} .
\end{align*}
它对三个位置均有$C^\infty(M)$-线性，故它是$\Tpq{1}{3}$型张量．
...
\begin{align*}
   ...
   &= Z^e \nabla_c R_{eab}^d + R_{eab}^d \nabla_c Z^e  +
      Z^e \nabla_a R_{ebc}^d + R_{ebc}^d \nabla_a Z^e +
      Z^e \nabla_b R_{eca}^d + R_{eca}^d \nabla_b Z^e \\
   &{\xLongrightarrow{\ref{chccr:eqn_Riemannian13-Tensor-commutator}}}
      +R_{eca}^d \nabla_b Z^e  -R_{bca}^e \nabla_e Z^d
      +R_{eab}^d \nabla_c Z^e  -R_{cab}^e \nabla_e Z^d
      +R_{ebc}^d \nabla_a Z^e  -R_{abc}^e \nabla_e Z^d \\
   &= Z^e \nabla_c R_{eab}^d + R_{eab}^d \nabla_c Z^e  +
...
\begin{exercise}
	证明式\eqref{chccr:eqn_Riemannian13-CoVec-commutator}、
	式\eqref{chccr:eqn_Riemannian13-Tensor-commutator}；并给出更一般性的公式．
\end{exercise}
...
\begin{align}\label{chccr:eqn_natural-base_change}
    ...
    ({e} ^i)_a = \left({\rm d} x^\alpha \right)_a \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\alpha}   ;
...
\index[physwords]{Solder form}

\begin{example}\label{chccr:exam_soldering}
	联接型式（Soldering，或者 Solder form）特例．
\end{example}
联接型式是纤维丛中的概念，在切丛$TM$上有一个特简单的例子；命
\begin{equation}\label{chccr:eqn_soldering}
	\delta^a_b \overset{def}{=} \sum\nolimits_{i=1}^{m} (e_i)^a (e^i)_b ;
	\qquad \delta^a_b \ \text{即为}\ {\rm  Soldering } \ \text{中特例}.
\end{equation}
对于任意切矢量$v^b=v^j (e_j)^b$或余切矢量$\omega_a=\omega_k (e^k)_a$，有
\begin{align*}
	\delta^a_b v^b = (e_i)^a (e^i)_b v^j (e_j)^b = (e_i)^a v^j \delta^i_j = (e_i)^a v^i = v^a;
	\quad \text{类似有}\ \delta^a_b \omega_a = \omega_b.
\end{align*}
换句话说式\eqref{chccr:eqn_soldering}为切丛上的恒等映射；那么，有
\begin{equation*}
	\nabla_c v^a = \nabla_c (\delta^a_b v^b) 
	\xlongequal[\text{法则}]{\text{Leibniz}}
	\left(\nabla_c \delta^a_b\right) v^b + \delta^a_b \nabla_c v^b .
\end{equation*}
因$v^a$是任意切矢量场，故必然可以得到
\begin{equation}\label{chccr:eqn_Dsoldering}
	\nabla_c \delta^a_b \equiv 0.
\end{equation}
目前，我们还没有引入度规；在引入度规后（\S \ref{chrg:sec_riemann}），会发现上面几式与度规相容．
需要注意式\eqref{chccr:eqn_soldering}的定义无需度规；联络$\nabla_c$可以有挠、也可以无挠．
\qed

\index[physwords]{联络!型式}
\subsection{联络型式}\label{chccr:sec_connectionForm}

%%{\  \Leftrightarrow \ } \Gamma^{k}_{ij}  =  (e^k)_a (e_j)^b \nabla _b (e_i)^a


给定联络$\nabla _a$（有挠无挠均可），
在任意基底场$(e_i)^a$及其对偶标架场$(e^i)_a$中，
与坐标基底场系数\eqref{chccr:eqn_Christoffel2}类似，
同样可以定义联络系数（仍记为$\Gamma$）：
\begin{subequations}\label{chccr:eqn_concoef}
\begin{align}
    (e_j)^b \nabla _b (e_i)^a  \overset{def}{=} & +\Gamma^{k}_{ij} (e_k)^a
    {\quad \Leftrightarrow \quad }
    \nabla _b (e_i)^a = +\Gamma^{k}_{ij} (e^j)_b (e_k)^a,   
    \label{chccr:eqn_concoef-a} \\
    (e_j)^b \nabla _b (e^i)_a  =& -\Gamma^{i}_{kj} (e^k)_a
    {\quad \Leftrightarrow \quad }
    \nabla _b (e^i)_a = -\Gamma^{i}_{kj}(e^j)_b  (e^k)_a  . 
    \label{chccr:eqn_concoef-b}
\end{align}
\end{subequations}
上式推导中用到了式\eqref{chccr:eqn_soldering}、\eqref{chccr:eqn_Dsoldering}．
自然坐标系$\{x^i\}$基底下的无挠联络系数关于下标是对称的，
即$\oversetmy{x}{\Gamma}^{k}_{ij}=\oversetmy{x}{\Gamma}^{k}_{ji}$．
然而，一般基底场$(e_i)^a$的无挠联络系数$\Gamma^{k}_{ij}$通常不具有这种对称性；
...
定义{\heiti\bfseries 联络1型式场}（或联络一型式场，或联络型式场）：
...
下面罗列一些标架场下的协变导数公式．对于任意标量场$f$，有
\begin{align}
	{e} _i (f)&= C_{\hphantom{a} i}^{j}(x) \frac{\partial f}{\partial x^j}, \\
	{\rm d}_a (f) &=  \frac{\partial f}{\partial x^j} ({\rm d} x^j)_a
	= \frac{\partial f}{\partial x^i}  C_{\hphantom{a} \nu}^{i}\cdot
	(C^{-T})^{\hphantom{a} \nu}_{j} ({\rm d} x^j)_a
	= [{e} _\nu (f)] (e^\nu)_a .
\end{align}
依照约定，切矢场作用在标量场上时，去掉抽象指标．易得：
\begin{align}
...
\begin{example}
	广义相对论中的四标架假设（tetrad postulate）．
\end{example}

先参见例题\ref{chccr:exam_soldering}．
我们将联接型式变换一下，可以导出两个公式．
\begin{align*}
	0 =& \nabla_c \delta^a_b = \nabla_c \left((\partial_\alpha)^a C_{\hphantom{a} i}^{\alpha} (e^i)_b  \right) \\
	%	=& \frac{\partial C_{\hphantom{a} i}^{\beta} }{\partial x^\sigma} (\partial_\beta)^a (e^i)_b ({\rm d}x^\sigma)_c
	%	+ C_{\hphantom{a} i}^{\alpha} \oversetmy{x}{\Gamma}_{\alpha \sigma}^\beta (\partial_\beta)^a (e^i)_b ({\rm d}x^\sigma)_c
	%	- C_{\hphantom{a} l}^{\beta} \Gamma^l_{ij} (\partial_\beta)^a (e^i)_b (e^j)_c \\
	=& \left(\frac{\partial C_{\hphantom{a} i}^{\beta} }{\partial x^\sigma}
	+ C_{\hphantom{a} i}^{\alpha} \oversetmy{x}{\Gamma}_{\alpha \sigma}^\beta
	- C_{\hphantom{a} l}^{\beta} \Gamma^l_{ij} \left(C^{-T}\right)^{\hphantom{\alpha}j}_{\sigma} 
	\right) (\partial_\beta)^a (e^i)_b ({\rm d}x^\sigma)_c .
\end{align*}
去掉基矢，有
\begin{subequations}\label{chccr:eqn_tetrad-postulate}
	\begin{align}
		\frac{\partial C_{\hphantom{a} i}^{\beta} }{\partial x^\sigma}
		+ C_{\hphantom{a} i}^{\alpha} \oversetmy{x}{\Gamma}_{\alpha \sigma}^\beta
		- C_{\hphantom{a} l}^{\beta} \Gamma^l_{ij} \left(C^{-T}\right)^{\hphantom{\alpha}j}_{\sigma} = &0,
		\label{chccr:eqn_tetrad-postulate-a} \\
		\frac{\partial \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\rho} }{\partial x^\sigma}
		- \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\mu} \oversetmy{x}{\Gamma}^\mu_{\rho\sigma} 
		+ \left(C^{-T}\right)^{\hphantom{\alpha}l}_{\rho} \Gamma^{i}_{lj} 
		\left(C^{-T}\right)^{\hphantom{\alpha}j}_{\sigma} = & 0. \label{chccr:eqn_tetrad-postulate-b}
	\end{align}
\end{subequations}
上式无非是\eqref{chccr:eqn_GAMMA-trans}或其变种．
式\eqref{chccr:eqn_tetrad-postulate-b}是由$0 = \nabla_c ( (e_i)^a ({\rm d}x^\mu)_b 
(C^{-T})^{\hphantom{\alpha}i}_{\mu} )$推演得到的．
有些文献将式\eqref{chccr:eqn_tetrad-postulate}称为“tetrad postulate”（四标架假设）；
由于上两式为微分恒等式，不是假设，故这个称呼不准确．\qed

\index[physwords]{tetrad postulate}

%\begin{align*}
%	0 =& \nabla_c \delta^a_b = \nabla_c \left( (e_i)^a ({\rm d}x^\mu)_b \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\mu}  \right) \\
%		=& \frac{\partial \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\rho} }{\partial x^\sigma} (e_i)^a ({\rm d}x^\rho)_b ({\rm d}x^\sigma)_c
%		- \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\mu} \oversetmy{x}{\Gamma}^\mu_{\rho\sigma} (e_i)^a  ({\rm d}x^\rho)_b ({\rm d}x^\sigma)_c
%		+ \left(C^{-T}\right)^{\hphantom{\alpha}l}_{\rho}  \Gamma^{i}_{lj} (e_i)^a ({\rm d}x^\rho)_b (e^j)_c  \\
%	=& \left(\frac{\partial \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\rho} }{\partial x^\sigma}
%	- \left(C^{-T}\right)^{\hphantom{\alpha}i}_{\mu} \oversetmy{x}{\Gamma}^\mu_{\rho\sigma} 
%	+ \left(C^{-T}\right)^{\hphantom{\alpha}l}_{\rho} \Gamma^{i}_{lj} \left(C^{-T}\right)^{\hphantom{\alpha}j}_{\sigma}
%	\right) (e_i)^a ({\rm d}x^\rho)_b  ({\rm d}x^\sigma)_c  .
%\end{align*}
...
\begin{align*}
    ...
    + \bigl( {\rm d}_a (e^i)_b \bigr) (e_j)^a (e_k)^b  {\qquad  \Rightarrow }   \\
%    T_{ab}^c (e^i)_c  & = \Gamma_{kj}^i (e^j)_a(e^k)_b -\Gamma_{jk}^i(e^j)_a(e^k)_b
%    +  {\rm d}_a (e^i)_b    {\qquad  \Rightarrow }   \\
    T_{ab}^c (e^i)_c  & = (\omega _{\hphantom{a} k}^i)_a(e^k)_b 
    -(e^j)_a(\omega _{\hphantom{a} j}^i)_b +  {\rm d}_a (e^i)_b .
\end{align*}
由此，可得
...
\end{equation}
注意${e_i} ( {\Gamma _{kj}^n} )$是切矢量场${e_i}$作用在标量函数场${\Gamma _{kj}^n}$上，
省略了抽象指标．依照式\eqref{chdf:eqn_d1form-value}，上式最后一项可表示为
\begin{align}
...
\begin{align}
    ...
    {\quad  \Leftrightarrow } \label{chccr:eqn_Cartan-Structure-II-comp} \\
...
\begin{align}
    ...
    (\Omega_{\hphantom{i} j} ^{i})_{ab} = &\frac{1}{2}R_{\hphantom{i} jkl}^i
    (e^k)_a \wedge (e^l)_b = R_{\hphantom{i} jkl}^i (e^k)_a (e^l)_b . \label{chccr:eqn_Cform-bak}
\end{align}
\CJKunderwave{则两个Cartan结构方程可以表示成如下常用形式}：
\begin{subequations}\label{chccr:eqn_CST}
\begin{align}
    {\rm d}_a (e^i)_b =&(e^j)_a \wedge (\omega _{\hphantom{i} j}^i)_b +(\Theta ^{i})_{ab}, \label{chccr:eqn_CST-I} \\
    {\rm d}_a ( \omega _{\hphantom{i} j} ^i  )_b  =&  ( \omega _{\hphantom{i} j}^l )_a
    \wedge ( \omega _{\hphantom{i} l}^i )_b + (\Omega_{\hphantom{i} j} ^{i})_{ab} . \label{chccr:eqn_CST-II}
\end{align}
\end{subequations}
...
\end{theorem}
\begin{proof}
    对式\eqref{chccr:eqn_CST-I}取外微分，有（见式\eqref{chdf:eqn_dwm}）
    \begin{align*}
        ...
        +{\rm d}_c (\Theta ^{i})_{ab} \quad  { \Rightarrow} \\
    ...
    \end{align*} 
    这便是第一式．再对\eqref{chccr:eqn_CST-II}取外微分，有
    \begin{align*}
        ...
        + {\rm d}_c (\Omega_{\hphantom{a} j} ^{i})_{ab} \quad  { \Rightarrow} \\
...
\subsection{平直与弯曲}\label{chccr:sec_flat-curved}


本小节内容在无挠仿射空间$(M,\nabla_a)$的局部开集$(U;x^i)$上讨论．

由测地线定义\ref{chccr:def_geodesic}以及式\eqref{chccr:eqn_px-formula}可知$U$中测地线方程为：
\begin{equation}\label{chccr:eqn_geodesic}
...
\end{equation}
若$U$中所有测地线能表述成直线形式
\begin{equation}\label{chccr:eqn_gd-line}
	x^i(t) = a^i t + b^i, \quad a^i,b^i\in \mathbb{R}, \quad 1 \leqslant i \leqslant m , \quad t\in (-\delta,\delta)
\end{equation}
那么我们可以认为$U$是平直的，这正是欧式空间平直属性的推广描述．
我们将上式带入式\eqref{chccr:eqn_geodesic}，并利用$\Gamma_{jk}^i(t)=\Gamma_{kj}^i(t)$可以得到：
\begin{equation}
	\Gamma_{jk}^i(t) =0 , \qquad t\in (-\delta,\delta), \quad 1\leqslant i,j,k \leqslant m.
\end{equation}
这也就是说在局部坐标系$(U;x^i)$中仿射联络系数都为零．
反之若存在局部坐标卡$(U;x^i)$使得其中仿射联络系数都为零，
那么由式\eqref{chccr:eqn_geodesic}可知：
$U$中测地线必然可以表示成上述直线形式\eqref{chccr:eqn_gd-line}．两者是相互等价的描述．

若$U$平直（测地线全为直线形式\eqref{chccr:eqn_gd-line}），
则由式\eqref{chccr:eqn_Riemannian13-component}可知：
\begin{equation}
	R^i_{jkl} = 0.
\end{equation}
这便论证了：若$M$的局部开集$U$平直（测地线全为直线形式\eqref{chccr:eqn_gd-line}），
那么$U$上黎曼曲率恒为零．此命题的逆命题也成立，即有

\begin{theorem}\label{chccr:thm_local-EuclideanSpace}
	设有无挠仿射空间$(M,\nabla_a)$，$M$为平直（测地线全为直线形式）
	的充分必要条件是：其切标架丛（或切丛）上黎曼曲率张量为零．
\end{theorem}

下面证明“$\Leftarrow$”．因联络无挠，故方程\eqref{chccr:eqn_CST}变为
\begin{subequations}\label{chccr:eqn_CST-Rieman-Conn}
	\begin{align}
		{\rm d}_a (e^\mu)_b &=(e^\nu)_a \wedge (\omega _{\hphantom{\mu} \nu}^\mu)_b , \label{chccr:eqn_CST-Rieman-Conn-I} \\
		{\rm d}_a ( \omega _{\hphantom{\mu} \nu} ^\mu  )_b  &=  ( \omega _{\hphantom{\mu} \nu}^\sigma )_a
		\wedge ( \omega _{\hphantom{\mu} \sigma}^\mu )_b + (\Omega_{\hphantom{\mu} \nu} ^{\mu})_{ab}. \label{chccr:eqn_CST-Rieman-Conn-II}
	\end{align}
\end{subequations}
...
新的联络系数及联络微分型式场为式\eqref{chccr:eqn_connection-form-tilde}：
\begin{equation}
	(\tilde{e}_i)^b \nabla _b (\tilde{e}_j)^a  =
...
\end{equation}
我们的目的是在点$p\in U$附近寻找一组适当的光滑函数$A_{\hphantom{\mu} k}^{\sigma}\in C^\infty(U)$使
得$\det(A_{\hphantom{\mu} k}^{\sigma})\neq 0$，
并且在新标架场$(\tilde{e}_i)^a$下\CJKunderwave{新联络型式场}$(\tilde{\omega} _{\hphantom{k} j} ^{k})_a $恒为零，
也就是新的联络系数$\tilde{\Gamma}^{k}_{j i}$恒为零；
由上面的式子\eqref{chccr:eqn_connection-form-tilde}可
...
\begin{equation}\label{chccr:eqn_tmp-pi}
...
\begin{align*}
	...
	\xlongequal[\ref{chccr:eqn_tmp-pi}]{\ref{chccr:eqn_CST-Rieman-Conn-II}}&
...
\end{align*}
于是Pfaff方程组\eqref{chccr:eqn_tmp-pi}满足Frobenius条件\eqref{chdf:eqn_frobenius-Diff-Form}；
由Frobenius定理\ref{chdf:thm_frobenius-Diff-Form}可知此方程组完全可积．
也就是存在新的标架场$(\tilde{e}_i)^a$，其上的仿射联络系数都为零
（$(\tilde{\omega} _{\hphantom{\mu} \nu}^\mu)_b$恒为零），
将其带入Cartan第一结构方程\eqref{chccr:eqn_CST-Rieman-Conn-I}，可以得到：
\begin{equation}
...
在这个新标架中$U$上测地线全为直线形式\eqref{chccr:eqn_gd-line}（即$U$是平直的）．证毕．


\index[physwords]{平直}  \index[physwords]{弯曲}

在定义度量后，还有后续描述，见\S\ref{chrg:sec_localEuclideanSpace}．
有了上述论证，可定义：
\begin{definition}\label{chccr:def_flat-curved}
	设仿射空间$M$上有无挠联络$\nabla_a$．若其切丛上的黎曼曲率恒为零，
	则称$M$为{\heiti 平直的}（或{\heiti 平坦的}，flat）；  否则称$M$为{\heiti 弯曲的}（curved）．
\end{definition}
...
\begin{exercise}
	由式\eqref{chccr:eqn_1form-transformation}导出式\eqref{chccr:eqn_GAMMA-trans}．
\end{exercise}


\begin{exercise}
	由式\eqref{chccr:eqn_GAMMA-trans}导出式\eqref{chccr:eqn_Exchange-Christoffel}．
\end{exercise}

\begin{exercise}
	证明式\eqref{chccr:eqn_tetrad-postulate-b}．
	试着讨论式\eqref{chccr:eqn_GAMMA-trans}与式\eqref{chccr:eqn_tetrad-postulate}的关系．
\end{exercise}


%\begin{exercise}
%	先深入理解例题\ref{chccr:exam_soldering}（它与度规无关、与挠率无关），
%	然后审视本章给出的仿射联络框架体系是否存在逻辑缺陷．
%\end{exercise}
