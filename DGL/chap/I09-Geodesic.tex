...
本章最后介绍了Jacobi切矢量场，这是描述引力论中测地偏离方程的工具．
...
式\eqref{chgd:eqn_geodesic-formula}中的第二项（即$\Gamma_{jk}^i(t)
\frac{{\rm d}x^j(t)}{{\rm d} t} \frac{{\rm d}x^k(t)}{{\rm d} t}$）中
存在“$\frac{{\rm d}x^j(t)}{{\rm d} t} \frac{{\rm d}x^k(t)}{{\rm d} t}$”，
故在缩并过程对指标“$jk$”而言是对称的；
由此可以看出：联络系数$\Gamma_{jk}^i(t)$的反对称部分（若存在）在测地线中根本不起作用，起作用的只有对称部分．
...
上述初始条件的唯一解；也就是说开集$U$内存在\CJKunderwave{唯一一条测地线}$\gamma(t;p,v^a)$满足
...
\begin{definition}
    设$\gamma(t)$是广义黎曼流形$(M,g)$上的一条测地线，
    用$T^a=(\frac{\partial}{\partial t})^a$表示其切线切矢量；
...
\subsection{仿射变换}
设有映射$t(u)$将\CJKunderwave{正则}测地线$\gamma(t)$实参数$t$变成了$u$（这称为重参数化），
...
由此式可以看出当变换$t(u)$是\CJKunderwave{仿射变换}时，即
...
这显然是仿射变换，变换后的曲线$\tilde{\gamma}(u)\equiv\gamma(\lambda u;p,v^a)$仍是测地线，并且有
\begin{equation}
...
我们可借助流形自身的某些对称性来寻找测地线，为此先介绍一个命题．
...
\begin{proof}
    ...
    \begin{equation*}
        \sigma\circ\beta(s_0)=\beta(s_0)=\gamma(s_0),\quad
...
\begin{example}
	共形映射下的测地线．
\end{example}
设光滑流形$M$上有度规场$g_{ab}$、$\tilde{g}_{ab}$，
且两者满足式\eqref{chrg:eqn_conformal-transformation}（$\tilde{g}_{ab} = \Omega^2 g_{ab}$）．
$\gamma(t)$用度规$g_{ab}$衡量是测地线，$t$是仿射参数，
则其切线切矢量$T^a = (\partial / \partial t)^a$满足$T^a \nabla_a T^b=0$．
由式\eqref{chrg:eqn_conformal-Xi-nabla}、
\eqref{chrg:eqn_conformal-X-D}（$\widetilde{\nabla}_a T^b =\nabla_a T^b+ \Xi_{ca}^b T^c$）可知：
\begin{align}
	T^a \widetilde{\nabla}_a T^b  =& T^a \nabla_a T^b+\Xi_{ca}^b T^a T^c = \Xi_{ca}^b T^a T^c \notag \\
%	=& T^a T^c \delta_{a}^b \nabla_{c} \ln \Omega + T^a T^c \delta_{c}^b \nabla_{a} \ln \Omega 
%	-T^a T^c g_{a c} g^{b e} \nabla_e \ln \Omega \notag \\
	=&2 T^b T^c \nabla_c \ln \Omega-\left(g_{ac} T^a T^c\right) g^{b e} \nabla_e \ln \Omega.
	\label{chgd:eqn_TNTgdT}
\end{align}

由式\eqref{chgd:eqn_TNTgdT}可知，当$\Omega$为常数时，$T^a \widetilde{\nabla}_a T^b =0$；
故在度规$\tilde{g}_{ab}$衡量下$\gamma(t)$是测地线，$t$是仿射参数．下面讨论$\Omega$不是常数情形．

设$\gamma(t)$是零模测地线（用${g}_{ab}$衡量），则$g_{ac} T^a T^c=0$；
故式\eqref{chgd:eqn_TNTgdT}变为$T^a \widetilde{\nabla}_a T^b=(2 T^c \nabla_c \ln \Omega) T^b$．
上式表明，用$\tilde{g}_{ab}$衡量，$\gamma(t)$是非仿射参数化的测地线，
我们可以选择适当参数$\tilde{t}$使其成为仿射参数化的测地线．
寻找仿射参数过程留给读者当练习．

当$\gamma(t)$是类时或类空测地线时（用${g}_{ab}$衡量），
由式\eqref{chgd:eqn_TNTgdT}可知：
用$\tilde{g}_{ab}$衡量时，$\gamma(t)$一般不是测地线．
\qed
...
我们先引入指数映射定义，然后借用指数映射介绍黎曼法坐标
系（Riemanian Normal Coordinates，也翻译为{\kaishu 黎曼简正坐标系}）．
...
\end{equation}
指数映射\eqref{chgd:eqn_geodesic-exp}只是借助测地线将$T_pM$中矢量映射到流形$M$中，
此时参数$t\equiv 1$；
式\eqref{chgd:eqn_geodesic-exptv}则是测地线本身，参数$t$是变量．


\begin{theorem}\label{chgd:thm_exp-homeomorphism}
    设有$m$维广义黎曼流形$(M,g)$．$\forall p \in M$，在$T_p M$中存在原点的开邻域$V$，
    使得指数映射$\exp_{p}$是从$V$到$M$中$U=\exp_{p}(V)$上的微分同胚．
\end{theorem}
\begin{proof}
...
上面用抽象的映射定义了法坐标系，下面我们用分量语言再次看一下此问题，
并且给出法坐标系下诸多几何量的展开式；这需要分成几部分来叙述．
...
假设\eqref{chgd:eqn_geodesic-expand}式是收敛的；即便如此，$s$也不能太大，
因为在离$p$点非常远的地方（比如$q\in M$）测地线可能相交，这相当于坐标线相交于两点$p$和$q$．
%%出现多值性，需要增加一个坐标卡来覆盖这个流形了．



继续式\eqref{chgd:eqn_geodesic-expand}的讨论．
黎曼本人引入了坐标\eqref{chgd:eqn_normal-coord}和测地线方程\eqref{chgd:eqn_normal-coord-gedU}式，
...
\begin{definition}\label{chgd:def_arc-variation}
    设$C:[r_1,r_2]\to M$是广义黎曼流形$(M,g)$中的一条光滑曲线段，若有$\epsilon > 0$以及
    光滑映射$\sigma:[r_1,r_2]\times (-\epsilon, \epsilon) \to M$使
    得$\sigma(s,0)=C(s),\ \forall s\in [r_1,r_2]$，则称映射$\sigma$是
    ...
    若固定$s$，由$\phi_s(\tau)\equiv \sigma(s,\tau)$定义
...
$T^a(s)$是沿曲线$C(s)$定义的光滑切矢量场，称为变分$\sigma$的{\heiti 变分切矢量场}．
...
\end{equation}
很明显$L(\tau)$是一个标量函数，对其取$\tau$的导数（需把联络理解成诱导联络）．
\begin{align}
...
\end{theorem}
\begin{proof}
    $T_pM$是线性空间，自然可以看成流形；$v\in T_pM$是这个流形上一点，
    $T_v(T_p M)$是指流形$T_pM$中点$v$处的切空间．
    定理中已假设$\exp_{p}$在$v^a$处有定义；我们任取$w^a \in T_pM$，
    存在充分小的$\epsilon >0$使得下面的映射有定义：
    ...
    \begin{equation}
        ...
        = (\exp_{p})_{*s v} v^a .
    \end{equation}
    由于式\eqref{chgd:eqn_gltmp01}是指数映射，所以得到的曲线是测地线；
    当$s$固定、$\tau$变化时，将其记为$\alpha(\tau;q,s w^a)$，其中$q=\exp_p(s v^a)$．
    ...
    \end{align*}
    因我们采用的是弧长参数，故$|v|=1$．
    结合上式与式\eqref{chgd:eqn_gltmp03}便可得\eqref{chgd:eqn_Gauss-lemma}式．
    上式中的$v_aw^a$是切空间$T_p M$上的内积，在$p$点取值．
\end{proof}

\begin{lemma}\label{chgd:thm_Gauss-lemma-old}
    $\exp_{p}$把$T_pM$中正交切矢量$v^a$和$w^a$映射到$M$中正交切矢量．
\end{lemma}
\begin{proof}
...
所以通常把这样的测地线称为{\heiti 径向测地线}． 
\end{remark}

\index[physwords]{测地球} \index[physwords]{测地球面} \index[physwords]{径向测地线}


\index[physwords]{Jacobi场} \index[physwords]{Jacobi切矢量场}
...
\begin{equation}
    ...
    =R^{a}_{\hphantom{a} bcd} \tilde{S}^c\tilde{T}^d \tilde{S}^b .
\end{equation}
...
\begin{equation}\label{chgd:eqn_Jacobi}
    ...
    R^{a}_{\hphantom{a} bcd} {S}^c{J}^d {S}^b.
\end{equation}
...
\begin{definition}\label{chgd:def_Jacobi-field}
    ...
    沿$C(s)$定义的一个光滑矢量场．若$J^a(s)$满足式\eqref{chgd:eqn_Jacobi}，
    则称$J^a(s)$是沿测地线$C(s)$定义的一个{\bfseries \heiti Jacobi切矢量场}，
...
\begin{proposition}\label{chgd:thm_vvJ}
    ...
    则$C(s)$测地变分的变分矢量场是沿测地线$C(s)$的Jacobi切矢量场．
\end{proposition}

此定理之逆也成立，见\ref{chgd:thm_vvJ-inv}．
$C(s)$的变分矢量场的方向一般不平行于$C(s)$的切线切矢量（称为纵向），
直观感觉它沿横向拉着$C(s)$偏离原来的位置，
...
\begin{align}
    ...
    &\nabla_{\frac{\partial}{\partial s}} \nabla_{\frac{\partial}{\partial s}} J^a(s)
     -R^{a}_{\hphantom{a} bcd} {S}^c{J}^d {S}^b = \left( \frac{\partial^2 J^i(s)}{\partial s^2}
     -R^i_{\hphantom{a} 11j}\bigl(C(s)\bigr) J^j(s) \right)(e_i)^a .
\end{align}
...
\begin{proposition}\label{chgd:thm_Jacobi-unique}
    设$C:[r_1,r_2]\to M$是广义黎曼流形$(M,g)$上一条测地线，
    那么$\forall v^a,w^a\in T_{C(r_1)}M$，
    存在唯一的一个沿$C(s)$的Jacobi切矢量场$J^a(s)$满
    足$J^a(r_1)=v^a$、$\frac{{\rm d}J^a}{{\rm d}s}|_{r_1}=w^a$．
\end{proposition}
\begin{proof}
    ...
    \begin{equation}\label{chgd:eqn_Jacobi-componet-1stOrder}
        ...
        \end{cases} .
    \end{equation}
    依据常微分方程组解存在唯一性定理可知上述方程式解存在、唯一．
\end{proof}

我们知道$T_{C(r_1)}M$是$m$维的线性空间，
而Jacobi场方程\eqref{chgd:eqn_Jacobi-componet-1stOrder}的解
由初始条件$(v^a,w^a)\in T_{C(r_1)}M\oplus T_{C(r_1)}M$唯一确定；
由上述命题可知这个关系是一个\CJKunderwave{双射}．
因Jacobi方程是线性的，所有解的集合构成线性空间，故有如下命题：
...
\begin{proposition}\label{chgd:thm_vvJ-inv}
    设$C:[r_1,r_2]\to M$是$m$维广义黎曼流形$(M,g)$上的测地线，
    $J^a(s)$是一沿测地线$C(s)$的Jacobi切矢量场．
    则$J^a(s)$必是$C(s)$的某个测地变分曲线的变分切矢量场．
\end{proposition}
\begin{proof}
    为了叙述简单，令$r_1=0$，并记$p=C(0)$．在流形$M$上任取一条光滑曲线$\beta(\tau)$，
    其中$\tau\in (-\epsilon,\epsilon)$，使得
    \begin{equation}\label{chgd:eqn_tmpinit}
        \beta(0)=p,\quad \left.\left(\frac{\rm{d}}{{\rm d} \tau}\right)^a \right|_{\beta(0)}=v^a;
        \qquad \text{对任意固定的}\  v^a \in T_pM .
    \end{equation}
    再取对于任意固定的$w^a \in T_pM$，
    把$w^a$和$S^a(0)=(\frac{\rm{d}}{{\rm d} s})^a|_{C(0)}$（测地线
    端点的切线切矢量）沿曲线$\beta(\tau)$作平行移动，
    得到两个沿$\beta(\tau)$平行的矢量场$W^a(\tau)$和$S^a(\tau)$，它们满足
    ...
    \begin{equation}\label{chgd:eqn_sigmaJT}
        \sigma(s,\tau)= \exp_{\beta(\tau)}\Bigl[s\bigl(S^a(\tau)+\tau W^a(\tau)\bigr)\Bigr],
    ...
    \begin{equation*}
        ...
        =\bigl(\exp_{\beta(\tau)}\bigr)_{*0} \bigl(S^a(\tau)+ \tau W^a(\tau) \bigr)
        =S^a(\tau)+ \tau W^a(\tau) .
    \end{equation*}
    ...
    \begin{equation*}
        ...
        = \left.\left(\frac{\partial}{\partial \tau}\right)^a  \right|_{\tau=0}\left(\beta(\tau)\right) = v^a.
    \end{equation*}
...
\begin{corollary}\label{chgd:thm_Jexp}
    设$C:[0,b]\to M$是$m$维广义黎曼流形$(M,g)$上测地线；$J^a(s)$是一沿$C(s)$的Jacobi场，
    它满足$J^a(0)=0$和$J^a(s)$是测地变分（式中$p\equiv C(0)$）
...
\end{corollary}
\begin{proof}
    式\eqref{chgd:eqn_Jexp-1}只不过是\eqref{chgd:eqn_sigmaJT}的变种，其中$w^a$是任取的．
    经计算有$\frac{{\rm d}J^a(0)}{{\rm d}s} =(\exp_{p})_{* 0} ( w^a)  =w^a$；
...
\begin{proposition}
	法Jacobi场$J^a_{\perp}=J^a \pm S^a (J^b S_b) $符合Jacobi方程\eqref{chgd:eqn_Jacobi}．
\end{proposition}
\begin{proof}
	如果$J^b$与$S^b$正交（即$J^b S_b=0$），
	那么$J^a_{\perp}=J^a$自然符合Jacobi方程\eqref{chgd:eqn_Jacobi}．
	下面假设它们不正交．因度规不同，$C(s)$的单位切线切矢量$S^b$的归一化可取$+1$或$-1$；
	当$S^b S_b =+1$时，令$J^a_{\perp}=J^a - S^a (J^b S_b) $；
	当$S^b S_b =-1$时，令$J^a_{\perp}=J^a + S^a (J^b S_b) $．
	在这样约定下，必然有
	\begin{align*}
		S_a J^a_{\perp}=S_a J^a \pm S_aS^a (J^b S_b) = S_a J^a - J^b S_b = 0.
	\end{align*}
	即$J^a_\perp$与$S^a$正交．

    因$S^a$是测地线$C(s)$的切线切矢量，
    故必有$\nabla_{\frac{\partial}{\partial s}}S^a=0$．
    直接计算，有
    \begin{align*}
    	\nabla_{\frac{\partial}{\partial s}} \nabla_{\frac{\partial}{\partial s}} J^a_{\perp}
    	=& \nabla_{\frac{\partial}{\partial s}} \nabla_{\frac{\partial}{\partial s}} \left( J^a \pm S^a (J^b S_b)\right)
    	= R^{a}_{\hphantom{a} bcd} {S}^b {S}^c{J}^d
    	\pm S^a S_b \nabla_{\frac{\partial}{\partial s}}\nabla_{\frac{\partial}{\partial s}} J^b \\
    	\overset{*}{=}& R^{a}_{\hphantom{a} bcd} {S}^b {S}^c{J}^d
    	\pm \cancel{S^a \left(R_{[be]cd} S^{(b} S^{e)} \right) {S}^c{J}^d} \\
    	\overset{\bullet}{=}& R^{a}_{\hphantom{a} bcd} {S}^b {S}^c {J}^d \pm 
    	R^{a}_{\hphantom{a} bcd} {S}^b {S}^c \left( S^d (J^e S_e) \right) .
    \end{align*}
    上式中的$\overset{*}{=}$步中，由黎曼曲率反对称性和$S^{(b} S^{e)}$的对称性可知它们的缩并为零．
    在$\overset{\bullet}{=}$步中，同样因黎曼曲率反对称性和$S^{(c} S^{d)}$的对称性可知这个附加项恒为零．
    但增加这个恒零项后，便有
    \begin{align*}
    	\nabla_{\frac{\partial}{\partial s}} \nabla_{\frac{\partial}{\partial s}} J^a_{\perp}
    	= R^{a}_{\hphantom{a} bcd} {S}^b {S}^c \left( {J}^d \pm S^d (J^e S_e) \right)
    	= R^{a}_{\hphantom{a} bcd} {S}^b {S}^c {J}^d_\perp.
    \end{align*}
    证毕．
\end{proof}
...
\begin{proposition}\label{chgd:thm_normal-Jacobi}
    ...
    测地线$C(s)$的Jacobi场，则$J^a(s)$与测地线$C(s)$处处正交的充要条件是
...
\begin{proposition}
    设$C:[r_1,r_2]\to M$是$m$维广义黎曼流形$(M,g)$上的一条测地线，
    设$J^a(s)$沿$C(s)$的Jacobi切矢量场．若存在两个不同
...
\begin{proposition}
    ...
    用$\mathfrak{J}^{\bot}(C)$表示$M$上沿测地线$C(s)$法Jacobi场集合，
    则$\mathfrak{J}^{\bot}(C)$是一个$2m-2$维线性空间．
\end{proposition}
\begin{proof}
    由命题\ref{chgd:thm_Jacobi-2mD}可知$M$上全体沿测地线$C(s)$的Jacobi场集合
    是$2m$维线性空间．从命题\ref{chgd:thm_Jacobi-2mD}证明过程知，
    初始条件$(v^a,w^a)\in T_{C(r_1)}M\oplus T_{C(r_1)}M$使下式
    \begin{equation}
...
\subsection{常曲率空间}
本节讨论常曲率空间（见\S \ref{chrg:sec_const-curvature}）的Jacobi场．
...
\begin{align*}
    ...
    &\Rightarrow\quad R_{i11j} = -c (\eta_{i1}\eta_{1j}- \eta_{ij}\eta_{11} )
       = c \eta_{ij}\eta_{11}   .
\end{align*}  %%\setlength{\mathindent}{2em}
...
恒为零，即$J_1(s)\equiv 0$；故上式中的指标$i$从$2$记起；其通解是
...
%$\nabla_a$是其Levi-Civita联络；存在光滑\CJKunderwave{浸入}映射$\phi:N\to M$．
