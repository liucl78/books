...
\S\ref{chfb:sec_G-connection}内容需要一些李群、李代数概念（见第\ref{chlg}章）．
...
\begin{definition}
    ...
    $\forall b\in B$，空间$\pi^{-1}(b)\in E$称为植于$b$点的{\heiti 纤维}（fibre）．
\end{definition}
...
粘合规则是\eqref{chfb:eqn_gbayy}式．

由下面命题可见：矢量丛 $\pi: E \to M$在直观上可以想象为“植”在流形$M$上的一“簇”矢量空间；
...
\begin{proof}
    ...
    本定理说明过渡函数族是构造矢量丛的核心．
%    可参考文献\parencite{chen-li-2026-2ed-v2}203页定理2.2；
...
设微分流形$M$有微分结构$\mathscr{A}=\{(U_\alpha,\varphi_\alpha;x^i)\mid\alpha \in I \}$，令
\begin{equation*}
...
因$T^*_pM$是$m$维的，而点$p\in U_\alpha$（$U_\alpha$是$m$维）又是可变动的，
所以可知$\pi^{-1} (U_\alpha)$是$2m$维；进而$T^*M$也是$2m$维的．


设$(x_p^1,\cdots,x_p^m)$是$p$点在映射$\varphi_\alpha$作用下的坐标；
矢量$\omega \in \pi^{-1} (U_\alpha)$可以在$p$点自然余切
基矢$\left.{\rm d}x^i \right | _p$下展开，
其分量是$(\omega_1,\cdots,\omega_m)$．$\forall \alpha \in I$，
定义局部平凡化$\psi_\alpha: \pi^{-1} (U_\alpha) \to U_\alpha \times \mathbb{R}^m 
...
若在$V$和$V^*$中分别取彼此对偶的基底，$V$中元素$y$记作列矢量，$V^*$中元素$\lambda$记作行矢量，
则 $V$ 和 $V^*$ 的配合表现为矩阵乘法（注$\langle y, \lambda\rangle=\langle\lambda,y\rangle$）：
\begin{equation*}
...
\section{矢量丛联络}\label{chfb:sec_connection}
...
\begin{definition}\label{chfb:def_vb-conncection}
    设$(E,M,\pi)$是流形$M$上的一个矢量丛，$\Gamma(E)$是该矢量丛光滑截面的集合．
    矢量丛 $E$上的一个 {\heiti 联络} $\nabla$是指存在映射
    $\nabla: \Gamma(E) \times \mathfrak{X}(M) \rightarrow \Gamma(E)$．
...
的$\nabla$是$M$上切矢场的联络\ref{chccr:def_connection}．
...
\end{equation}
通过上面操作得到了一个映射$(\alpha^*, X) \to \nabla_X^* \alpha^*$，
容易看出映射$\nabla^*$是从笛卡尔积$\Gamma(E^*)\times \mathfrak{X}(M)$到
对偶丛截面$\Gamma (E^*)$的一个联络（请读者验证式\eqref{chfb:eqn_dual-con}中的$\nabla_X^*$满足
定义\ref{chfb:def_vb-conncection}）；称$\nabla^*$为与矢量丛联络 $\nabla$相互对偶的对偶丛联络．
\qed
...
设$M$、$N$是光滑流形，维数分别为$m$、$n$；$M \xrightarrow{\phi} N$是光滑浸入映射．命
...
$\forall X \in T_p M$，构造{\heiti 拉回丛上的诱导联络}为：
...
于是，得到映射 $\nabla: \Gamma\left(\phi^* T N\right)\times \mathfrak{X}(M) 
...
\section{矢量丛曲率}\label{chfb:sec_curvature}
...
这再次说明式\eqref{chfb:eqn_wftmat}（也就是
式\eqref{chfb:eqn_wform-trans}）是微分几何中非常基本的重要公式．
...
\begin{definition}\label{chfb:def_Curvature}
...
如用抽象指标表示，则有$(\Omega_{\hphantom{\alpha}\alpha}^\beta)_{ab}$，
需要用$M$上切矢量去缩并两个抽象指标“${}_{ab}$”，不能用丛空间的矢量去缩并；可参见\S\ref{chfb:sec_tvpf}．
由于曲率型式$\Omega=\left(\Omega_{\hphantom{\alpha}\alpha}^\beta\right)$在局部标架场改变时
按\eqref{chfb:eqn_TATA}式变换，故$\Omega_{\hphantom{\alpha}\alpha}^\beta(X, Y)$是
...
\begin{definition}\label{chfb:def_fibre-bundles}
    ...
    {\bfseries (1)} $M$有一个开覆盖$\left\{U_\alpha \mid \alpha \in I\right\}$，
...
\begin{theorem}\label{chfb:thm_fb-existence}
    设 $M$ 是一个 $m$ 维光滑流形，$\left\{U_\alpha\mid \alpha \in I\right\}$是 $M$ 的一个开覆盖；
...
\begin{proof}
    可参考文献\parencite{Husemoller-1994}第五章定理3.2；
    或\parencite[\S 10.2]{chen-li-2026-2ed-v2}定理2.2以及\S 10.3定理3.1．
...
以上内容可参阅\parencite{chen-li-2026-2ed-v2}第十章或\parencite{spivak-dif-2}第八章．
...
本节取自\parencite{chen-li-2026-2ed-v2}第10.2节．
...
设矢量丛$(E,M,\pi)$的局部平凡化结构是$\left\{\left(U_{\alpha}, \psi_{\alpha}\right) \mid  \alpha \in I\right\}$，
...
对前面的叙述做一个小结：
设$\left\{\left(U_{\alpha}, \psi_{\alpha}\right) \mid \alpha \in I\right\}$是矢量丛$\pi: E \rightarrow M$的
...
的$\mathfrak{g l}(q)$-值的1次外微分式，其中$\mathfrak{gl}(q)$是一般线性群$GL(q)$的李代数．
\end{theorem}
...
这超出了本书的范畴，可参考文献\parencite{chen-li-2026-2ed-v2}第十章或类似文献．
...
\begin{remark}
从上面得到$G$型联络的过程来看，貌似我们只需将矢量丛的结构群$GL(q)$直接限定在其子群$G$上即可．
实际情况要复杂的多．首先，需要验证子群$G$满足{\kaishu 约化定理}（\parencite[\S 10.3]{chen-li-2026-2ed-v2}定理3.5）；
其次，再去探讨联络的约化．
\end{remark}
...
\end{align*}\end{small} 
若\CJKunderwave{仿射联络满足无挠条件}（$\oversetmy{x}{\Gamma}^l_{kj}=\oversetmy{x}{\Gamma}^l_{jk}$），
则可得Bianchi微分恒等式：
...
在经典场论中，需将$M$选为平直的四维闵氏流形$(\mathbb{R}^4_1,\eta)$；
...
\begin{remark}
	...
	从纯数学角度来看这是不严谨的．因笔者只想初步阐述纤维丛与规范场关系，
	故这种写法是划算的．
\end{remark}

\begin{remark}\label{chfb:rem_qft}
	笔者初级观点：若物理学者不了解纤维丛，并不影响理解平直时空的Yang--Mills理论．
...
\index[physwords]{张量值型式}

\section{张量值型式}\label{chfb:sec_tvpf}

本小节采用抽象指标记号，并与之前几节部分公式作比较．

我们采用不那么严谨的数学语言来叙述一下纤维丛中张量值$p$-型式概念（tensor valued $p$-form）．
流形$M$上的具有{\bfseries\heiti $\binom{r}{s}$型的张量值$p$-（外）型式}是指：
\begin{equation*}
	\underbrace{\mathfrak{X}(M)\times \cdots \times \mathfrak{X}(M)}
	_{\text{共}p\text{个}} \overset{T}{\longrightarrow} 
	\mathcal{T}^r_s(V)\text{张量分量};	\quad \text{记为}\ 
	\left(T^{i_1 \cdots i_r}_{\hphantom{i_1 \cdots i_r}
		k_1\cdots k_s}\right)_{a_1 \cdots a_p}.
\end{equation*}
其中映射$T$是一个全反对称的$p$重线性映射；
空间$V$可以是流形$M$，也可以是$\mathbb{R}^q$（或$\mathbb{C}^q$）；
指标$\{i,k\}$取值范围跟随空间$V$的维数；
抽象指标$\{a_1\cdots a_p\}$表示此型式属于流形$M$余切丛，
具体分量指标取值范围是$M$的维数．

\begin{example}
	$\binom{0}{0}$型张量值$p$-型式就是通常的$p$-型式，比如$M$上余切矢量$\mu_a$．
\end{example}

\begin{example}
	$M$上余切标架场$(e^i)_a$（$(e_i)_a$）为$\binom{1}{0}$（$\binom{0}{1}$）型张量值$1$-型式．
\end{example}

\begin{example}
	\S\ref{chfb:sec_curvature}中的曲率型式$(\Omega^\mu_{\hphantom{\mu}\nu})_{ab}$为
	流形$M$上的$\binom{1}{1}$型张量值$2$-型式．
\end{example}

\begin{example}
	因联络系数不是张量，故$M$上的联络型式场$(\omega^i_{\hphantom{i}j})_a$不是张量值型式．
	由于它是方阵，所以一般情形下称之为$\mathfrak{gl}(q)$-外型式，参见式\eqref{chfb:eqn_lt}．
\end{example}

我们定义作用在$\binom{r}{s}$型的张量值$p$-型式上{\heiti 外协变微分}为：
\begin{align}
	\mathcal{D}_b \left(T^{i_1 \cdots i_r}_{k_1\cdots k_s}\right)
	_{a_1 \cdots a_p} \overset{def}{=} & 
	{\rm d}_b \left(T^{i_1 \cdots i_r}_{\hphantom{i_1 \cdots i_r}k_1\cdots k_s}
		\right)_{a_1 \cdots a_p}  \quad\qquad {\rm d}_b \, \text{为外微分算子} \notag \\
	&+ \sum_{n=1}^{r} (\omega^{i_n}_{\hphantom{i_n}l})_b \wedge
	\left(T^{i_1 \cdots i_{n-1} l i_{n+1} \cdots i_r}
	_{\hphantom{i_1 \cdots i_{n-1} l i_{n+1} \cdots i_r}k_1\cdots k_s}\right)_{a_1 \cdots a_p} \notag  \\
	&-\sum_{n=1}^{s} (\omega^{l}_{\hphantom{l}k_n})_b \wedge
	\left(T^{i_1 \cdots i_r}_{\hphantom{i_1 \cdots i_r}
		k_1\cdots k_{n-1} l k_{n+1} k_s}\right)_{a_1 \cdots a_p} . \label{chfb:eqn_Dpform}
\end{align}
从式\eqref{chfb:eqn_Dpform}右端的构造可以看出：$\mathcal{D}_b \left(T^{i_1 \cdots i_r}
_{\hphantom{i_1 \cdots i_r}k_1\cdots k_s}\right)_{a_1 \cdots a_p}$是$\binom{r}{s}$型张量值的$(p+1)$-型式．
严谨证明可参阅主纤维丛联络理论．


\begin{example}
	对$\binom{r}{s}$型张量值的$0$-型式来说，$\mathcal{D}$就是矢量丛上的联络$\nabla$．
\end{example}

\begin{example}
	对$\binom{0}{0}$型张量值的$p$-型式来说，$\mathcal{D}$就是外微分算子${\rm d}$．
\end{example}

	
\begin{example}
Cartan第一结构方程\eqref{chccr:eqn_CST-I}可改写为：
\begin{align}
	\mathcal{D}_a(e^i)_b = {\rm d}_a (e^i)_b 
	+(\omega _{\hphantom{i} j}^i)_a \wedge (e^j)_b 
	= (\Theta ^{i})_{ab}.
\end{align}
\end{example}

\begin{example}
第一Bianchi恒等式\eqref{chccr:eqn_Bianchi-CI}可改写为：
\begin{align}
	\mathcal{D}_c (\Theta ^{i})_{ab} =
		{\rm d}_c (\Theta ^{i})_{ab}  + (\omega _{\hphantom{a} j}^i)_{c} \wedge ( \Theta ^{j} )_{ab}
...
\begin{example}	
	定义\ref{chfb:def_Curvature}中的曲率型式（包含式\eqref{chccr:eqn_CST-II}）可改写为：
	\begin{align}
		\mathcal{D}_a ( \omega _{\hphantom{\mu} \nu}^\mu )_b = 
		{\rm d}_a ( \omega _{\hphantom{\mu} \nu}^\mu )_b  +  (\omega _{\hphantom{\mu} \rho}^\mu)_a
		\wedge ( \omega _{\hphantom{\mu} \nu}^\rho )_b = (\Omega_{\hphantom{\mu} \nu} ^{\mu})_{ab} .  
	\end{align}
\end{example}


\begin{example}	
定理\ref{chfb:thm_2ndBianchi}中第二Bianchi恒等式（包含式\eqref{chccr:eqn_Bianchi-CII}）可改写为：
	\begin{align}
		\mathcal{D}_c (\Omega_{\hphantom{a} \nu} ^{\mu})_{ab} =
		{\rm d}_c (\Omega_{\hphantom{a} \nu} ^{\mu})_{ab}  -  ( \omega _{\hphantom{a} \nu}^\rho )_c
		\wedge (\Omega_{\hphantom{a} \rho} ^{\mu})_{ab} + (\omega _{\hphantom{a} \rho}^\mu )_{c}
		\wedge	( \Omega_{\hphantom{a} \nu} ^{\rho} )_{ab} =0.  
	\end{align}
\end{example}
...
双外协变微分为：
\begin{align}
	\mathcal{D}_c\mathcal{D}_b \left(T^{i_1 \cdots i_r}_{
		k_1\cdots k_s}\right)_{a_1 \cdots a_p} {=} 
	&+ \sum_{n=1}^{r} (\Omega^{i_n}_{\hphantom{i_n}l})_{cb} \wedge
	\left(T^{i_1 \cdots i_{n-1} l i_{n+1} \cdots i_r}
	_{\hphantom{i_1 \cdots i_{n-1} l i_{n+1} \cdots i_r}k_1\cdots k_s}\right)_{a_1 \cdots a_p} \notag  \\
	&-\sum_{n=1}^{s} (\Omega^{l}_{\hphantom{l}k_n})_{cb} \wedge
	\left(T^{i_1 \cdots i_r}_{\hphantom{i_1 \cdots i_r}
		k_1\cdots k_{n-1} l k_{n+1} k_s}\right)_{a_1 \cdots a_p} . \label{chfb:eqn_DDpform}
\end{align}
直接带入即可证明上式，但计算较为繁琐（略）．
\end{example}
...
\section*{小结}
本章主要参考了\parencite{cc2001-zh}\S 3.1、第四章，\parencite{chen-li-2026-2ed-v2}第十章．


从\S\ref{chdm:sec_differentiable-manifold}到本页，我们没有实质使用度量（除\S\ref{chdm:sec_tmpdmcon}、
