...
虽然两维空间的曲率不能决定度量，
但在高维空间中有更强的定理描述了曲率与度量间的关系，
若读者有兴趣可参阅文献\parencite{Kulkarni-1970}．
...
\begin{proof}
    ...
    此定理是在说：在局部等距同构意义下，定理中描述的空间是{\kaishu 唯一的}；
...
也就是$p$点和其对径点$-p$，因而$p$是$\sigma_p$的孤立不动点．
所以$\sigma_p$是$S^m$的关于点$p$的中心对称；因$p$是任选的，故$S^m$是对称空间．
\qed
...
\begin{proof}
    ...
    $B_p(\varepsilon)$、$\mathscr{B}_p(\varepsilon)$定义见第\pageref{chgd:eqn_Ball}页的
    式\eqref{chgd:eqn_Ball}、式\eqref{chgd:eqn_gdBall}．令
    \begin{equation}
    ...
    从\eqref{chhss:eqn_geo-sym}式能更明显地看出此点．}
\end{proof}
...
则中心对称$\sigma_q$将测地线$\gamma$映射到它自身，
同时$\sigma_q$在$\gamma$上的限制是$\gamma$关于点$q$的对称，即
\begin{equation}\label{chhss:eqn_geo-sym}
    \sigma_q\bigl(\gamma(2t_0- t)\bigr) = \gamma(t) \quad \Leftrightarrow \quad
...
\begin{proposition}\label{chhss:thm_h-killing}
    广义黎曼均匀空间的任意切矢量都可以延拓成Killing切矢量场．
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{chhss:thm_iso2homo}
...
\begin{proof}
    ...
    那么$\{e^{-\sigma} (\frac{\partial}{\partial u^i})^a \}$是一个正交归一活动标架场，
    它的对偶基矢场为$\{(\theta^i)_a\equiv e^{\sigma} ({\rm d}u^i)_a \}$．
    可通过$(\theta^i)_a$的Cartan结构方程来求取黎曼曲率，
    经过繁琐计算可得黎曼曲率等于$C$．
\end{proof}
...
\begin{equation}
    k= \begin{cases}
        +1 & \text { 当最大对称子空间的曲率恒正，即超球面或}\mathbb{RP}^n \\ 
...
\section*{小结}
本章直接取自\parencite{oneill1983,weinberg_grav-1972,
    wolf_SCC-2011,chen-li-2026-2ed-v2}的相应章节．


文献\textcite{wolf_SCC-2011}是常曲率空间的权威专著．

虽然\textcite{helgason-2001}的著作成书于1960年代，
但是该书仍是{\kaishu 对称空间}领域的最权威、最全面的专著．
其它类似书籍或直接或间接取自此书；
比如\parencite[Ch.9]{chen-li-2026-2ed-v2}取自\parencite{mengdj-dckj-2005}，
而\parencite{mengdj-dckj-2005}直接取自该书．
文献\parencite{oneill1983}相应章节的内容也是取自该书．

对黎曼对称空间的分类需要更多李代数知识，
如读者有兴趣可参考\parencite{helgason-2001, mengdj-dckj-2005}．
