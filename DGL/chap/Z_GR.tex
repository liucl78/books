...
{\bfseries (4)} 任意矢量$\boldsymbol{G }$在两个参考系（绝对和相对）中变化率公式
是（参见文献\parencite[\S 3.6]{zhuzx-zy-vI}第218页(3.34)式，或类似文献）：
...
还需要求Fermi导数与缩并可交换，以及满足Leibniz律和线性性；不再单独列出公式．
...
由式\eqref{chsm:eqn_fermi-walker-fun}和Leibniz律，
...
\end{equation}
借助上式以及Leibniz律，可得度规场$g_{ab}$的Fermi导数，
...
此观点可见\textcite[p.6]{stratton1941}，
\textcite[p.603]{courant_hilbert-v2}，
邓煜\cite{deng-2017}和绝大多数计算电磁学书籍．
...
\begin{equation}
	...
	\nabla^a G_{ab}=8\pi \nabla^a T_{ab}
	\quad \xLongrightarrow[\ref{chrg:eqn_Div-Einstein-tensor=0}]{\nabla^a G_{ab}=0} \quad
	\nabla^a T_{ab}=0 .
\end{equation}

最早提出此观点是爱因斯坦\cite{einstein_infeld_1949}本人以及Hilbert，
...
偏微分方程组（$\sum_{j}\frac{\partial}{\partial x^j} F_{ji} = J_{i} {\ \Longleftrightarrow \ }
...
还需补充初边值条件、拓扑条件等等；所有这些存在性充分条件构成的集合只能是“充分”条件，
不可能是“充分必要”条件！
...
\end{equation}
有一种观点（\textcite[p.6]{stratton1941}，
\textcite[p.603]{courant_hilbert-v2}，邓煜\cite{deng-2017}等）认为：
只须要求式\eqref{chfd:eqn_gauss-nosource}在初始时刻（$t=0$）满足，
则由式\eqref{chfd:eqn_mxe}就能得到\eqref{chfd:eqn_gauss-nosource}式在任意时刻（$t>0$）都成立；
...
这样必须同时考虑全部八个麦氏方程，不能将任何一个方程分离．对于电磁理论而言，
{\fangsong 现有理论已不再支持将式\eqref{chfd:eqn_gauss-nosource}和\eqref{chfd:eqn_fam-nosource}分开处理．}
...
\paragraph{小结}
若偏微分方程组中含有潜在的微分恒等式，比如单旋度方程、麦氏方程、爱氏场方程、Yang--Mills方程、……；
则一定要将它们的“相容性条件”看成解存在性条件之一，从而独立于方程组本身．
否则，很可能陷入循环逻辑．
