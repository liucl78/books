...
\section{数个代数概念}
本小节只罗列一些后面章节会用到的代数概念，
详尽内容请参考\parencite{mengdj-cxds-1}或类似文献．
...
\begin{definition}\label{chtop:def_map}
    ...
    元素$b$称为$a$在映射$\sigma$下的{\heiti 像}；
    而$a$称为$b$在映射$\sigma$下的{\heiti 原像}．
    
    若$\forall a_1,a_2 \in M$都有$\sigma(a_1) \neq \sigma(a_2)$，
    则称映射$\sigma$是单一的，或{\heiti 单射}．
    
    若$\forall b\in N$在$M$中都至少存在一个原像$a$，
    则称映射$\sigma$是满的，或{\heiti 满射}．
    
    既单又满的映射$\sigma$称为{\heiti 双射}或{\heiti 一一映射}；
    双射的逆是存在的，记为$\sigma^{-1}$．
\end{definition}
...
\begin{definition}\label{chtop:def_number-field}
    ...
    如果$\mathbb{F}$中任意两个数（可以是相同的两个数）的和、
    差、积、商（除数不为$0$）仍是$\mathbb{F}$中的数（即对上述运算有封闭性），
...
\begin{definition}\label{chtop:def_group}
    ...
    从集合$G$中任意两个元素$a$和$b$（两者可相等）得到的$a*b$恒在$G$中（即群乘法是{\heiti 封闭的}）．
    再给出关于群乘法的三条性质：
    
    \fbox{甲} 结合律：即$\forall a,b,c \in G$，恒有$(a*b)*c=a*(b*c)$；
    
    \fbox{乙} 单位元（幺元）：存在幺元$e\in G$使得$\forall a \in G$有$a*e=e*a=a$；
    
    \fbox{丙} 逆元：$\forall a \in G$，都存在元素$b\in G$使得$a*b=b*a=e$；记$a^{-1}=b$．
    
    如果集合$G$和群乘法只满足封闭性，那么称$G$为{\heiti 原群}（Magma）．
    如果原群和群乘法只满足性质\fbox{甲}，那么称$G$为{\heiti 半群}．
    如果原群和群乘法只满足性质\fbox{甲}和\fbox{乙}，那么称$G$为{\heiti 幺半群}．
    如果原群和群乘法满足性质\fbox{甲}、\fbox{乙}和\fbox{丙}，那么称$G$为{\heiti 群}．
...
\begin{definition}
    设$H$为$G$的{子群}，由固定的$g\in G$且$g\notin H$可生成子群$H$的{\heiti 左陪集}：
    $g H = \{g h \mid \forall  h \in H \}$；
...
我们不加证明地引入群同态基本定理（证明见\parencite[\S 1.7]{mengdj-cxds-1}或类似书籍）：
...
\begin{definition}\label{chtop:def_group_representation}
    ...
    简称{\heiti 表示}． 如果$\phi$是同构映射，则称为{\heiti  忠实表示}．
...
\begin{definition}
    ...
    当$\Omega$是$n$个有限元素的集合时，
    $\Omega$到自身的{\kaishu 双射}称为一个{\heiti $\boldsymbol{n}$元置换}；
    $S(\Omega)$上的全体变换称为{\heiti $\boldsymbol{n}$元对称群}，记为$S_n$；
...
则称$e$是$R$中的{\heiti 幺元}或单位元，
此时环$R$称为{\heiti 幺环}或者有单位元的环；
可证单位元是唯一的，通常会用“$1$”来代替$e$．
...
如果环$R$中有元素$b\neq 0$，存在元素$a\neq 0$使得$ab=0$（或$ba=0$），
则称元素$a$为$R$的一个{\heiti 左}（或{\heiti 右}）{\heiti 零因子}，
两者都可简称为{\heiti 零因子}．元素0也可称为（平凡）零因子．
...
环和域还有{\kaishu 特征}（Characteristic）的概念（省略不写）；
...
\end{equation*}
所有四元数组成的集合记为$\mathbb{H}$，规定$\mathbb{H}$的加法和乘法为：
\begin{align*}
...
\begin{definition}\label{chtop:def_top}
    非空集合$X$上的一子集族$\mathscr{T}$，若它满足如下条件：
...
\begin{definition}\label{chtop:eqn_neighbor}
    ...
    我们称$x$为$N$的{\heiti 内点}，所有内点的集合称为$N$的内部，记作$\stackrel{\circ}{N}$．
\end{definition}


\index[physwords]{极限点} \index[physwords]{聚点} \index[physwords]{闭包}
\begin{definition}
若$A$是拓扑空间$X$的一个子集，$x$是$X$的一个点，
如果$x$的任意一个邻域与$A$的交含有异于$x$的点，
我们称$x$为$A$的一个{\heiti 极限点}（或{\heiti 聚点}）．
$X$所有极限点的集合与$X$的并集称为它的{\heiti 闭包}，记作$\bar{X}$．
\end{definition}
注意：如果$x$属于$A-\{x\}$的闭包，则$x$便是$A$的一个极限点．
这个点$x$可以在$A$中，也可以不在$A$中，在此定义中$x$是否在$A$中是无关紧要的．

    
\begin{definition}\label{chtop:def_Hausdorff}
    拓扑空间$X$中任意两个不同点$x$和$y$，
    若有不相交的邻域$U$和$V$分别包含它们，
    即$x\in U$、$y\in V$且$U \cap V = \varnothing$，
    则称$X$为{\bfseries \heiti Hausdorff空间}．
\end{definition}
...
\begin{definition}\label{chtop:def_closedset}
    设$U$是拓扑空间$X$的子集，若$X-U$是开集，则称$U$为{\heiti 闭集}． 
\end{definition}
...
\begin{definition}
	...
	则称$A$是{\heiti 可数的}（countable）；否则称为{\heiti 不可数}．
\end{definition}
有限集合自然可以和正整数集合的某个子集建立双射关系，故必然可数．

\index[physwords]{稠密}  \index[physwords]{可分}
\begin{definition}
	拓扑空间$X$的子集$A$称为{\heiti 稠密的}（dense）是指$\bar{A}=X$．
	如果$X$有可数的稠密子集，则称$X$是{\heiti 可分的}（separable）拓扑空间．
\end{definition}



\index[physwords]{笛卡尔积}
\begin{definition}\label{chtop:def_cartensian}
    设有两个非空集合$A$和$B$，所有有序偶对$(a,b)$（其中$a\in A, b\in B$）
    的集合定义为{\heiti 笛卡尔积}，简称为{\heiti 积}，
...
\section{度量空间}\label{chtop:sec_RmTP}

\begin{definition}\label{chtop:def_metric}
    集合$X$的{\heiti 度量}是一个函数：$d:X\times X \to \mathbb{R}$；
    它使得以下性质成立：
...
定义\ref{chtop:def_metric}中的度量是集合$X$上的度量．
之后，我们还会在线性空间、黎曼流形上引入度量概念；
若它们是正定的（比如欧式度量，黎曼度量），则符合定义\ref{chtop:def_metric}；
若它们是不定的（比如Lorentz度量），则不符合定义\ref{chtop:def_metric}中的第(3)条．
...
\begin{example}
仿照$\mathbb{R}^m$中度量\eqref{chtop:eqn_Rdbc}，我们定义$\mathbb{C}^m$中度量：
\begin{subequations}\label{chtop:eqn_Rdbc-cx}
	\begin{align}
		d_b(\boldsymbol{x}, \boldsymbol{y})=& 
		\sqrt{|x^1-y^1|^2+\cdots +|x^m-y^m|^2}, \label{chhs:eqn_Rdb} \\
		d_c(\boldsymbol{x}, \boldsymbol{y})=& 
		\max\{|x^1-y^1|,\cdots,|x^m-y^m| \}. \label{chhs:eqn_Rdc}
	\end{align}
\end{subequations}
当$m=1$时，$|x-y|$就是两个复数$x$、$y$之差的模长． \qed
\end{example}
...
\begin{proof}
    ...
    设$B_{d}(x_1,\varepsilon_1)$、
    $B_{d}(x_2,\varepsilon_2)$是$(\mathbb{R}^m,d)$的两个$\varepsilon$-球，
    再假设它们的交集非空，
    ...
    也就是：{\kaishu $(X,d)$上任意两个$\varepsilon$-球的交集是若干$\varepsilon$-球的并集．}
...
\begin{example}
	因$(a,b)$是$\mathbb{R}$中的开集，
	由定义\ref{chtop:def_closedset}可知$(-\infty, a]$、$[b,+\infty)$以及单点集都是闭集．
...
\begin{remark}
	上述$\mathbb{R}^m$中的标准开集是通过度量\eqref{chtop:eqn_Rdbc}诱导得到的；
	若黎曼几何中（见后面章节）的附加度量是正定的，那么这种定义标准开集的方式是妥当的．
	若黎曼几何附加不定度量，那么不定黎曼度量与度量\eqref{chtop:eqn_Rdbc}可能存在逻辑上的不自洽．
	其实可以通过“序拓扑”方式引入$\mathbb{R}^m$中的标准开集，
	序拓扑定义的标准开集与上面由度量\eqref{chtop:eqn_Rdbc}诱导得到的标准开集完全相同；
	可参见\parencite[\S 10,\S 14,\S 15]{munkres-2000-topology}．
\end{remark}
...
\begin{theorem}\label{chtop:thm_bcb}
    若 $X$ 是 $\mathbb{R}^m$ 的一个子空间，
    那么无论是用 $X$上的欧几里得度量还是按确界度量，
    $X$ 的开集族是相同的； 对于$X$的闭集族也同样成立．    
\end{theorem}

定理\ref{chtop:thm_bcb}表明：对于拓扑问题来说，$\mathbb{R}^m$上的这两个度量是等价的．




\begin{definition}\label{chtop:def_induced-top}
    设$A$是拓扑空间$(X,\mathscr{T})$的一个非空子集，
    构建如下子集族$\mathscr{T} _A \equiv \{U\cap A \mid U\in \mathscr{T}\}$．
    称$\mathscr{T}_A$是$A$上的{\heiti 子空间拓扑}或从$(X,\mathscr{T})$得到的{\heiti 诱导拓扑}．
\end{definition}

\index[physwords]{诱导拓扑}  \index[physwords]{子空间拓扑}
...
$Y$也选为$\mathbb{R}$，但将拓扑选为所谓的{\kaishu 下限拓扑}，
即$\mathbb{R}$中的半开半闭区间$[a,b)$是开集，
同时将其记为$\mathbb{R}_\ell$．写到这里需要对拓扑再做些说明，
拓扑的规定是十分任意的，也是人为的，只要符合拓扑三公理即可，
下限拓扑完全符合拓扑三公理，
请读者自行验证或参考\parencite[\S 13]{munkres-2000-topology}中的$\mathbb{R}_K$拓扑定义．
现在，我们选恒等映射
\begin{equation}
    \imath : \mathbb{R} \to \mathbb{R}_\ell,\quad
    \text{即对于任意一个实数}x,\  \imath(x) =x .
\end{equation}
但是映射$\imath${\kaishu 不是}一个连续函数．
因为$\mathbb{R}_\ell$中的开集$[a,b)$的原像还是$[a,b)$，
它不是$\mathbb{R}$（标准拓扑）中的开集． \qed

\begin{definition}\label{chtop:def_homeomorphism}
    设$X$和$Y$是两个拓扑空间，如果存在双射$f:X\to Y$，并且$f$和$f^{-1}$是连续的，
    则称映射$f$为{\heiti 同胚}． 此处指整体同胚．  
\end{definition}
\index[physwords]{同胚}


正是因为有例\ref{chtop:exm_txyr}这种情形，定义\ref{chtop:def_homeomorphism}中必须同时满足$f$、$f^{-1}$连续．
在同胚映射下不变的属性称为{\heiti 拓扑性质}，比如紧致性、连通性、可分性等．

\index[physwords]{拓扑流形}
\begin{definition}\label{chtop:def_topological-manifold}
    设$X$为具有可数拓扑基的Hausdorff空间，
    若它的每一点$x$都有一个开邻域$U$同胚于$\mathbb{R}^m$中的一个标准开子集，
    则称$X$为$m$维{\heiti 拓扑流形}．
\end{definition}
...
\begin{theorem}
    ...
    如果 $f: X \rightarrow Y$ 在 $x_0$点是连续的，
    那么限制函数 $\left.f\right|_A: A \rightarrow Y$在$x_0$点也是连续的．
...
我们不加证明（可参考\parencite{munkres-2000-topology}）地列举一些拓扑空间的定理、例题．


%\section{紧致性}

\begin{definition}
	度量空间$(X, d)$上的点列$\left\{x_n\right\}$叫作收敛到$x_0$是指：
	当$n \rightarrow \infty$时，$d\left(x_n, x_0\right) \rightarrow 0$；
	记作 $\lim _{n \rightarrow \infty} x_n=x_0$．
	其中$x_0$可能属于$X$，也可能不属于$X$．
\end{definition}

\index[physwords]{收敛序列}  \index[physwords]{列紧}  \index[physwords]{自列紧}

\begin{definition}
	设$(X, d)$是一个度量空间，$A$为其一子集．
	若$A$中的任意点列在$X$中有一个收敛子列，则称$A$是{\heiti 列紧的}；
	若收敛子列极限点属于$A$，则称$A$是{\heiti 自列紧的}．
	若空间$X$是自列紧的，则称$X$为{\heiti 自列紧空间}．
\end{definition}


\begin{definition}\label{chtop:def_covering}
    给定拓扑空间$(X,\mathscr{T})$，设$\{U_\alpha\}, \alpha\in I$是$X$的一个
    子集族，如果$\{U_\alpha\}$的成员之并等于$X$，则称$\{U_\alpha\}$为$X$的{\heiti 覆盖}．
    若$\{U_\alpha\}$每个成员都是开子集，则称为{\heiti 开覆盖}．
\end{definition}

\index[physwords]{开覆盖}   \index[physwords]{紧致}
...
\begin{theorem}
	设$(X,d)$是一个度量空间；$X$是紧致的充要条件是$X$为自列紧的．
\end{theorem}
...
\begin{theorem}
    Hausdorff空间$X$在$x$处局部紧致的充要条件是：对于$x$的任意邻域$U$，存在$x$的一个邻域$V$，
    使得$\bar{V}$紧致并且$x\in \bar{V}\subset U$．
\end{theorem}
...
\begin{proposition}\label{chtop:thm_uvb}
...
\begin{proposition}\label{chtop:thm_ccb}
...
\begin{definition}\label{chtop:def_complete-metric}
    给定度量空间$(X,d)$．
    $X$中的Cauchy序列$\{x_n\}$是指：$\forall \epsilon >0 $，
    存在正整数$N$使得$d(x_n,x_m)<\epsilon,\ \forall n,m>N$成立．
    如果$X$中的每个Cauchy序列都是收敛的（极限点属于$X$），
    则称$X$是{\heiti 度量完备}的，简称{\heiti 完备}．
\end{definition}

\index[physwords]{完备} \index[physwords]{完备!度量完备}
    
\begin{proposition}
    对于度量\eqref{chtop:eqn_Rdbc}（\eqref{chtop:eqn_Rdbc-cx}）而言，
    欧氏空间$\mathbb{R}^m$（$\mathbb{C}^m$）是完备的．
\end{proposition}

\begin{theorem}
	紧致度量空间（即自列紧度量空间）必是度量完备空间．
\end{theorem}


\begin{remark}\label{chtop:rem_nc}
	不同度量下，空间的完备属性可能不同（例如参见\parencite[1.5-5, 1.5-9]{Kreyszig-1991}）．
\end{remark}
...
虽然上面叙述了很多完备空间，但并不是所有空间都是完备的．
比如可数的有理数域$\mathbb{Q}$就是不是完备的，
其中的序列可能收敛于一个无理数．
但有理数域的闭包$\bar{\mathbb{Q}}(=\mathbb{R})$是完备的．
也就是说，不完备的空间可以延拓成完备空间；
这一结论在更大范围内也是成立的，为此先给出等距定义．

\index[physwords]{度量空间等距同构}

\begin{definition}
	设$(X,d)$、$(Y, \rho)$是两个度量空间．
	若存在双射$\phi: X \rightarrow Y$使得
	\begin{equation*}
		d(x, x')=\rho\bigl(\phi(x) , \phi (x')\bigr), \qquad \forall x, x' \in X
	\end{equation*}
	成立，则称$(X, d)$和$(Y, \rho)$是{\heiti 等距同构的}，并称$\phi$为{\heiti 等距同构映射}．
\end{definition}

若$(X,d)$等距于$(Y, \rho)$的某个子空间，则称$X$可等距嵌入到$Y$中．

\begin{theorem}\label{chtop:thm_iso-DS}
	对于任意度量空间$(X, d)$，一定存在一个完备度量空间$(\hat{X}, \hat{d})$，
	使得它的稠密子空间$\widehat{W} \subset \hat{X}$与$X$等距同构．
	如果对等距空间不加区分，则空间$\hat{X}$是唯一的；也就是说，
	若$(\tilde{X},\tilde{d})$是另一个完备度量空间且有稠密子空间$\widetilde{W}$和$X$等距，
	则$(\tilde{X},\tilde{d})$与$(\hat{X}, \hat{d})$是等距同构的．
\end{theorem}


\index[physwords]{l${}^p$空间}

\begin{example}\label{chtop:exam_lp}
	$l^p$序列空间；其中$p \geqslant 1$是一个固定的实常数．
\end{example}

$l^p$中每个元素是一个无穷复数列$\left(\xi_1, \xi_2, \cdots\right)$，
它满足$\sum_{i=1}^{\infty}\left|\xi_i\right|^p<\infty$．

$l^p$上的度量定义为（$(\xi)$、$(\eta)$都是$l^p$中的元素）：
\begin{equation}
	d(\xi, \eta)=\left(\sum_{i=1}^{\infty}\left|\xi_i-\eta_i\right|^p\right)^{1 / p} .
\end{equation}
可以证明（略）：$l^p$序列空间是度量空间，且可分、完备．
证明上述属性时会用到如下不等式．
设$p\geqslant 1$，且是固定不变的实常数．再定义$q$为：
\begin{equation}
	\frac{1}{p}+\frac{1}{q}=1 .
\end{equation}
H{\"o}lder不等式和Minkowski不等式为（其中$\left(\xi_{j}\right) \in l^{p}$、$\left(\eta_{j}\right) \in l^{p}$）：
\begin{align}
	\sum_{j=1}^{\infty}\left|\xi_{j} \eta_{j}\right| \leqslant & 
	\left(\sum_{k=1}^{\infty}\left|\xi_{k}\right|^{p}\right)^{1 / p}
	\left(\sum_{i=1}^{\infty}\left|\eta_{i}\right|^{q}\right)^{1 / q}, \label{chtop:eqn_Holder} \\
	\left(\sum_{j=1}^{\infty}\left|\xi_{j}+\eta_{j}\right|^{p}\right)^{1 / p} \leqslant &
	\left(\sum_{k=1}^{\infty}\left|\xi_{k}\right|^{p}\right)^{1 / p}
	+\left(\sum_{i=1}^{\infty}\left|\eta_{i}\right|^{p}\right)^{1 / p}. \label{chtop:eqn_Minkowski} 
\end{align}
若$p=2$（则$q=2$），这时式\eqref{chtop:eqn_Holder}给出著名的Cauchy--Schwarz不等式．


当$p=2$时，便得到了最为常用的Hilbert序列空间$l^2$．
\qed

\index[physwords]{H{\"o}lder不等式}
\index[physwords]{Minkowski不等式}
\index[physwords]{Cauchy--Schwarz不等式}
...
%定义\ref{chtop:def_covering}中的覆盖和定义\ref{chtop:def_covering-map}中的覆叠的英文都是“cover”．
%定义\ref{chtop:def_covering}是点集拓扑中的概念；
%定义\ref{chtop:def_covering-map}是代数拓扑中的概念．
%“cover”也被翻译成“复叠”、“覆盖”等等．
...
%\begin{example}\label{chtop:exam_cab}
%	令$X$为定义在闭区间$[a, b]$上的所有连续实函数的集合，用
%	\begin{equation}
%		d(x, y)=\max_{a \leqslant t \leqslant b}|x(t)-y(t)| ,
%		\qquad \forall x(t), y(t) \in X
%	\end{equation}
%	作为函数空间$X$上的度量；通常将$X$记为$C[a, b]$．
%	在上述度量下，$C[a, b]$是完备空间（证明见\parencite{zhang-lin-2021}例1.1.7，或类似文献）．
%\end{example}
%
%
%\begin{example}\label{chtop:exam_cab-nc}
%	举例说明上述{注解}．令$X$是$[0,1]$上所有连续实值函数的集合，
%	且定义度量：$d(x, y)=\int_0^1|x(t)-y(t)| \mathrm{d} t$．
%	可以证明\parencite[\S 1.5-9]{Kreyszig-1991}：这个度量空间$(X, d)$是不完备的．
%	请与例\ref{chhs:exam_cab}中完备度量比较．
%\end{example}
