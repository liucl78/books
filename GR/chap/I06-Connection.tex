...
\end{equation}
关于此式的更多讨论可参见例题\ref{chccr:exam_soldering}．
...
无论$\Gamma^k_{ij}$有挠与否，则对称部分$\oversetmy{s}{\Gamma}^k_{ij}$肯定无挠的．
...
\end{align}
将黎曼曲率中的两个指标缩并可定义Ricci曲率：
\begin{equation}\label{chccr:eqn_Ricci-component}
    R_{jn} \overset{def}{=} R_{\hphantom{a} jkn}^k = {\partial_k} \Gamma_{jn}^{k} -\partial_n \Gamma_{jk}^{k}
    + \Gamma_{jn}^{k} \Gamma_{kl}^{l}- \Gamma_{jl}^{k}\Gamma_{kn}^{l} .
\end{equation}
这两个表达式适用于有挠或无挠联络，需注意$\Gamma$下标的顺序．

除了上述Ricci曲率外，我们还可以定义如下曲率（它几乎无用）：
\begin{equation}
	R'_{cd} \overset{def}{=} R_{\hphantom{a} acd}^a ;
	\qquad \text{显然有}\ 	R'_{cd} = - R'_{dc}.
\end{equation}
...
\begin{align}\label{chccr:eqn_natural-base_change}
    ...
    ({e} ^i)_a = \left({\rm d} x^\alpha \right)_a \left(C^{-1}\right)^{i}_{\hphantom{a} \alpha}   ;
...
%	- C_{\hphantom{a} l}^{\beta} \Gamma^l_{ij} (\partial_\beta)^a (e^i)_b (e^j)_c \\
	...
	- C_{\hphantom{a} l}^{\beta} \Gamma^l_{ij} \left(C^{-1}\right)^{j}_{\hphantom{a} \sigma} 
...
\begin{subequations}\label{chccr:eqn_tetrad-postulate}
	\begin{align}
		...
		+ C_{\hphantom{a} i}^{\alpha} \oversetmy{x}{\Gamma}_{\alpha \sigma}^\beta
		- C_{\hphantom{a} l}^{\beta} \Gamma^l_{ij} \left(C^{-1}\right)^{j}_{\hphantom{a} \sigma} = &0,\\
		\frac{\partial \left(C^{-1}\right)^{i}_{\hphantom{a} \rho} }{\partial x^\sigma}
		- \left(C^{-1}\right)^{i}_{\hphantom{a} \mu} \oversetmy{x}{\Gamma}^\mu_{\rho\sigma} 
		+ \left(C^{-1}\right)^{l}_{\hphantom{a} \rho} \Gamma^{i}_{lj} 
		\left(C^{-1}\right)^{j}_{\hphantom{a} \sigma} = & 0.
	\end{align}
\end{subequations}
上式无非是\eqref{chccr:eqn_GAMMA-trans}或其变种．
上式中的第二式是由$0 = \nabla_c ( (e_i)^a ({\rm d}x^\mu)_b 
(C^{-1})^{i}_{\hphantom{a} \mu} )$推演得到．
有些文献将上式称为“tetrad postulate”；
由于上两式为恒等式，不是假设，故这个称呼不准确．\qed

\index[physwords]{tetrad postulate}

%\begin{align*}
%	0 =& \nabla_c \delta^a_b = \nabla_c \left( (e_i)^a ({\rm d}x^\mu)_b \left(C^{-1}\right)^{i}_{\hphantom{a} \mu}  \right) \\
%		=& \frac{\partial \left(C^{-1}\right)^{i}_{\hphantom{a} \rho} }{\partial x^\sigma} (e_i)^a ({\rm d}x^\rho)_b ({\rm d}x^\sigma)_c
%		- \left(C^{-1}\right)^{i}_{\hphantom{a} \mu} \oversetmy{x}{\Gamma}^\mu_{\rho\sigma} (e_i)^a  ({\rm d}x^\rho)_b ({\rm d}x^\sigma)_c
%		+ \left(C^{-1}\right)^{l}_{\hphantom{a} \rho}  \Gamma^{i}_{lj} (e_i)^a ({\rm d}x^\rho)_b (e^j)_c  \\
%	=& \left(\frac{\partial \left(C^{-1}\right)^{i}_{\hphantom{a} \rho} }{\partial x^\sigma}
%	- \left(C^{-1}\right)^{i}_{\hphantom{a} \mu} \oversetmy{x}{\Gamma}^\mu_{\rho\sigma} 
%	+ \left(C^{-1}\right)^{l}_{\hphantom{a} \rho} \Gamma^{i}_{lj} \left(C^{-1}\right)^{j}_{\hphantom{a} \sigma}
