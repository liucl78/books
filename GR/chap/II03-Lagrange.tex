...
\subsection{度规变分规则}\label{chlh:sec_dgab}
...
因为$g_{ab}(x;u)$不能随意进出$\partial_u \left(\bullet \right)$．
...
为令计算简洁一些，先给出电流密度表达式（详见\S\ref{chlh:sec_charge}）
\begin{equation}\label{chlh:eqn_eCurrent}
    J^a(x) \equiv \sum_{p} e_{p} \int  \frac{{\rm d} x^a}{{\rm d} \tau}
    \delta^4(x^\alpha-x^\alpha_p)\frac{1}{\sqrt{-g}} {\rm d} \tau .
\end{equation}
已知$\delta^4(x^\alpha-x^\alpha_p) = \delta^3(\boldsymbol{x}-\boldsymbol{x}_p)
\delta(t-x^0_p)$；由此我们可以将积分号去掉，得到电流密度的不显示协变的三维形式：
\begin{equation}\label{chlh:eqn_eCurrent-3j}
	J^a(x) = \sum_{p} e_{p} \frac{{\rm d} x^a}{{\rm d} t}
	\delta^3(\boldsymbol{x}-\boldsymbol{x}_p) \frac{1}{\sqrt{-g}}  .
\end{equation}
...
\begin{equation}\label{chlh:eqn_Ipi}
...
\begin{align*}
   ...
   \Rightarrow \, 
   0 &= {e_p} F_{\hphantom{c}a}^c \frac{{\rm d} x^a_p}{{\rm d} \tau} 
...
\setlength{\mathindent}{0em}
\begin{subequations}\label{chlh:eqn_EM+Plasma-BRtensor}
\begin{align}
    ...
    =& F^{ac} F^{b}_{\hphantom{b} c} - \frac{1}{4} g^{ab} F_{cd} F^{cd} 
...
式\eqref{chlh:eqn_EM+Plasma-BRtensor}的第一式为协变形式，第二式为非协变的三维形式；
其变换用到了：
\begin{align*}
	U^0 = \frac{{\rm d}t}{{\rm d}\tau}, \quad
	U^i = \frac{{\rm d}x^i}{{\rm d}\tau}; \qquad
	\delta^4(x^\alpha-x^\alpha_p) = \delta^3(\boldsymbol{x}-\boldsymbol{x}_p)\delta(t-x^0_p) .
\end{align*}


\section{理想流体力学}\label{chlh:perfect-fluid}

到目前为止，我们对于非引力物理学的模型一直是基于点粒子（可能带电）及其相关的电磁场的模型．
对于某些应用来说，这是一个有用的模型；但对于大多数的应用而言，它过于微观了．
当描述太阳、星系等大片连续物质时，点粒子模型就不适用了。
此时，我们需要使用理想流体（指粘性、热传导和剪切应力可忽略不计）作为物质的模型，这属于连续介质模型．
这个模型对物质在一定尺度范围内进行“粗粒化”描述．
假定在这些尺度范围内物质是电中性的，故电磁场的直接影响可以忽略不计．
因此，可以在流体内部的某一小块物质（称为{\kaishu 流体质团}）上谈论它的密度、压力和速度等．

下面介绍一点相对论热力学内容\cite[\S 22.2]{mtw1973}．
我们先给出共动参考系（相对流体质团静止的参考系）的数个物理量：
{\bfseries (1)} 粒子数密度$n$，指重子数减去反重子数．
{\bfseries (2)} 总质量密度$\mu$，包括质团的静止质量$\rho$、内能．
{\bfseries (3)} 各向同性压强$p$．
{\bfseries (4)} 平衡态的温度$T$．
...
在这个初步介绍中，我们只考虑宏观、经典、理想流体，
不考虑量子效应、核反应、化学反应等；
也不具体区分流体质团成分；这是相对论流体力学的最简模型．
在这些假设下，流体的热力学属性只由两个参量来描述，
我们选为{\kaishu 粒子数密度}和{\kaishu 单个粒子熵}；
我们进一步假设\CJKunderwave{流动是等熵、无旋的}．
...
\begin{equation}\label{chlh:eqn_1TDL}
	{\rm d}\mu = n T {\rm d}s +\frac{\mu+p}{n} {\rm d}n 
	\xlongequal[\text{成立条件下}]{\text{若等熵假设}} 
	\frac{\mu+p}{n} {\rm d}n  .
\end{equation}



我们采用的理想流体作用量\cite{Ray-1972}是（国际单位制，之后采用自然单位制）：
\begin{equation}\label{chlh:eqn_perfect-fluid}
	\begin{aligned}
		I_{PF}&=\frac{1}{c}\int \Bigl(-\mu c^2
		+\kappa_1\left(g_{ab}U^a U^b+c^2 \right) 
		+\kappa_2 \nabla_a (n U^a)  \Bigr)
		\sqrt{-g}  {\rm d}^4x  \\
		&\xlongequal[\text{自然单位制}]{\text{分部积分}}
		\int \Bigl(-\mu  +\kappa_1\left(g_{ab}U^a U^b+1 \right) 
		-n U^a \nabla_a\kappa_2   \Bigr)
		\sqrt{-g}  {\rm d}^4x .
	\end{aligned}
\end{equation}
...
并丢掉了边界项．
把式\eqref{chlh:eqn_perfect-fluid}中第二行积分号内的部分记为$\mathscr{L}_{PF}$，即
\begin{equation}
	\mathscr{L}_{PF}\equiv -\mu	+\kappa_1\left(g_{ab}U^a U^b+1 \right) -n U^a \nabla_a\kappa_2.
\end{equation}
计算几个关于$\mathscr{L}_{PF}$的导数：
\begin{align}
	\frac{\partial \mathscr{L}_{PF}}{\partial n} = &
	-\frac{\partial\mu }{\partial n} - U^a \nabla_a\kappa_2 .\\
...
\begin{equation}\label{chlh:eqn_tmp-k1}
	\kappa_1 = \frac{1}{2}n \frac{\partial\mu }{\partial n}
	\xlongequal{\ref{chlh:eqn_1TDL}} \frac{\mu+p}{2} .
\end{equation}
由上两式可得
\begin{equation}\label{chlh:eqn_tmp-k2}
	\nabla_a \kappa_2 = \frac{\mu+p}{n} U_a . 
\end{equation}
不难计算出：
\begin{equation}
	\frac{\partial \mathscr{L}_{PF}}{\partial g_{ab}} = 
	\kappa_1 U^a U^b = \frac{\mu+p}{2} U^a U^b .
\end{equation}
由式\eqref{chlh:eqn_BR-tensor}得
\begin{equation*}
	T^{ab}  %= 2\frac{\partial \mathscr{L}_{NG}}{\partial g_{ab}}    +  g^{ab} \mathscr{L}_{NG} 
	=(\mu + p)U^a U^b + g^{ab} \left( -\mu
...
\begin{equation}\label{chlh:eqn_perfect-fluid-Tab}
	T^{ab}=\left(\mu  + \frac{p}{c^2}  \right) U^a U^b +p g^{ab} 
	=\left(\rho +\rho \frac{\Pi}{c^2} + \frac{p}{c^2}  \right) U^a U^b +p g^{ab} .
\end{equation}
上式第一个等号后的$\mu$是指相对论能量，包含静质量能、内能；
在第二个等号中，我们将它分开处理，即$\mu = \rho +\rho\frac{\Pi}{c^2}$，
其中$\rho$是静质量，$\Pi$是单位质量内能．



为了保持本章框架的整体性，我们从作用量\eqref{chlh:eqn_perfect-fluid}出发，
经过变分法得到了理想流体的能动张量\eqref{chlh:eqn_perfect-fluid-Tab}；这种方法并不是很常用．
读者应能看出作用量\eqref{chlh:eqn_perfect-fluid}有拼凑的痕迹，
它不像作用量\eqref{chlh:eqn_I-NG}、\eqref{chlh:eqn_I-G}那样精简及物理意义明显．
更多相对论流体力学内容可参考\parencite[Ch.15]{landau_6-fluid}或\parencite[\S 2.10]{weinberg_grav-1972}．

\paragraph{参数$\rho^*$}
流体必然遵守粒子数守恒方程\eqref{chlh:eqn_particle-cons}（即$\nabla_a (n U^a) = 0$）；
设每个流体质团的质量为$m$（常数），乘上这个因子后，此方程变为流体的连续性方程：
\begin{equation}\label{chlh:eqn_fluid-continum}
	0=\nabla_\alpha (\rho U^\alpha) 
	\xlongequal{\ref{chrg:eqn_divergence}} \frac{1}{\sqrt{-g}} 
	\frac{\partial}{\partial x^\alpha}\left( \sqrt{-g}\rho U^\alpha \right).
\end{equation}
注意$\rho$表示不含内能的静质量．我们定义（注$U^\mu = \frac{\mathrm{d}x^\mu}{\mathrm{d} \tau}$）：
\begin{align}
	\gamma \equiv & \frac{\mathrm{d} t}{\mathrm{d} \tau} 
	=\frac{1}{c}\frac{\mathrm{d}x^0}{\mathrm{d} \tau}= U^0/c , \label{chlh:eqn_gamma}\\
	u^i \equiv & \frac{{\rm d} x^i}{{\rm d}t} 
	=\frac{{\rm d} x^i}{{\rm d}\tau} \frac{\mathrm{d} \tau}{\mathrm{d} t}  
	= U^i /\gamma, \label{chlh:eqn_ui}\\
	\rho^* \overset{def}{=} & \sqrt{-g} \gamma \rho
	=\sqrt{-g} \rho U^0 / c . \label{chlh:eqn_rho-star}
\end{align}
那么，方程\eqref{chlh:eqn_fluid-continum}可以改写为（注$x^0 \equiv c t$）：
\begin{equation}\label{chlh:eqn_cont-star}
	0= \frac{\partial}{\partial t}\left( \sqrt{-g}\rho U^0/c \right)
	+\frac{\partial}{\partial x^i}\left( \sqrt{-g}\rho\gamma u^i \right)
	= \frac{\partial \rho^* }{\partial t}
	+\frac{\partial  \rho^* u^i }{\partial x^i}.
\end{equation}
用$\rho^*$表示的连续性方程与我们熟悉的流体力学连续性方程形式相同．

在描述辅助参量$\rho^*$的这一段落中我们采用了国际单位制，之后令$c=1$．

\subsection{平直闵氏时空中的相对论流体力学}
...
但在平直闵氏时空中，表述很简单．由$\partial^a T_{a b}=0$得
\begin{equation}\label{chlh:eqn_pfm}
	0=\partial^a T_{a b}=U_a U_b \partial^a(\mu+p)+(\mu+p)\left(U^a \partial_a U_b+U_b \partial_a U^a\right)+\partial_b p .
\end{equation}
式\eqref{chlh:eqn_pfm}可分别在共动观者的时间和空间方向投影．用$U^b$缩并式\eqref{chlh:eqn_pfm}得
\begin{align*}
	0=U^b \partial^a T_{a b}=-U_a \partial^a(\mu+p)+(\mu+p)\left(U^b U^a \partial_a U_b-\partial_a U^a\right)+U^b \partial_b p .
\end{align*}
由四速度属性可得：
$U^b U^a \partial_a U_b=\frac{1}{2} U^a \partial_a\left(U^b U_b\right)
=\frac{1}{2} U^a \partial_a\left(-1\right)=0$．
便有
\begin{equation}\label{chlh:eqn_pfm-t}
	U^a \partial_a \mu+(\mu+p) \partial_a U^a=0.
\end{equation}
上此即为式\eqref{chlh:eqn_pfm}在共动观者时间方向的投影，即相对论流体力学时间方向的分方程．
以空间投影算符$h_c^b \equiv \delta_c^b+U_c U^b$作用在式\eqref{chlh:eqn_pfm}上，
便可得相对论流体力学空间方向的分方程：
\begin{align}\label{chlh:eqn_pfm-s}
	(\mu+p) U^a \partial_a U_c+\partial_c p+U_c U^b \partial_b p=0 .
\end{align}
在天体物理或相对论中，压强为零的理想流体叫尘埃（dust）．
对尘埃而言，式\eqref{chlh:eqn_pfm-s}简化为 $U^a \partial_a U_c=0$，
可见尘埃的世界线为测地线．

下面给出式\eqref{chlh:eqn_pfm-t}、\eqref{chlh:eqn_pfm-s}的低速、低能的非相对论近似．
我们选任一惯性参考系$\left\{t, x^i\right\}$，并对四速度$U^a$作$1+3$分解（注Lorentz因子$\gamma \approx 1$）：
\begin{equation}\label{chlh:eqn_Utu}
	U^a=\gamma\left[\left(\tfrac{\partial}{\partial t}\right)^a+u^a\right] 
	\approx \left(\tfrac{\partial}{\partial t}\right)^a+u^a.
\end{equation}
其中$u^a$是流体在该系中的三维速度．
把上式代入式\eqref{chlh:eqn_pfm-t}，得（注 $p \ll \mu$）
\begin{align}
	0=\left[ \left(\tfrac{\partial}{\partial t}\right)^a+u^a \right]  \partial_a \mu
	+\mu \partial_a \left[ \left(\tfrac{\partial}{\partial t}\right)^a+u^a \right] 
	\xlongequal{\ref{chccr:eqn_dpdd=0}}
	\frac{\partial \mu}{\partial t}+\partial_a\left(\mu u^a\right).
\end{align}
因 $u^a$ 是惯性系$\{t,x^i\}$的空间矢量，且此惯性系的基矢是常矢量，
故$\partial_a\left(\mu u^a\right)=\partial_i\left(\mu u^i\right)={\nabla} \cdot(\mu \boldsymbol{u})$ ．
所以上式即为流体力学中的连续性方程．

用$\left(\partial / \partial x^i\right)^c$与式\eqref{chlh:eqn_pfm-s}缩并，
得（注\eqref{chccr:eqn_dpdd=0}、\eqref{chlh:eqn_Utu}、$p \ll \mu$）
\begin{align}
	0 & =\mu\left[\left(\tfrac{\partial}{\partial t}\right)^a +u^a \right]\partial_a u_i 
	+\left(\tfrac{\partial}{\partial x^i}\right)^c \partial_c p
	+u_i\left[\left(\tfrac{\partial}{\partial t}\right)^b+u^b\right] \partial_b p \notag \\
	& =\mu\left(\frac{\partial u_i}{\partial t}+u^j \partial_j u_i\right)
	+\frac{\partial p}{\partial x^i}+u_i \frac{\partial p}{\partial t}
	+u_i u^j \frac{\partial p}{\partial x^j}. 
\end{align}
在相对论低速（$|u_i| \ll 1$）近似下还有$|u_i \partial p / \partial t| \ll |\partial p / \partial x^i|$ 
和$|u_i u^j \partial p / \partial x^j| \ll |\partial p / \partial x^i|$，
所以上式第二行最后两项与$\partial p / \partial x^i$相比可以忽略．
此式即为流体力学中的欧拉方程（无粘的Navier--Stokes方程）．

请读者将上述低速近似方程与连续性方程\cite[\S 1]{landau_6-fluid}和欧拉方程\cite[\S 2]{landau_6-fluid}作对比．
...
总之，如果你期待引力场的能动量（stress-energy）满足：引力理论是度规引力理论，
等效原理成立（比如GR）只依赖于局域的引力场．
即，是度规和度规的偏导数的函数．（别忘了GR里度规的协变导数总是零．）是一个张量．
即，在任意坐标变换下transforms as a tensor.满足守恒律．
即，协变散度为零．那么，你将无法找到一个合适的能动量的定义，
因为唯一同时满足上述四个条件的张量是0．
