...
导数$\dot{q}^i(t)$；此时$v^i\equiv \dot{q}^i(t)$\uwave{显示}地依赖$q^i(t)$，
...
在式\eqref{chlh:eqn_2ode-t}中$\dot{\boldsymbol{r}}$自然\uwave{不}独立于$\boldsymbol{r}$，注意此方程是二阶的．

在式\eqref{chlh:eqn_1ode-t}中独立变量是$(\boldsymbol{r}, \boldsymbol{v})$，
$\boldsymbol{v}=\dot{\boldsymbol{r}}$自然\uwave{独立}于$\boldsymbol{r}$；此方程是一阶的．


到底独立不独立？是不是很奇怪？若增加下标，则问题可能就清晰了！

在式\eqref{chlh:eqn_2ode-t}中$\dot{\boldsymbol{r}}_2$自然\uwave{不}独立于$\boldsymbol{r}_2$，注意此方程是二阶的．

在式\eqref{chlh:eqn_1ode-t}中$\dot{\boldsymbol{r}}_1$自然\uwave{独立}于$\boldsymbol{r}_1$，注意此方程是一阶的．
...
\end{equation}
两个导数并不相等！
这说明虽然两个映射的像重合，但我们\uwave{没有理由}认为
...
输出结果是缩并后的标量场．
映射$\mathscr{L}_2$第一个宗量是$(2,0)$型张量场，第二宗量位置是光滑\uwave{余}切矢量场；
输出结果是缩并后的标量场．
...
\begin{remark}\label{chlh:rek_dynamical-variables}
    ...
    为此我们约定\uwave{动力学变量}为：实物粒子的动力学变量角标在上面，比如位矢$x^a$、速度$u^a$等等；
...
只能说为：电荷守恒与电磁场规范势的规范变换\uwave{相容}（compatible）．
