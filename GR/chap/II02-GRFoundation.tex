...
\noindent\fbox{\heiti  丙}：描述物质（包括有质量的粒子和无质量的粒子）运动的方程是：
...
\noindent\fbox{\heiti 丁}：物质场（$T_{ab}$）分布决定时空弯曲，这由下述爱因斯坦引力场方程来描述：
...
\section{Fermi--Walker导数}\label{chfd:sec_FW}

本节较多参考了文献\parencite[\S 4.1]{hawking-ellis1973}、\parencite[\S 6.5]{mtw1973}．

这是一节依附于物理的数学知识，不宜放在纯数学部分，故放在这里讲述．
...
由式\eqref{chfd:eqn_DF-Z=0}、\eqref{chfd:eqn_DF-ei}不难看出：
...
\begin{equation}\label{chfd:eqn_4DJSD}
	\Omega_{\mu\nu}=\begin{pmatrix}
		0 & A_1 & A_2 & A_3 \\
...
此加速度的分量$A_0$未必是零，但不出现在上述角速度矩阵\eqref{chfd:eqn_4DJSD}中．

我们已经学过了很多导数，作一个简单的对比是有教益的，见表\ref{chfd:tab-Derivative-all}．
...
\begin{table}[htb]
	...
	\begin{tabular}{|*3{c|}}
		...
		李导数 & $\Lie_{X}$ & 预先给定矢量场$X^a$，依赖于$X^a$的诱导单参数同胚群 \\ \hline
		Fermi导数 & $\frac{{\rm D}_F }{{\rm D}\tau}$ &
		预先给定类时线$G(\tau)$；借助协变导数；只适用于闵氏时空 \\ \hline
	\end{tabular}
\end{table}
...
其中$\epsilon_{\alpha\mu\nu\beta}$是体积元\eqref{chrg:eqn_volume-element}在固有坐标系的分量，
故$\epsilon_{0123}=1=-\epsilon^{0123}$（见\eqref{chrg:eqn_RV}）；
记$\epsilon^{ijk}=\epsilon_{ijk}=\epsilon_{0ijk}$．
...
牛顿引力论的背景流形是$\mathbb{R}^4$，其上附加有如下结构：
