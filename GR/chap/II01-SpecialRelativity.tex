...
\index[physwords]{伽利略变换} \index[physwords]{Galilei变换}

\subsection{Galilei变换}\label{chsr:sec_Galileo}
在相对论发现之前的经典物理学
...
当把Galilei变换用到光速时会有矛盾．比如某运动员在踢足球，足球从他的脚飞出来后
具有速度$v$，那么从他脚上反射的光速是$c$，而从足球上反射的光速是$v+c$；这样
因足球上出来的光先到你的眼睛里，脚上出来的光后到你的眼睛里，故你先看到足球从
他的脚上飞出来，再看到他的脚踢球．因果关系颠倒了！

还有很多其它实验与Galilei变换不符，可参阅文献\parencite{jackson1998,liu_fei2008,zhangyz-2023}相应章节．
...
只不过它要求力学理论对Galilei变换不变．
光速不变原理是一个非常大胆假设，这个假设与Galilei变换直接相悖．
...
\subsection{Lorentz变换}\label{chsr:sec_lorentz-transofrm}
由间隔不变性（式\eqref{chsr:eqn_interval-inv}）出发，容易求得{\heiti Lorentz变换}：
...
\begin{equation}\label{chsr:eqn_lorentz-transofrm-x-inv}
    ...
    z = z'.
\end{equation}
从Lorentz变换容易看到，时间和空间不再彼此独立，两者可以相互转化．

Lorentz变换\eqref{chsr:eqn_lorentz-transofrm-x}的微分形式为：
...
式\eqref{chsr:eqn_lorentz-transofrm-dx}就退化到Galilei变换\eqref{chsr:eqn_galileo-trans-dx}．
然而，在有限距离的Lorentz变换\eqref{chsr:eqn_lorentz-transofrm-x}中，当$x$是个大量（比如日地距离）时，
变换\eqref{chsr:eqn_lorentz-transofrm-x}中第一式变为$t' \approx t-\frac{v}{c^2}x$，
不能退化到Galilei变换\eqref{chsr:eqn_galileo-trans-x}．
...
Lorentz变换的发现历史十分复杂，读者如有兴趣可参阅相应文献，比如Wikipedia条目
{\footnote{\url{https://en.wikipedia.org/wiki/History\_of\_Lorentz\_transformations}}}．
Lorentz本人为了区分自己的理论与爱因斯坦的理论，而将爱氏理论称为“相对论”．




把前面给出的时空Lorentz变换\eqref{chsr:eqn_lorentz-transofrm-x}用四维矩阵再表述一次：
...
就退化到Galilei变换\eqref{chsr:eqn_galileo-trans-vel}．

\begin{example}
    证明在Lorentz速度变换\eqref{chsr:eqn_lorentz-transofrm-vel}下，光速不变．
\end{example}
...
\end{equation}
这说明在Lorentz变换下，若速度是不变量，则必是光速．\qed
...
\begin{remark}
    笔者认为“时空间隔不变性”是比“Lorentz变换”更基本的内容，间隔不变性是狭义相对论两条基本假设的数学表述．
    有学者先给出Lorentz变换，然后用它再导出时空间隔不变；笔者个人认为这种叙述方式逻辑颠倒了．
\end{remark}
...
都要受到它的约束；数学上来说就是这些粒子的动力学方程要Lorentz协变．
在牛顿绝对空间的时空观下，经典力学满足Galilei变换，显然不是Lorentz协变的；我们需要修改牛顿
力学使之变成Lorentz协变，并且在质点运动速度远远小于光速时，它能退化到Galilei变换．
...
\index[physwords]{Lorentz四速度} \index[physwords]{四速度} \index[physwords]{四加速度}
...
为Lorentz标量；$\boldsymbol{u}$是质点3-速度．
...
才能得到，比如电磁学中带电粒子所受Lorentz力是$\boldsymbol{F}=q \boldsymbol{E}+q \boldsymbol{u}\times\boldsymbol{B}$．
...
相对论三维力$\boldsymbol{F}=\gamma_{u}^{-1} \boldsymbol{K}$的Lorentz变换公式
\begin{equation}
...
设光子（即电磁波量子）的传播方向是单位矢量$\boldsymbol{n}$，
其频率是$\nu$，角频率$\omega=2\pi \nu$；
...
下面讨论一下光子四波矢的Lorentz变换．
设选定惯性系$S$为静系，另选一个惯性系$S'$，它与$S$系的坐标轴相互平行且同向；
$S'$系相对于$S$系以速度${\boldsymbol{v} }=v {\boldsymbol{e}}_x$沿$x$轴作匀速运动，
两系波矢量间的Lorentz变换是
\begin{equation}\label{chsr:eqn_photon-k}
	\omega' = \gamma \left({\omega-v k_x}\right), \quad
	k'_x = \gamma \left({k_x-\dfrac{v}{c^2}\omega }\right), \quad
	k'_y = k_y, \quad
	k'_z = k_z.
\end{equation}
四维波矢量表征光子（也就是电磁波）的传播方向、频率大小、波长等信息．


设光波传播方向${\boldsymbol{k}}$与${\boldsymbol{v}}$所在平面为$x-y$平面，
${\boldsymbol{k}}$与${\boldsymbol{v}}$的夹角为$\theta$，
${\boldsymbol{k}}'$与${\boldsymbol{v}}$的夹角为$\theta'$；
因${\boldsymbol{v}}$沿$x(x')$轴，
故此夹角也是${\boldsymbol{k}}({\boldsymbol{k}}')$与$x(x')$轴的夹角．则有
\begin{align*}
	\cos\theta &= \dfrac{{\boldsymbol{k}}\cdot{\boldsymbol{v}}}{k v} 
	= \dfrac{{\boldsymbol{k}}\cdot{\boldsymbol{e}}_x}{k} = \dfrac{k_x}{k}
	\quad \xLongrightarrow{\omega=kc}  \quad 
	c k_x = \omega \cos\theta ,\\
	\cos\theta' &= \dfrac{{\boldsymbol{k}'}\cdot{\boldsymbol{v}}}{k' v} 
	= \dfrac{{\boldsymbol{k}'}\cdot{\boldsymbol{e}}_x}{k'} = \dfrac{k'_x}{k'}
	\quad \xLongrightarrow{\omega'=k'c} \quad 
	c k'_x = \omega' \cos\theta' . 
%	k_y & = k\sin\theta =   k'\sin\theta' = k'_y .
\end{align*}
将上式带入式\eqref{chsr:eqn_photon-k}可得（求解过程留作习题）：
\begin{align}
	\omega' &= \omega \gamma \left( 1- \dfrac{v}{c}\cos\theta\right) . \label{chsr:eqn_doppler} \\
	\tan\theta' &= \dfrac{\gamma^{-1}\sin\theta}{\cos\theta -v/c} 
	=\dfrac{ \sqrt{1-v^2/c^2}\ c\sin\theta}{c \cos\theta -v} . \label{chsr:eqn_aberration}
\end{align}
式\eqref{chsr:eqn_doppler}称为{\heiti\bfseries Doppler公式}．
式\eqref{chsr:eqn_aberration}称为{\heiti 光行差公式}．


\index[physwords]{Doppler效应} \index[physwords]{多普勒效应} 
\index[physwords]{光行差公式}


下面略微讨论一下Doppler效应．
设光源相对于$S'$系静止，光源在此系发光的频率记为固有频率$\omega'=\omega_0=2\pi \nu_0$；
在$S$系来看，光源是运动的，此时看到的频率由Doppler公式\eqref{chsr:eqn_doppler}可得
\begin{equation}\label{chsr:eqn_doppler-omega}
	\omega = \dfrac{\omega_0}{ \gamma \left( 1- \beta\cos\theta\right) }
	=\omega_0 \dfrac{ \sqrt{1-\beta^2}}{  1-\beta\cos\theta }, \qquad \beta = \frac{v}{c}.
\end{equation}
有几种特殊情形：

\fbox{甲}如果$\theta=0$，光源朝着观测者飞来，观测者收到的频率大于固有频率，
电磁波被压缩，波长变短，称为蓝移：
\begin{equation}\label{chsr:eqn_doppler-blue}
	\omega %=\dfrac{\omega_0 \sqrt{1-\beta^2}}{  1-\beta }
	=\omega_0 \sqrt{\frac{1+\beta}{1-\beta}} 
	\xlongequal[\text{很小}]{\text{当}\, \beta} 
	\omega_0 \left(1+\beta +\frac{\beta ^2}{2}+O(\beta ^3)\right).
\end{equation}

\fbox{乙}如果$\theta=\pi$，光源背向观测者飞离，观测者收到的频率小于固有频率，
电磁波被拉伸，波长变长，称为红移：
\begin{equation}\label{chsr:eqn_doppler-red}
	\omega %=\dfrac{\omega_0 \sqrt{1-\beta^2}}{  1+\beta }
	=\omega_0 \sqrt{\frac{1-\beta}{1+\beta}}
	\xlongequal[\text{很小}]{\text{当}\, \beta}
	\omega_0 \left(1-\beta +\frac{\beta ^2}{2}+O(\beta ^3)\right).
...
\fbox{丙}如果$\theta=\pi/2$，光源相对于观测者横向运动，也产生红移：
\begin{equation}\label{chsr:eqn_doppler-red-perp}
	\omega =\omega_0 \sqrt{1-\beta^2}
	\xlongequal[\text{很小}]{\text{当}\, \beta} 
	\omega_0 \left(1-\frac{\beta ^2}{2}-\frac{\beta ^4}{8}+O\left(\beta ^5\right)\right).
\end{equation}
这种横向Doppler现象是相对论特有的，非相对论情形不存在．
...
\end{pmatrix} .
\end{equation}
惯性系间的变换由Lorentz变换描述．惯性系到匀加速运动参考系间的变换由式\eqref{chsr:eqn_Rindler-trans}、
...
Lorentz力\eqref{chsr:eqn_lorenz-force}（再加上牛顿第二定律）构成了宏观电磁动力学
...
丹麦物理学家Lorenz{\footnote{L. V. Lorenz（洛伦茨）不是荷兰物理学家
		Lorentz（H. A. Loren{\textcolor{red}{t}}z），
...
如果规范势$A_\mu$不满足Lorenz规范，即$\Theta \equiv \partial_\mu A^\mu \neq 0$，
那么可作规范变换\eqref{chsr:eqn_gauge-trans-covariant}导致新的
规范势$A^{\prime}_{\mu}$满足Lorenz规范：
\begin{equation}
...
新的规范势$A^{\prime}_{\mu}$满足Lorenz规范$\partial^\mu A^{\prime}_{\mu}=0$．
这也就说明了：我们一定有办法使Lorenz规范得以满足．
...
\subsubsection{规范势$A^\mu$是Lorentz四矢量吗？}\label{chsr:sec_isAvec}
...
在不指定规范条件的前提下，规范势$A^\mu$被{\heiti 人为地规定}为Lorentz四矢量．
显然，如果选取的规范条件是Lorentz协变的（比如Lorenz规范条件），那么
规范势也是四矢量．
如果选取的规范条件{\heiti 不}是Lorentz协变的（比如库仑规范或瞬时规范），
...
{\bfseries (2)} 它的度规类型是Lorentz的，即度规特征值是$(-1,1,1,1)$．
...
一定是Lorentz变换\eqref{chsr:eqn_lorentz-transofrm-x}或其逆变换；
...
我们仍需假设时空是四维连续的、连通的空间，这个空间的度规是Lorentz度规．
...
时空间隔是参考系变换的不变量，也就是Lorentz变换下的不变量．根据时空间隔\eqref{chsr:eqn_interval-inv-ds}，
...
由于连续Lorentz变换不改变
时间的正负，所以区域I和II是不可能通过Lorentz变换相联系的；这种划分是绝对的．
...
区域的任何两个事件没有因果关系，可以通过Lorentz变换改变事件的发生时序．
如果选择两参考系速度差$v$的范围是$c^2\frac{\Delta t}{\Delta x}<v<c$，
那么就可以颠倒事件$O$和$Q$的先后顺序；但
这不违反因果律，因为这两个事件没有因果关系．

在第\ref{chsr:sec_lorentz-transofrm}节末尾曾经讨论过Lorentz变换把光速变成光速，
把亚光速变成亚光速，把超光速变成超光速；所以在Lorentz变换下上述划分是绝对的，
变换前后不会出现跨区现象．
...
\end{equation}
其中${\rm d}t$是$S$系坐标时．
注意这里的$\gamma_u$与Lorentz变换\eqref{chsr:eqn_lorentz-transofrm-x}中
...
我们看一下动系的时空坐标是什么样子．
Lorentz变换\eqref{chsr:eqn_lorentz-transofrm-x}是
$t' = \gamma ({t-v x})$，$  x' = \gamma ({x-vt})$．
在静系$t-x$中，$t'$轴就是$x'=0$的世界线，也就是原点$O'$的世界线，必然类时；由Lorentz变换容易得到
它在$t-x$系的方程是$x=vt$，就是斜率倒数是速度$v$的过原点斜直线．
在静系$t-x$中，$x'$轴就是$t'=0$的世界线，必然类空；由Lorentz变换容易得到
...
飞船从地球出发再次返回地球，它不可能做匀速运动，肯定有Lorentz四加速度．
\qed
