...
\end{equation*}
这样定义的映射对每个宗量都是线性的，在基矢变化时定义右端项具有
形式不变性．
...
{\kaishu 例一}：牛顿力学将$A$、$B$限定在$SO(3)$中；比如第\ref{chcdg}章中的张量．
{\kaishu 例二}：狭义相对论将$A$、$B$限定在$SO^{+}(1,3)$中，称为{\heiti \bfseries Lorentz四矢量}，简称四矢量．
{\kaishu 例三}：量子力学将$A$、$B$限定在$SU(2)$中，称为Pauli旋量．
{\kaishu 例四}：量子场论将$A$、$B$限定在$SL(2,\mathbb{C})$中，称为$SL(2,\mathbb{C})${\heiti 旋量}（或Lorentz旋量）；
见第\ref{chlar}章．
