...
以后还会介绍\CJKunderwave{标量积}（也称为半内积）概念，此时$\left<\alpha,\alpha\right>$可正可负可为零．
...
因为其系数$y$和$-x$是{\CJKunderwave{变量}}，不是数域$\mathbb{C}$中{\CJKunderwave{常数}}，所以
...
系数只能是$\mathbb{F}$值\CJKunderwave{常数}，\CJKunderwave{不能是变数}．
...
称它是\CJKunderwave{$V$对子空间$W$的商集}，即
...
下面开始着重讲述较为简单的双线性函数．
由定理\ref{chmla:thm_bifunmatrix}易得双线性\CJKunderwave{函数}空间$\mathscr{L}(U,V;\mathbb{F})$的基矢
...
\begin{proposition}
    ...
    那么$V$上度规$g$的正（负）特征值个数等于$W$上度规$g|_W$正（负）特征值个数\CJKunderwave{加上}
...
比如我们可以把对偶空间定义\ref{chmla:def_dual-space}中对偶基\eqref{chmla:eqn_dual-bases}略作限制，
以便更符合物理中的情形；我们约定：
\CJKunderwave{对偶基\eqref{chmla:eqn_dual-bases}中的$\delta_i^j$必须是无量纲的}．
也就是说，空间$V$和其对偶空间$V^*$的量纲必须互为倒数；
比如位矢空间（$\boldsymbol{r}$）的量纲是{\kaishu 长度}，
...
\begin{example}\label{chmla:exm_T11TR}
    ...
    $\Psi$是线性同构映射．因此：\CJKunderwave{$V$上线性变换等同于$\binom{1}{1}$张量}． \qed
\end{example}
...
来$\alpha \otimes \beta =\beta \otimes \alpha$是\CJKunderwave{不正确}的．
...
型式$\alpha$的\CJKunderwave{求值公式}，需注意
